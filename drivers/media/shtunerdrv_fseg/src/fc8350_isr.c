...
s32 (*fc8350_ac_callback)(ulong userdata, u8 bufid, u8 *data, s32 length);
s32 (*fc8350_ts_callback)(ulong userdata, u8 bufid, u8 *data, s32 length);
ulong fc8350_ac_user_data;
...
static void fc8350_data(HANDLE handle, DEVICEID devid, u8 buf_int_status)
{
	...
	for (i = 4; i < 8 && (buf_int_status & 0xf0); i++) {
		if (buf_int_status & (1 << i)) {
			...
			if (fc8350_ac_callback)
				(*fc8350_ac_callback)(fc8350_ac_user_data,
						i, &ac_buffer[0], size);
		}
	}
}
...
static void fc8350_aux_int(HANDLE handle, DEVICEID devid, u8 aux_int_status)
{
	...
	if (aux_int_status & AUX_INT_SYNC_RELATED_INT) {
		u8 sync = 0;
		bbm_byte_read(handle, devid, BBM_SYS_MD_INT_CLR, &sync);

		if (sync) {
			bbm_byte_write(handle, devid, BBM_SYS_MD_INT_CLR,
								sync);
