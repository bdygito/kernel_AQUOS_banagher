...
const struct of_device_id fc8350_match_table[] = {
	{
	...
	, u8 *txbuf, u16 tx_length, u8 *rxbuf, u16 rx_length)
{
	...
	if (x.len > 188) {
		...
		for (i = tx_length; i >= 0; i--)
			rdata_buf[i-1] = txbuf[tx_length - i];
	} else
#endif
	x.bits_per_word = 8;
#ifdef EVB
...
#ifdef FC8350_DEBUG
	print_log(0, "fc8350_spi_bulkread ok : %x,%x\n", data[0], data[1]);
#endif
	return BBM_OK;
}
...
#ifdef FC8350_DEBUG
	print_log(NULL, "FC8350_SPI dataread addr=%x length=%d  %x %x res=%d\n",addr, length, data[0], data[1], res);
#endif
	return res;
}
