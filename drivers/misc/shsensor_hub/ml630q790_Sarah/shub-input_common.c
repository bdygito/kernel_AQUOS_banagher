...
/* SHMDS_HUB_0138_01 add S */
#ifdef SHUB_SW_POWER_RESET
int shub_check_regulator(void)
{
    int ret = 0;
    COM_DBG("[shub]%s: begin", __FUNCTION__);

    if (shub_regulator == NULL) {
        shub_regulator = devm_regulator_get(&client_mcu->dev, "shub_reset");
        if (IS_ERR(shub_regulator)){
            printk("[shub]%s: devm_regulator_get Error(IS_ERR(vreg)=%d)", __FUNCTION__, IS_ERR(shub_regulator));
            shub_regulator = NULL;
            return -1;
        }
    }
    ...
    if (regulator_is_enabled(shub_regulator)) {
        COM_DBG("[shub]%s: regulator is enable", __FUNCTION__);
        shub_reset_enable = SHUB_RESET_H;
    } else {
        COM_DBG("[shub]%s: regulator is disable", __FUNCTION__);
        shub_reset_enable = SHUB_RESET_L;
    }

    if (shub_reset_enable == SHUB_RESET_H) {
        COM_DBG("[shub]%s: call regulator_enable", __FUNCTION__);
        ret = regulator_enable(shub_regulator);
        if (ret != 0){
            printk("[shub]%s: regulator_enable Error(ret=%d)", __FUNCTION__, ret);
            regulator_put(shub_regulator);
            shub_regulator = NULL;
            return ret;
        }
    }
    COM_DBG("[shub]%s: end", __FUNCTION__);
    return ret;
}
...
int shub_reset_signal(int enable)
{
    ...
    if (enable == SHUB_RESET_H) {
        ...
        COM_DBG("[shub]%s: call regulator_disable", __FUNCTION__);
...
void shub_release_regulator(void)
{
    COM_DBG("[shub]%s: begin", __FUNCTION__);
    if(shub_regulator != NULL) {
        COM_DBG("[shub]%s: release regulator", __FUNCTION__);
        devm_regulator_put(shub_regulator);
        shub_regulator = NULL;
    }
    shub_reset_enable = 0xff;
    COM_DBG("[shub]%s: end", __FUNCTION__);
}
