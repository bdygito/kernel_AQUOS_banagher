...
#define LSM6DS3_ACC_GYRO_TAP_CFG0       (0x56)  /* SHMDS_HUB_0103_41 mod */
...
#define LSM6DS3_SLAVE_ADDRESS           (0x6Bu)
...
#define ACC_MIN_ST (819)   /* SHMDS_HUB_0103_40 mod */
#define ACC_MAX_ST (27868)  /* SHMDS_HUB_0103_40 mod */
#endif
/* SHMDS_HUB_0103_32 mod E */
#define CHECK_GYRO_REG (2)
#define GYRO_MIN_ST (4285)   /* SHMDS_HUB_0103_40 mod */
#define GYRO_MAX_ST (20000)  /* SHMDS_HUB_0103_40 mod */
/* SHMDS_HUB_0103_14 add E */
...
#else
#define SELF_TEST_ACC_SLEEPTIMES (100)   /* SHMDS_HUB_0103_40 mod */
#endif
/* SHMDS_HUB_0103_32 add E */
#define SELF_TEST_GYRO_SLEEPTIMES (100)  /* SHMDS_HUB_0103_40 add */
...
static int shub_diag_acc_initialize(void)
{
    ...
    setData = 0xE0;  /* SHMDS_HUB_0103_41 mod (0x38->0xE0) */
    ...
    /* HR mode HPF disable */
    setData = 0x00;
    ret = shub_diag_accGyroSensorWriteSendCmd(LSM6DS3_ACC_GYRO_TAP_CFG0, setData);  /* SHMDS_HUB_0103_41 mod */
    if(ret != 0) {
        return -1;
    }
    /* HP & LPF disable */
...
/* SHMDS_HUB_0103_32 mod S */
    ...
    /* Accelerometer axis output enable */
    setData = 0x00;  /* SHMDS_HUB_0103_40 mod */
    ret = shub_diag_accGyroSensorWriteSendCmd(LSM6DS3_ACC_GYRO_CTRL9_XL, setData);
    if(ret != 0) {
        return -1;
    }
...
/* SHMDS_HUB_0103_41 mod S */
    setData = 0xF0;
    ret = shub_diag_accGyroSensorWriteSendCmd(LSM6DS3_ACC_GYRO_CTRL9_XL, setData);
    if(ret != 0) {
        return -1;
    }
/* SHMDS_HUB_0103_41 mod E */
    /* Block data update enable */
    setData = 0x44;
    ret = shub_diag_accGyroSensorWriteSendCmd(LSM6DS3_ACC_GYRO_CTRL3_C, setData);
    if(ret != 0) {
...
static int shub_diag_gyro_self_test_initialize(void)
{
    ...
    /* CTRL2_G setting */
    setData = 0x58;  /* SHMDS_HUB_0103_40 mod */
    ret = shub_diag_accGyroSensorWriteSendCmd(LSM6DS3_ACC_GYRO_CTRL2_G, setData);
    if(ret != 0) {
        return -1;
    }
...
static int shub_diag_gyro_mode_change(int mode)
{
    ...
    if(mode){
        ...
        setData = 0x78;     /* SHMDS_HUB_0136_01 SHMDS_HUB_0103_41 mod (0x7C->0x78->58->0x78) */
    }
    else{
        /* power down mode */
        setData = 0x00;
...
static int shub_diag_gyro_self_test_check(int16_t* gyroData)
{
	...
	msleep(SELF_TEST_GYRO_SLEEPTIMES);  /* SHMDS_HUB_0103_40 mod */
...
/* SHMDS_HUB_0103_34 add S */
        case SHUB_DIAG_START_GYRO:
            {
                struct IoctlDiagRes res;
                memset(&res, 0, sizeof(struct IoctlDiagRes));
                if(!initial_gyro_flg){
                ...
                if(res.rtn == 0){
                    res.rtn = shub_diag_gyro_mode_change(1);
                }
                ret = copy_to_user(argp, &res, sizeof(struct IoctlDiagRes));
                if (ret) {
                    printk("error : shub_ioctl(cmd = SHUB_DIAG_START_GYRO)\n");
                    return -EFAULT;
                }
            }
            break;
        case SHUB_DIAG_START_ACC:
            {
                struct IoctlDiagRes res;
                memset(&res, 0, sizeof(struct IoctlDiagRes));
#ifndef CONFIG_ACC_U2DH
                ...
                if(res.rtn == 0){
                    res.rtn = shub_diag_acc_mode_change(1);
                }
#endif
                ret = copy_to_user(argp, &res, sizeof(struct IoctlDiagRes));
                if (ret) {
                    printk("error : shub_ioctl(cmd = SHUB_DIAG_START_ACC)\n");
                    return -EFAULT;
                }
            }
            break;
        case SHUB_DIAG_START_BARO:
            {
                struct IoctlDiagRes res;
                memset(&res, 0, sizeof(struct IoctlDiagRes));
                if(!initial_baro_flg){
                ...
                if(res.rtn == 0){
                    res.rtn = shub_diag_baro_mode_change(1);
                }
                ret = copy_to_user(argp, &res, sizeof(struct IoctlDiagRes));
                if (ret) {
                    printk("error : shub_ioctl(cmd = SHUB_DIAG_START_BARO)\n");
                    return -EFAULT;
                }
            }
            break;
        case SHUB_DIAG_START_MAG:
            {
                struct IoctlDiagRes res;
                memset(&res, 0, sizeof(struct IoctlDiagRes));
                if(!initial_mag_flg){
                ...
                ret = copy_to_user(argp, &res, sizeof(struct IoctlDiagRes));
                if (ret) {
                    printk("error : shub_ioctl(cmd = SHUB_DIAG_START_MAG)\n");
                    return -EFAULT;
                }
            }
            break;
/* SHMDS_HUB_0103_34 add E */
...
#else
                printk("error : SMEM is not supported: \n");
                return -EFAULT;
#endif
