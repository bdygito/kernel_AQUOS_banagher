...
#define SHUB_STILL_THR     50      /* SHMDS_HUB_0362_02 add */
...
//del static int32_t input_gyrounc[INDEX_SUM]= {0};
//del static int32_t input_flg = 0;
...
//del static struct timespec shub_normal_last_ts;
...
//del static int64_t shub_local_ts_ns_old = 0;		/* SHMDS_HUB_0311_02 add */
...
/* SHMDS_HUB_0362_04 mod S */
        ...
        && (shub_get_sensor_same_delay_flg(SHUB_SAME_NOTIFY_GYRO) == 1)){
            if(shub_get_sensor_first_measure_info(SHUB_SAME_NOTIFY_GYRO) & SHUB_ACTIVE_SENSOR) {
                ...
                xyz_gyro[INDEX_X] = xyz[INDEX_X] - xyz[INDEX_X_BIAS];
                xyz_gyro[INDEX_Y] = xyz[INDEX_Y] - xyz[INDEX_Y_BIAS];
                xyz_gyro[INDEX_Z] = xyz[INDEX_Z] - xyz[INDEX_Z_BIAS];
                xyz_gyro[INDEX_ACC] = xyz[INDEX_ACC];
                xyz_gyro[4] = xyz[INDEX_TM];
                xyz_gyro[5] = xyz[INDEX_TMNS];
...
static void shub_set_sensor_poll(int32_t en)
{
    ...
    if(en){
//del         input_flg = 0; /* SHMDS_HUB_0321_01 add */
/* SHMDS_HUB_0362_02 add S */
#ifdef SHUB_SW_GYRO_OFFSET
        shub_offset_flg = 0;
        shub_notify_cnt = 0;
