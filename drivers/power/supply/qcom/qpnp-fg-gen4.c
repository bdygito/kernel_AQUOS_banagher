...
#ifdef CONFIG_BATTERY_SHARP
	rc = of_property_read_u32(profile_node, "sharp,ibatt-full-ma",
			&temp);
	if (!rc) {
		chip->dt.sys_term_curr_ma = temp;
	} else {
		pr_err("can't read ibatt-full-ma, rc:%d\n", rc);
	} 
#endif /* CONFIG_BATTERY_SHARP */
...
static int fg_gen4_bp_params_config(struct fg_dev *fg)
{
	...
	rc = fg_sram_write(fg, fg->sp[FG_SRAM_SYS_TERM_CURR].addr_word,
			fg->sp[FG_SRAM_SYS_TERM_CURR].addr_byte, buf_array,
...
#ifdef CONFIG_BATTERY_SHARP
	rc = power_supply_get_property(fg->batt_psy, POWER_SUPPLY_PROP_SW_HEALTH,
		&prop);
...
#ifdef CONFIG_BATTERY_SHARP
		if (msoc >= 95 && (fg->health == POWER_SUPPLY_HEALTH_GOOD || fg->health == POWER_SUPPLY_HEALTH_COOL)) {
#else /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
		if (chip->cl->active) {
			if((fg->profile_load_status != PROFILE_LOADED) && !chip->cl->learned_cap_uah) {
				int act_cap_mah;
				rc = fg_get_sram_prop(fg, FG_SRAM_ACT_BATT_CAP, &act_cap_mah);
				if (rc < 0) {
				...
				fg_dbg(fg, FG_CAP_LEARN, "Get learned capacity from sram %lld\n", chip->cl->learned_cap_uah);
			}
