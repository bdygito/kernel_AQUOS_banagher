...
#ifdef CONFIG_BATTERY_SHARP
static int qg_get_battery_padded_capacity(struct qpnp_qg *chip, int *soc)
{
	if (!chip->dt.correct_capacity_enable)
		return qg_get_battery_capacity(chip, soc);
...
static int qg_get_ttf_param(void *data, enum ttf_param param, int *val)
{
	...
	case TTF_CHG_DONE:
		*val = chip->charge_done;
		break;
#ifdef CONFIG_BATTERY_SHARP
	case TTF_DIRECTCHARGE_ACTIVE:
		*val = is_directcharge_active(chip);
		break;
#endif /* CONFIG_BATTERY_SHARP */
	default:
...
#ifdef CONFIG_BATTERY_SHARP
	chip->cl->current_batt_age_level = batt_age_level;
#endif /* CONFIG_BATTERY_SHARP */
			       ...
			       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_STORED_LEARNED_CAPACITY:
		...
		break;
	case POWER_SUPPLY_PROP_PADDED_CAPACITY:
		if (chip->dt.correct_capacity_enable)
			rc = qg_get_battery_padded_capacity(chip, &pval->intval);
		else
			rc = qg_get_battery_capacity(chip, &pval->intval);
		break;
	case POWER_SUPPLY_PROP_PADDED_CAPACITY_RAW:
		pval->intval = chip->catch_up_padded_soc;
		break;
#endif /* CONFIG_BATTERY_SHARP */
	default:
...
static enum power_supply_property qg_psy_props[] = {
	...
	POWER_SUPPLY_PROP_PADDED_CAPACITY,
	POWER_SUPPLY_PROP_PADDED_CAPACITY_RAW,
#endif /* CONFIG_BATTERY_SHARP */
};
...
#ifdef CONFIG_BATTERY_SHARP
	if (is_directcharge_active(chip))
		return false;
#endif /* CONFIG_BATTERY_SHARP */
	...
	if (!chip->dt.cl_disable) {
		...
		if (rc < 0) {
			...
			if(!chip->profile_loaded && !chip->cl->learned_cap_uah) {
				...
				qg_dbg(chip, QG_DEBUG_ALG_CL, "Get learned capacity from sram %lld\n", chip->cl->learned_cap_uah);
			}
#endif /* CONFIG_BATTERY_SHARP */
#ifdef CONFIG_BATTERY_SHARP
...
#ifdef CONFIG_BATTERY_SHARP
skip_update:
			qg_dbg(chip, QG_DEBUG_ALG_CL, "batt_soc=%d, batt_temp=%d\n", chip->batt_soc, batt_temp);
#endif /* CONFIG_BATTERY_SHARP */
		}
	}
...
static int qg_load_battery_profile(struct qpnp_qg *chip)
{
...
#ifdef CONFIG_BATTERY_SHARP
			chip->cl->current_batt_age_level = chip->batt_age_level;
#endif /* CONFIG_BATTERY_SHARP */
		}
	...
	rc = of_property_read_u32(profile_node, "sharp,qg-iterm-ma",
			&chip->bp.iterm_ma);
	if (rc < 0) {
		pr_err("battery nominal capacity unavailable, rc:%d\n", rc);
		chip->bp.iterm_ma = -EINVAL;
	}

	if (chip->bp.iterm_ma != -EINVAL)
		chip->dt.iterm_ma = chip->bp.iterm_ma ;
#endif /* CONFIG_BATTERY_SHARP */
...
static int qg_determine_pon_soc(struct qpnp_qg *chip)
{
	...
	if (!shutdown[SDAM_VALID]) {
		pr_info("Using PON_SOC @ PON, SDAM data is not valid\n");
		goto use_pon_ocv;
	}

	if (!is_between(0, chip->dt.ignore_shutdown_soc_secs,
			(rtc_sec - shutdown[SDAM_TIME_SEC]))) {
		pr_info("Using PON_SOC @ PON, Time delta is over ignore_shutdown_time\n");
		goto use_pon_ocv;
	}
	...
	if (!is_between(0, chip->dt.shutdown_temp_diff,
		...
		pr_info("Using PON_SOC @ PON, Temperature delta is over shutdown_temp_diff\n");
		goto use_pon_ocv;
	}
	...
	if ((chip->dt.shutdown_soc_threshold != -EINVAL) &&
		...
		pr_info("Using PON_SOC @ PON, SoC is over shutdown_soc_threshold\n");
		goto use_pon_ocv;
	}
#else
	if (!shutdown[SDAM_VALID])
		...
		if (chip->wa_flags & QG_PON_OCV_WA) {
			if (ocv[S3_LAST_OCV].ocv_raw == FIFO_V_RESET_VAL) {
				if (!ocv[SDAM_PON_OCV].ocv_uv) {
...
#ifdef CONFIG_BATTERY_SHARP
					pr_info("S7_PON_OCV is used as ocv_uv, SDAM_PON_OCV is zero.\n");
#endif /* CONFIG_BATTERY_SHARP */
					...
					ocv_uv = ocv[S7_PON_OCV].ocv_uv;
#ifdef CONFIG_BATTERY_SHARP
					pr_info("S7_PON_OCV is used as ocv_uv, SDAM_PON_OCV is lower than S7_PON_OCV.\n");
#endif /* CONFIG_BATTERY_SHARP */
					...
					ocv_uv = ocv[S7_PON_OCV].ocv_uv;
#ifdef CONFIG_BATTERY_SHARP
					pr_info("S7_PON_OCV is used as ocv_uv, SDAM_PON_OCV is %d[uV] higher than S7_PON_OCV.\n", S7_ERROR_MARGIN_UV);
#endif /* CONFIG_BATTERY_SHARP */
				} else {
					strlcpy(ocv_type, "SDAM_PON_SOC", 20);
					ocv_uv = ocv[SDAM_PON_OCV].ocv_uv;
#ifdef CONFIG_BATTERY_SHARP
					pr_info("SDAM_PON_OCV is used as ocv_uv\n");
#endif /* CONFIG_BATTERY_SHARP */
				}
			} else {
...
#ifdef CONFIG_BATTERY_SHARP
					pr_info("S3_LAST_OCV is used as ocv_uv\n");
#endif /* CONFIG_BATTERY_SHARP */
				} else {
					strlcpy(ocv_type, "S7_PON_SOC", 20);
					ocv_uv = ocv[S7_PON_OCV].ocv_uv;
#ifdef CONFIG_BATTERY_SHARP
					pr_info("S7_PON_OCV is used as ocv_uv, S3_LAST_OCV is lower than S7_PON_OCV.\n");
#endif /* CONFIG_BATTERY_SHARP */
				}
			}
		} else {
			...
			ocv_uv = ocv[S7_PON_OCV].ocv_uv;
#ifdef CONFIG_BATTERY_SHARP
			pr_info("S7_PON_OCV is used as ocv_uv\n");
#endif /* CONFIG_BATTERY_SHARP */
		}
...
#ifdef CONFIG_BATTERY_SHARP
	qg_update_padded_soc(chip, true);
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	rc = qg_sdam_read(SDAM_VALID, &sdam_valid);
	if (rc < 0)
		pr_err("Failed to read sdam vlid rc=%d\n", rc);

	pr_info("using %s @ PON ocv_uv=%duV soc=%d sdam_valid=%d\n",
			ocv_type, ocv_uv, chip->msoc, sdam_valid);
#else
...
#ifdef CONFIG_BATTERY_SHARP
	chip->cl->dt.capacity_delta_force_10pct = false;
	if (of_property_read_bool(node, "sharp,capacity-delta-force-10pct")) {
		chip->cl->dt.capacity_delta_force_10pct = true;
	}
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	chip->dt.correct_capacity_enable = of_property_read_bool(node,
					"sharp,correct-capacity-enable");
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	chip->padded_soc = INT_MIN;
	chip->catch_up_padded_soc = INT_MIN;
#endif /* CONFIGU_BATTERY_SHARP */
