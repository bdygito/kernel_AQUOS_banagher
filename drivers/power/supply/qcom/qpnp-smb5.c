...
#ifdef CONFIG_BATTERY_SHARP
	rc = of_property_read_u32(node, "sharp,shdiag-fv-max-uv",
...
static enum power_supply_property smb5_batt_props[] = {
	...
	POWER_SUPPLY_PROP_SW_HEALTH,
	POWER_SUPPLY_PROP_DIRECT_CHARGE_MODE_EN,
#endif /* CONFIG_BATTERY_SHARP */
};
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TYPEC_OVERHEAT_STATUS:
		val->intval = get_client_vote(chg->cc_disable_votable,
					      USB_OVERHEAT_VOTER);
		break;
	...
	case POWER_SUPPLY_PROP_OFFCHG_MODE:
		...
		break;
	case POWER_SUPPLY_PROP_SW_HEALTH:
		rc = smblib_get_jeita_status(chg, val);
		break;
	case POWER_SUPPLY_PROP_DIRECT_CHARGE_MODE_EN:
		val->intval = get_client_vote(chg->chg_disable_votable, DIRECT_CHARGE_VOTER);
		break;
#endif /* CONFIG_BATTERY_SHARP */
	default:
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX_AGED:
		...
		break;
	case POWER_SUPPLY_PROP_SW_HEALTH:
		chg->sw_health = val->intval;
		break;
	case POWER_SUPPLY_PROP_DIRECT_CHARGE_MODE_EN:
		rc = smblib_set_direct_charge_mode(chg, (bool)val->intval);
		break;
#endif /* CONFIG_BATTERY_SHARP */
	default:
		rc = -EINVAL;
...
static int voltage_max_set(const char *val, const struct kernel_param *kp)
{
    int ret,value;
    struct smb_charger *chg = &the_chip->chg;
    ...
    if(!ret) {
        pr_debug("%s set value to %d \n", __func__, value);
	chg->batt_profile_fv_uv = value;
	vote(chg->fv_votable, BATT_PROFILE_VOTER, true, value);
    }
...
static int voltage_max_get(char *buf, const struct kernel_param *kp)
{
    int ret, value;
    struct smb_charger *chg = &the_chip->chg;
    ...
    value = get_client_vote(chg->fv_votable, BATT_PROFILE_VOTER);
    ret = sprintf(buf, "%d\n", value);
...
module_param_call(param_voltage_max, voltage_max_set, voltage_max_get, NULL, 0644);

static int constant_charge_current_max_set(const char *val, const struct kernel_param *kp)
{
    int ret,value;
    struct smb_charger *chg = &the_chip->chg;
    ...
    if(!ret) {
        pr_debug("%s set value to %d \n", __func__, value);
	chg->batt_profile_fcc_ua = value;
	vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, value);
    }
...
static int constant_charge_current_max_get(char *buf, const struct kernel_param *kp)
{
    int ret, value;
    struct smb_charger *chg = &the_chip->chg;
    ...
    value = get_client_vote(chg->fcc_votable, BATT_PROFILE_VOTER);
    ret = sprintf(buf, "%d\n", value);
...
module_param_call(param_constant_charge_current_max, constant_charge_current_max_set, constant_charge_current_max_get, NULL, 0644);
...
static int input_suspend_set(const char *val, const struct kernel_param *kp)
{
    int ret,value;
    struct smb_charger *chg = &the_chip->chg;
	...
	ret= vote(chg->usb_icl_votable, USER_VOTER, (bool)value, 0);
	if (ret < 0) {
		pr_err("Couldn't vote to %s USB rc=%d\n", (bool)value ? "suspend" : "resume", ret);
		return ret;
	}

	ret = vote(chg->dc_suspend_votable, USER_VOTER, (bool)value, 0);
	if (ret < 0) {
...
static int input_suspend_get(char *buf, const struct kernel_param *kp)
{
    int ret, value;
    struct smb_charger *chg = &the_chip->chg;
    ...
    value = (get_client_vote(chg->usb_icl_votable, USER_VOTER) == 0)
...
module_param_call(param_input_suspend, input_suspend_set, input_suspend_get, NULL, 0644);
...
#define MISC_AFP_MODE_REG 0x1641
#define AFP_MODE_EN_BIT BIT(0)
static int enable_afp_mode(const char *val, const struct kernel_param *kp)
{
	int rc;
	struct smb_charger *chg = &the_chip->chg;

	rc = smblib_masked_write(chg, MISC_AFP_MODE_REG, AFP_MODE_EN_BIT, 1);
	if (rc < 0)
		pr_err("Couldn't enable AFP mode rc=%d\n", rc);

	return rc;
}
static const struct kernel_param_ops param_enable_afp_mode = {
	.get = NULL,
	.set = enable_afp_mode,
};
module_param_cb(enable_afp, &param_enable_afp_mode, NULL, 0200);
...
int smblib_typec_safety_disabled = 0;

module_param_named(
	typec_safety_disabled, smblib_typec_safety_disabled, int, S_IRUSR | S_IWUSR
);
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	/* disable ADC_LDO_EN */
	rc = smblib_masked_write(chg, VADC_ANA_PARAM_3_REG, ADC_LDO_EN_BIT, 0);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't disable ADC_LDO_EN rc=%d\n", rc);
	} else {
		pr_info("Success disable ADC_LDO_EN\n");
		rc = smblib_read(chg, VADC_ANA_PARAM_3_REG, &val);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't read VADC_ANA_PARAM_3_REG rc=%d\n",rc);
		} else {
			pr_info("VADC_ANA_PARAM_3_REG = 0x%x\n", val);
		}
	}
#endif /* CONFIG_BATTERY_SHARP */
}
