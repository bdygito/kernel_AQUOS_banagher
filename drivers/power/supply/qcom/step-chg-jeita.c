...
#define JEITA_SUSPEND_FV_HYST_UV		50000
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	u32 step_chg_rise_hysteresis = 0;
	u32 step_chg_fall_hysteresis = 0;
	u32 jeita_fv_hysteresis = 0;
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	rc = of_property_read_u32(profile_node, "sharp,step-chg-rise-hysteresis",
					&step_chg_rise_hysteresis);
	if (rc < 0) {
		pr_err("step_chg_rise_hysteresis, rc=%d, use default value of %d\n",
					rc, STEP_CHG_HYSTERESIS_DEFAULT);
		step_chg_rise_hysteresis = STEP_CHG_HYSTERESIS_DEFAULT;
	}
	chip->step_chg_config->param.rise_hys = step_chg_rise_hysteresis;

	rc = of_property_read_u32(profile_node, "sharp,step-chg-fall-hysteresis",
					&step_chg_fall_hysteresis);
	if (rc < 0) {
		pr_err("step_chg_fall_hysteresis, rc=%d, use default value of %d\n",
					rc, STEP_CHG_HYSTERESIS_DEFAULT);
		step_chg_fall_hysteresis = STEP_CHG_HYSTERESIS_DEFAULT;
	}

	chip->step_chg_config->param.fall_hys = step_chg_fall_hysteresis;
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	rc = of_property_read_u32(profile_node, "sharp,jeita-fv-hysteresis-uv",
					&jeita_fv_hysteresis);
	if (rc < 0) {
		jeita_fv_hysteresis = JEITA_SUSPEND_FV_HYST_UV;
	}
	chip->jeita_fv_config->fv_hysteresis_uv = jeita_fv_hysteresis;
	pr_info("jeita_fv_hysteresis %d\n", chip->jeita_fv_config->fv_hysteresis_uv);
#endif /* CONFIG_BATTERY_SHARP */
	return rc;
}
...
static int handle_jeita(struct step_chg_info *chip)
{
	...
	u64 elapsed_us;
#ifdef CONFIG_BATTERY_SHARP
	int sw_health = POWER_SUPPLY_HEALTH_UNKNOWN;
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
		if (!rc && (pval.intval > fv_uv + chip->jeita_fv_config->fv_hysteresis_uv))
#else
		if (!rc && (pval.intval > fv_uv))
#endif
...
#ifdef CONFIG_BATTERY_SHARP
	if(chip->jeita_fcc_index == chip->jeita_fv_index) {
		switch(chip->jeita_fcc_index) {
		case 0:
			sw_health = POWER_SUPPLY_HEALTH_COOL;
			break;
		case 1:
			sw_health = POWER_SUPPLY_HEALTH_GOOD;
			break;
		case 2:
			sw_health = POWER_SUPPLY_HEALTH_WARM;
			break;
		}
	}

	pval.intval = sw_health;
	rc = power_supply_set_property(chip->batt_psy,
		POWER_SUPPLY_PROP_SW_HEALTH, &pval);
	if (rc < 0)
		pr_err("Couldn't write SW_HEALTH rc=%d\n", rc);
#endif /* CONFIG_BATTERY_SHARP */
