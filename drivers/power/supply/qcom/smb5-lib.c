...
static const struct apsd_result *smblib_get_apsd_result(struct smb_charger *chg)
{
	...
	if(rc < 0){
		smblib_err(chg, "Couldn't read SNK_DAM_500MA_BIT rc=%d\n", rc);
		return result;
	}
	if(snk_dam){
...
#endif /* CONFIG_BATTERY_SHARP */
#ifdef CONFIG_BATTERY_SHARP
		if (chg->wa_flags & SDP_CURRENT_WA) {
...
#ifdef CONFIG_BATTERY_SHARP
		if (chg->wa_flags & SDP_CURRENT_WA) {
			rc = smblib_set_charge_param(chg, &chg->param.usb_icl, USBIN_450MA);
			if (rc < 0) {
...
#ifdef CONFIG_BATTERY_SHARP
	rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_PADDED_CAPACITY, val);
#else
				...
				union power_supply_propval *val)
{
	...
	smblib_dbg(chg, PR_REGISTER, "hw_jeita=%d, sw_jeita=%d\n", val->intval, chg->sw_health);
	if (val->intval != POWER_SUPPLY_HEALTH_COLD && val->intval != POWER_SUPPLY_HEALTH_OVERHEAT
			&& chg->sw_health != POWER_SUPPLY_HEALTH_UNKNOWN)
		val->intval = chg->sw_health;
				...
				union power_supply_propval *val)
{
...
#ifdef CONFIG_BATTERY_SHARP
	bool usbin_ov, dcin_ov, complete_chg_sft;
	u8 sh_stat, chg_stat1_reg;
	int capacity, jeita_cur_status = POWER_SUPPLY_HEALTH_UNKNOWN;
#endif
...
#ifdef CONFIG_BATTERY_SHARP
	rc = smblib_read(chg, USBIN_BASE + INT_RT_STS_OFFSET, &sh_stat);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't read USB_INT_RT_STS rc=%d\n", rc);
		return rc;
	}
	smblib_dbg(chg, PR_REGISTER, "USBIN_INT_RT_STS = 0x%02x\n", sh_stat);
	usbin_ov = sh_stat & USBIN_OV_RT_STS_BIT;

	if (usbin_ov && chg->usbov_flag) {
		usbin_ov = true;
	} else {
		usbin_ov = false;
		chg->usbov_flag = false;
	}
	...
	rc = smblib_read(chg, BATTERY_CHARGER_STATUS_2_REG, &sh_stat);
	if (rc < 0) {
		smblib_err(chg, "Couldn't read BATTERY_CHARGER_STATUS_2 rc=%d\n", rc);
		return rc;
	}

	complete_chg_sft = sh_stat & CHARGER_ERROR_STATUS_SFT_EXPIRE_BIT;
	if (complete_chg_sft) {
		val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
		return 0;
	}
	...
	smblib_get_jeita_status(chg, &pval);
	...
	if (usb_online || dc_online) {
		if (((chg_stat1_reg == TERMINATE_CHARGE) || (chg_stat1_reg == INHIBIT_CHARGE))
		...
		if ((chg_stat1_reg == PAUSE_CHARGE) && (jeita_cur_status == POWER_SUPPLY_HEALTH_COLD)) {
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
			return 0;
		}

		if ( is_client_vote_enabled(chg->chg_disable_votable, THERMAL_DAEMON_VOTER)
				|| is_client_vote_enabled(chg->chg_disable_votable, DIRECT_CHARGE_VOTER) ) {
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
			return 0;
		}
	}
...
#ifdef CONFIG_BATTERY_SHARP
	smblib_dbg(chg, PR_MISC, "BATTERY_CHARGER_STATUS_5 = 0x%02x\n", stat);
#endif
...
#ifdef CONFIG_BATTERY_SHARP
	{
		smblib_dbg(chg, PR_MISC, "battery status %d -> %d\n", val->intval, POWER_SUPPLY_STATUS_DISCHARGING);
		val->intval = POWER_SUPPLY_STATUS_DISCHARGING;
	}
#else
		val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
				...
				const union power_supply_propval *val)
{
	...
	if (pval.intval == POWER_SUPPLY_STATUS_FULL && chg->system_temp_level >= chg->thermal_levels - 1)
	{
		smblib_dbg(chg, PR_MISC, "Ignore system_temp_level=%d in FULL(%d)\n", chg->system_temp_level, pval.intval);
		return 0;
	}
...
#ifdef CONFIG_BATTERY_SHARP
int smblib_set_direct_charge_mode(struct smb_charger *chg, bool enable)
{
	u8 stat;
	int rc = 0;

	vote(chg->chg_disable_votable, DIRECT_CHARGE_VOTER, enable, 0);

	if (enable && chg->wa_flags & CHG_TERMINATION_WA) {
		rc = smblib_read(chg, BATTERY_CHARGER_STATUS_1_REG, &stat);
		if (rc < 0) {
			smblib_err(chg, "Couldn't read BATTERY_CHARGER_STATUS_1 rc=%d\n",
					rc);
...
static int smblib_get_prop_ufp_mode(struct smb_charger *chg)
{
	...
	if (stat & (SNK_DAM_500MA_BIT)){
		smblib_masked_write(chg, TYPE_C_DEBUG_ACCESS_SINK_REG, TYPEC_DEBUG_ACCESS_SINK_MASK,
								EN_DEBUG_ACCESS_SNK|EN_CHG_ON_DEBUG_ACCESS_SNK|DAM_SEL_ICL_3A_OR_FMB|EN_TYPEC_FMB_BIT);
	}
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
#define CHG_TERM_WA_EXIT_DELAY_MS		180000		/* 3 min */
#else
...
static void smblib_eval_chg_termination(struct smb_charger *chg, u8 batt_status)
{
	...
	int rc = 0;
#ifdef CONFIG_BATTERY_SHARP
	ktime_t term_wa_remaining_time;
	int directcharge_active;
	int input_present;
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	smblib_is_input_present(chg, &input_present);
	directcharge_active = get_client_vote(chg->chg_disable_votable, DIRECT_CHARGE_VOTER) && input_present;
#endif /* CONFIG_BATTERY_SHARP */
	 ...
	 */
#ifdef CONFIG_BATTERY_SHARP
	smblib_dbg(chg, PR_MISC, "dchg=%d, input_present=%d\n", directcharge_active, input_present);

	if (((batt_status == TERMINATE_CHARGE) && (pval.intval == 100)) || directcharge_active) {
#else
	if ((batt_status == TERMINATE_CHARGE) && (pval.intval == 100)) {
#endif /* CONFIG_BATTERY_SHARP */
#ifdef CONFIG_BATTERY_SHARP
		term_wa_remaining_time = alarm_expires_remaining(&chg->chg_termination_alarm);
		// SET NEW ALARM, and clear old values only if No alarm is pending.
		if (ktime_to_ms(term_wa_remaining_time) <= 0) {
			chg->cc_soc_ref = 0;
			chg->last_cc_soc = 0;
			chg->term_vbat_uv = 0;
			smblib_dbg(chg, PR_MISC, "SET NEW ALARM; term_wa_remaining_time: %dms\n", ktime_to_ms(term_wa_remaining_time));
			alarm_start_relative(&chg->chg_termination_alarm,
					ms_to_ktime(CHG_TERM_WA_ENTRY_DELAY_MS));
		} else {
			smblib_dbg(chg, PR_MISC, "The alarm still running.\n");
		}
#else /* CONFIG_BATTERY_SHARP */
		chg->cc_soc_ref = 0;
		chg->last_cc_soc = 0;
...
#endif  /* CONFIG_BATTERY_SHARP */
#ifdef CONFIG_BATTERY_SHARP
	} else if (pval.intval < 100 && !directcharge_active) {
#else
...
irqreturn_t chg_state_change_irq_handler(int irq, void *data)
{
	...
	if (stat != TERMINATE_CHARGE && stat != INHIBIT_CHARGE) {
		...
		if (rc < 0)
			smblib_err(chg, "Couldn't set system_temp_level rc=%d\n", rc);
	}
#endif /* CONFIG_BATTERY_SHARP */
...
irqreturn_t usb_plugin_irq_handler(int irq, void *data)
{
	...
	smblib_dbg(chg, PR_INTERRUPT, "IRQ: %s usb_therm_work start.\n", irq_data->name);
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
		/* If go here, legacy cable is detected. Don't use rp based ICL. */
		vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true,
					DCP_CURRENT_UA);
#else
		rp_ua = get_rp_based_dcp_current(chg, typec_mode);
...
static void smblib_usbov_dbc_work(struct work_struct *work)
{
	...
	if (!(chg->wa_flags & USBIN_OV_WA)) {
		...
		if (stat & USBIN_OV_RT_STS_BIT) {
			smblib_dbg(chg, PR_MISC, "Confirm USBOV\n");
			chg->usbov_flag = true;
		}
		return;
	}
...
irqreturn_t usbin_ov_irq_handler(int irq, void *data)
{
	...
	if (stat & USBIN_OV_RT_STS_BIT) {
		smblib_dbg(chg, PR_MISC, "Schedule USBOV recheck\n");
		schedule_delayed_work(&chg->usbov_dbc_work,
				msecs_to_jiffies(USB_OV_DBC_PERIOD_MS));
	} else {
		smblib_dbg(chg, PR_MISC, "Resetting USBOV\n");
		cancel_delayed_work_sync(&chg->usbov_dbc_work);
		chg->usbov_flag = false;
	}
...
static void smblib_chg_termination_work(struct work_struct *work)
{
	...
	ktime_t term_wa_remaining_time;
	u8 stat;
	bool directcharge_active = false;
#endif /* CONFIG_BATTERY_SHARP */
	...
	rc = smblib_get_prop_from_bms(chg,
				POWER_SUPPLY_PROP_REAL_CAPACITY, &pval);
#ifdef CONFIG_BATTERY_SHARP
	smblib_dbg(chg, PR_MISC, "real_capacity=%d\n", pval.intval);
#endif /* CONFIG_BATTERY_SHARP */

#ifdef CONFIG_BATTERY_SHARP
	directcharge_active = get_client_vote(chg->chg_disable_votable, DIRECT_CHARGE_VOTER);
	if ((rc < 0) || (pval.intval < 100 && !directcharge_active)) {
#else
	if ((rc < 0) || (pval.intval < 100)) {
		...
		stat = stat & BATTERY_CHARGER_STATUS_MASK;
		smblib_dbg(chg, PR_MISC, "BATTERY_CHARGER_STATUS=0x%x\n", stat);
		if(stat!=TERMINATE_CHARGE && !directcharge_active) {
#endif /* CONFIG_BATTERY_SHARP */
		vote(chg->usb_icl_votable, CHG_TERMINATION_VOTER, false, 0);
		vote(chg->dc_suspend_votable, CHG_TERMINATION_VOTER, false, 0);
		goto out;
	 ...
	 */
#ifdef CONFIG_BATTERY_SHARP
	if (pval.intval < DIV_ROUND_CLOSEST(chg->cc_soc_ref * 9950, 10000) && vbat_now_uv <= 4400000) {
#else
	if (pval.intval < DIV_ROUND_CLOSEST(chg->cc_soc_ref * 10050, 10000)) {
		...
		delay = CHG_TERM_WA_ENTRY_DELAY_MS;
		  ...
		  && ((vbat_now_uv > chg->term_vbat_uv) &&
		     (vbat_now_uv > max_fv_uv))) || (vbat_now_uv > 4400000)) {
#else
	} else if ((pval.intval > DIV_ROUND_CLOSEST(chg->cc_soc_ref * 10075,
								10000))
		  && ((vbat_now_uv > chg->term_vbat_uv) &&
	...
	// SET NEW ALARM, Only if OLD alarm expired OR the new alarm set time will fire earlier then time remaining
	if( (ktime_to_ms(term_wa_remaining_time) <= 0) || (ktime_to_ms(term_wa_remaining_time) > delay)) {
		smblib_dbg(chg, PR_MISC, "SET NEW ALARM; term_wa_remaining_time: %dms, delay: %dms\n", ktime_to_ms(term_wa_remaining_time), delay);
		alarm_start_relative(&chg->chg_termination_alarm, ms_to_ktime(delay));
	} else { // ELSE DO NOTHING, if the alarm time remaining is less then the new alarm, to avoid race condition, let the old one fire
		smblib_dbg(chg, PR_MISC, "Avoiding setting new alarm to avoid race condition.\n");
	}
#else /* CONFIG_BATTERY_SHARP */
	alarm_start_relative(&chg->chg_termination_alarm, ms_to_ktime(delay));
#endif /* CONFIG_BATTERY_SHARP */
...
extern int smblib_typec_safety_disabled;

int smblib_get_usb_therm(struct smb_charger *chg, int *usb_therm)
{
	int rc = 0;
	static int debug_usb_therm_count = 0;

	if(chg->iio.usb_therm_chan == 0)
	{
		rc = -EINVAL;
		smblib_err(chg, "[E] ret:%d usb_therm_chan:0x%x is not set.\n",
				rc, chg->iio.usb_therm_chan );
		return rc;
	}
	...
	if(rc < 0)
	{
		smblib_err(chg, "[E] ret:%d smblib_read_iio_channel() ch:0x%x is failed.\n",
					rc, chg->iio.usb_therm_chan );
		return rc;
	}

	if(smblib_debug_usb_therm) {
		if (smblib_typec_safety_disabled)
		{
			*usb_therm = smblib_debug_usb_therm;
		}
		else
		{
			if (smblib_debug_usb_therm > *usb_therm)
			{
				*usb_therm = smblib_debug_usb_therm;
				debug_usb_therm_count++;
				if (smblib_debug_usb_therm >= chg->typec_safety_trigger_threshold) {
					if(!(debug_usb_therm_count % GET_TEMP_RETRY_CONT_MAX))
						smblib_debug_usb_therm -= 1000;
				}
				else
				{
					smblib_debug_usb_therm -= 1000;
			...
			{
				smblib_debug_usb_therm = 0;
				debug_usb_therm_count = 0;
			}
		}

		smblib_dbg(chg, PR_MISC, "count=%d, debug_usb_therm=%d, usb_therm=%d\n", debug_usb_therm_count, smblib_debug_usb_therm, *usb_therm);
	}
...
static void smblib_typec_usb_therm_work(struct work_struct *work)
{
	...
	int rc;
	union power_supply_propval pval = {0, };
	bool usb_present, typec_overheat;
	int retry_cnt = 0;
	...
	typec_overheat = get_client_vote_locked(chg->cc_disable_votable, USB_OVERHEAT_VOTER);

	smblib_dbg(chg, PR_MISC, "typec_usb_overheat=%d, usb_present=%d\n", typec_overheat, usb_present);

	rc = smblib_get_usb_therm(chg, &temp);

	while(rc < 0 || temp >= chg->typec_safety_trigger_threshold)
	{
		if(retry_cnt == 0)
			smblib_err(chg, "rc = %d. temp = %d. Get temp retry process start.\n", rc, temp);
		...
		if(rc < 0)
		{
			if( retry_cnt >= GET_TEMP_RETRY_CONT_MAX ){
				smblib_err(chg, "[E] Couldn't get usb_therm rc =%d.\n", rc );
				// re-schedule delayed_work
				schedule_delayed_work(&chg->typec_usb_therm_work,
					msecs_to_jiffies(USB_THERM_POLLING_TIME));
				return;
			}
			smblib_err(chg, "rc = %d. temp = %d. retry count = %d.\n", rc, temp, retry_cnt);
			continue;
		}

		if(temp < chg->typec_safety_trigger_threshold){
			smblib_err(chg, "rc = %d. temp = %d. retry count = %d. Normal temperature obtained \n", rc, temp, retry_cnt);
			break;
		}else{
			if( retry_cnt >= GET_TEMP_RETRY_CONT_MAX ){
				smblib_err(chg, "rc = %d. temp = %d. retry count = %d. Retry count max.\n", rc, temp, retry_cnt);
				break;
			}else{
				smblib_err(chg, "rc = %d. temp = %d. retry count = %d.\n", rc, temp, retry_cnt);
			}
	...
	smblib_dbg(chg, PR_MISC, "usb_therm=%d\n", temp);
	...
	if (!usb_present && !typec_overheat)
		goto usb_therm_work_end;

	smblib_dbg(chg, PR_MISC, "typec_usb_overheat=%d, level=%d\n", typec_overheat, level);
