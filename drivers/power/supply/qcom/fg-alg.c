...
#define CAPACITY_DELTA_DECIPCT_10PCT 100
...
#ifdef CONFIG_BATTERY_SHARP
	if (cl->current_batt_age_level > 0) {
		res_batt_soc_cp -= 300;
	}
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
void sh_cap_learning_update(struct cap_learning *cl, int batt_temp,
		int batt_soc, int charge_status, bool charge_done,
		bool input_present, bool qnovo_en, bool directcharge_active)
{
	if (directcharge_active) {
		if (cl->active) {
			pr_debug("Capacity learning aborted. Directcharge is activated.\n");
			cap_learning_abort(cl);
		}
	} else {
		cap_learning_update(cl, batt_temp, batt_soc, charge_status, charge_done,
			input_present, qnovo_en);
	}
}
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
		if (cl->dt.capacity_delta_force_10pct) {
			pr_info("CAPACITY_DELTA_DECIPCT uses a temporary value = %d -> %d\n",
										CAPACITY_DELTA_DECIPCT, CAPACITY_DELTA_DECIPCT_10PCT);
			pct_nom_cap_uah = div64_s64((int64_t)cl->nom_cap_uah *
					CAPACITY_DELTA_DECIPCT_10PCT, 1000);
		} else {
			pct_nom_cap_uah = div64_s64((int64_t)cl->nom_cap_uah *
...
#ifdef CONFIG_BATTERY_SHARP
	rc = ttf->get_ttf_param(ttf->data, TTF_DIRECTCHARGE_ACTIVE, &directcharge_active);
	if (rc < 0) {
		pr_err("failed to get directcharge status, rc=%d\n", rc);
		goto end_work;
	}
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	if (charge_status == POWER_SUPPLY_STATUS_CHARGING && !directcharge_active) {
#else
	if (charge_status == POWER_SUPPLY_STATUS_CHARGING) {
#endif /* CONFIG_BATTERY_SHARP */
		rc = ttf_circ_buf_median(&ttf->ibatt, &ibatt_avg);
