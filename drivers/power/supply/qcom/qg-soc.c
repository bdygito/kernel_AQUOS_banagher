...
static int qg_padded_soc_complement_ms = 10000;
module_param_named(
	padded_soc_complement_ms, qg_padded_soc_complement_ms, int, 0600
);

static struct qg_setpoint setpoint[NUM_SETPOINT] = {
	{1000, 0},
	{700, 50},
	{500, 50},
	{200, 0},
...
module_param_named(
	setpoint_0_base, setpoint[0].base, int, S_IRUSR | S_IWUSR
);

module_param_named(
	setpoint_0_up, setpoint[0].up, int, S_IRUSR | S_IWUSR
);

module_param_named(
	setpoint_1_base, setpoint[1].base, int, S_IRUSR | S_IWUSR
);

module_param_named(
	setpoint_1_up, setpoint[1].up, int, S_IRUSR | S_IWUSR
);

module_param_named(
	setpoint_2_base, setpoint[2].base, int, S_IRUSR | S_IWUSR
);

module_param_named(
	setpoint_2_up, setpoint[2].up, int, S_IRUSR | S_IWUSR
);

module_param_named(
	setpoint_3_base, setpoint[3].base, int, S_IRUSR | S_IWUSR
);

module_param_named(
	setpoint_3_up, setpoint[3].up, int, S_IRUSR | S_IWUSR
);

module_param_named(
	setpoint_4_base, setpoint[4].base, int, S_IRUSR | S_IWUSR
);

module_param_named(
	setpoint_4_up, setpoint[4].up, int, S_IRUSR | S_IWUSR
);

module_param_named(
	setpoint_5_base, setpoint[5].base, int, S_IRUSR | S_IWUSR
);

module_param_named(
	setpoint_5_up, setpoint[5].up, int, S_IRUSR | S_IWUSR
);

module_param_named(
	setpoint_6_base, setpoint[6].base, int, S_IRUSR | S_IWUSR
);

module_param_named(
	setpoint_6_up, setpoint[6].up, int, S_IRUSR | S_IWUSR
);
...
#ifdef CONFIG_BATTERY_SHARP
	else if (chip->dt.correct_capacity_enable
		&& (chip->catch_up_padded_soc != chip->padded_soc))
		min_delta_soc_interval_ms = qg_padded_soc_complement_ms;
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	bool directcharge_active = is_directcharge_active(chip);
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	if (chip->catch_up_soc > chip->msoc && (!input_present || directcharge_active))
#else
...
#ifdef CONFIG_BATTERY_SHARP
static bool is_padding_required(struct qpnp_qg *chip)
{
	bool input_present = is_input_present(chip);
	bool directcharge_active = is_directcharge_active(chip);

	if (!chip->dt.correct_capacity_enable)
		return false;
	
	if (!chip->profile_loaded)
		return false;
	
	if ((abs(chip->catch_up_padded_soc - chip->padded_soc) < chip->dt.delta_soc) &&
		chip->catch_up_padded_soc != 0 && chip->catch_up_padded_soc != 100)
		return false;
	
	if (chip->catch_up_padded_soc == chip->padded_soc)
		return false;
	
	if (chip->catch_up_padded_soc > chip->padded_soc && (!input_present || directcharge_active))
		return false;
	
	if (chip->catch_up_padded_soc > chip->padded_soc && input_present &&
		(chip->charge_status != POWER_SUPPLY_STATUS_CHARGING &&
		chip->charge_status != POWER_SUPPLY_STATUS_FULL))
		return false;
...
#ifdef CONFIG_BATTERY_SHARP
			sh_cap_learning_update(chip->cl, batt_temp, chip->batt_soc,
					chip->charge_status, chip->charge_done,
					input_present, false, directcharge_active);
#else
...
#ifdef CONFIG_BATTERY_SHARP
	qg_update_padded_soc(chip, chip->force_soc);
#endif /* CONFIG_BATTERY_SHARP */
}
...
#ifdef CONFIG_BATTERY_SHARP
		qg_update_padded_soc(chip, force_soc);
#endif /* CONFIG_BATTERY_SHARP */
		goto done_psy;
	}

#ifdef CONFIG_BATTERY_SHARP
	if (!is_scaling_required(chip) && !is_padding_required(chip)) {
#else
	if (!is_scaling_required(chip)) {
#endif /* CONFIG_BATTERY_SHARP */
		scale_soc_stop(chip);
...
#ifdef CONFIG_BATTERY_SHARP
static int qg_correct_capacity(struct qpnp_qg *chip, int val)
{
	int i, slope, offset, corrected_value;
	int p1[2], p2[2];

	for (i=1; i<NUM_SETPOINT; i++) {
		if (val >= setpoint[i].base) {
			p1[0] = setpoint[i-1].base;
			p1[1] = setpoint[i-1].base + setpoint[i-1].up;
			p2[0] = setpoint[i].base;
			p2[1] = setpoint[i].base + setpoint[i].up;
			break;
		}
	}
	if (i == NUM_SETPOINT) {
		qg_dbg(chip, QG_DEBUG_STATUS, "val=%d < qg_setpoint_n_base=%d, no correction\n", val, setpoint[i-1].base);
		return DIV_ROUND_CLOSEST(val, 10);
	}
	qg_dbg(chip, QG_DEBUG_STATUS, "p1=(%d, %d), p2=(%d, %d)\n", p1[0], p1[1], p2[0], p2[1]);

	slope = DIV_ROUND_CLOSEST((p2[1] - p1[1]) * 1000, p2[0] - p1[0]);
	offset = DIV_ROUND_CLOSEST((p2[0] * p1[1] - p1[0] * p2[1]) * 1000, p2[0] - p1[0]);
	qg_dbg(chip, QG_DEBUG_STATUS, "slope*1000=%d, offset*1000=%d\n", slope, offset);

	corrected_value = val * slope + offset;
	qg_dbg(chip, QG_DEBUG_STATUS, "val=%d, slope=%d, offset=%d, corrected=%d\n", val, slope, offset, corrected_value);

	return DIV_ROUND_CLOSEST(corrected_value, 10000);
}

void qg_update_padded_soc(struct qpnp_qg *chip, bool force_soc)
{
	int padded_soc = CAP(0, 100, chip->padded_soc);
	int catch_up_padded_soc = CAP(0, 100, chip->catch_up_padded_soc);
	int real_soc;
	...
	if (chip->padded_soc == INT_MIN)
		force_soc = true;

	if (chip->dt.linearize_soc && chip->maint_soc > 0)
	{
		real_soc = chip->maint_soc;
		if (real_soc == FULL_SOC)
			force_soc = true;
	}
	else
		real_soc = chip->msoc;
	...
	if (real_soc == 0 || real_soc == 100)
		catch_up_padded_soc = real_soc;
	else
		catch_up_padded_soc = qg_correct_capacity(chip, real_soc * 10);

	chip->catch_up_padded_soc = CAP(0, 100, catch_up_padded_soc);

	if (catch_up_padded_soc != chip->padded_soc) {
		directcharge_active = is_directcharge_active(chip);
		if (force_soc)
			padded_soc = catch_up_padded_soc;
		else if (is_input_present(chip) && !directcharge_active && padded_soc < catch_up_padded_soc)
			padded_soc += chip->dt.delta_soc;
		else if (padded_soc > catch_up_padded_soc)
			padded_soc -= chip->dt.delta_soc;

		chip->padded_soc = CAP(0, 100, padded_soc);
		pr_info("Update padded soc: real=%d, padded=%d, padded_raw=%d\n", real_soc,
				padded_soc, catch_up_padded_soc);
    }
}
#endif /* CONFIG_BATTERY_SHARP */
