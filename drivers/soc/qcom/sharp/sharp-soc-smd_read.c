...
static int smd_ioctl_get_edgk(unsigned long arg)
{
	sharp_smem_common_type *sh_smem = NULL;
	...
	if( sh_smem != 0 )
	{
		if(copy_to_user((unsigned char __user *)arg, sh_smem->sh_diagenc_gk_ex, sizeof(sh_smem->sh_diagenc_gk_ex)) != 0)
		{
			printk("[%s]: copy_to_user FAIL\n",__func__);
			return -EFAULT;
		}
...
static int smd_ioctl_set_vib_val(unsigned long arg)
{
	sharp_smem_common_type *sh_smem = NULL;
	...
	if( sh_smem != 0 )
	{
		if( copy_from_user( sh_smem->shdiag_vib_param, (unsigned char __user *)arg, sizeof(sh_smem->shdiag_vib_param) ) != 0 )
		{
			printk("[%s]: copy_to_user FAIL\n",__func__);
			return -EFAULT;
		}
...
static int smd_ioctl_set_wlan_val(unsigned long arg)
{
	sharp_smem_common_type *sh_smem = NULL;
	...
	if( sh_smem != 0 )
	{
		if( copy_from_user( sh_smem->shdarea_WlanNVSwitch, (unsigned char __user *)arg, sizeof(sh_smem->shdarea_WlanNVSwitch) ) != 0 )
		{
			printk("[%s]: copy_to_user FAIL\n",__func__);
			return -EFAULT;
		}
...
static int smd_ioctl_set_als1_val(unsigned long arg)
{
	int ret = 0;
	struct shdiag_als1 als1_work;
	sharp_smem_common_type *sh_smem = NULL;

	if(copy_from_user(&als1_work, (unsigned char __user *)arg, sizeof(struct shdiag_als1)) != 0)
	{
	...
	else
	{
		...
		if( sh_smem != 0 )
		{
			sh_smem->sh_als_status			=	als1_work.buf[0];

			sh_smem->sh_als_adj0[0]			=	als1_work.buf[1];
			sh_smem->sh_als_adj0[0]			<<=	8;
			sh_smem->sh_als_adj0[0]			|=	als1_work.buf[2];

			sh_smem->sh_als_adj1[0]			=	als1_work.buf[3];
			sh_smem->sh_als_adj1[0]			<<=	8;
			sh_smem->sh_als_adj1[0]			|=	als1_work.buf[4];

			sh_smem->sh_als_shift[0]		=	0x00;
			sh_smem->sh_als_shift[0]		|=	als1_work.buf[5];

			sh_smem->sh_als_clear_offset[0] =	als1_work.buf[6];

			sh_smem->ir_offset[0]			=	als1_work.buf[7];

			sh_smem->sh_als_adj0[1]			=	als1_work.buf[8];
			sh_smem->sh_als_adj0[1]			<<=	8;
			sh_smem->sh_als_adj0[1]			|=	als1_work.buf[9];

			sh_smem->sh_als_adj1[1]			=	als1_work.buf[10];
			sh_smem->sh_als_adj1[1]			<<=	8;
			sh_smem->sh_als_adj1[1]			|=	als1_work.buf[11];

			sh_smem->sh_als_shift[1]		=	0x00;
			sh_smem->sh_als_shift[1]		|=	als1_work.buf[12];

			sh_smem->sh_als_clear_offset[1] =	als1_work.buf[13];

			sh_smem->ir_offset[1]			=	als1_work.buf[14];

			sh_smem->sh_als_key_backlight	=	als1_work.buf[15];

			sh_smem->sh_als_chksum			=	als1_work.buf[16];
			sh_smem->sh_als_chksum			<<=	8;
			sh_smem->sh_als_chksum			|=	als1_work.buf[17];
			sh_smem->sh_als_chksum			<<=	8;
			sh_smem->sh_als_chksum			|=	als1_work.buf[18];
		}
		else
		{
			printk("[%s]: sh_smem_get_common_address() FAIL\n",__func__);
...
static int smd_ioctl_set_als2_val(unsigned long arg)
{
	int ret = 0;
	struct shdiag_als2 als2_work;
	sharp_smem_common_type *sh_smem = NULL;

	if(copy_from_user(&als2_work, (unsigned char __user *)arg, sizeof(struct shdiag_als2)) != 0)
	{
		printk("[%s]: copy_from_user FAIL\n",__func__);
		ret = -EFAULT;
	}
	else
	{
		...
		if( sh_smem != 0 )
		{
			sh_smem->sh_als_scale_a		=	als2_work.buf[0];
			sh_smem->sh_als_scale_a		<<=	8;
			sh_smem->sh_als_scale_a		|=	als2_work.buf[1];

			sh_smem->sh_als_scale_g		=	0x00;
			sh_smem->sh_als_scale_g		|=	als2_work.buf[2];
		}
		else
		{
			printk("[%s]: sh_smem_get_common_address() FAIL\n",__func__);
...
static int smd_ioctl_set_als3_val(unsigned long arg)
{
	int ret = 0;
	struct shdiag_als3 als3_work;
	sharp_smem_common_type *sh_smem = NULL;
	int cnt;

	if(copy_from_user(&als3_work, (unsigned char __user *)arg, sizeof(struct shdiag_als3)) != 0)
	{
		printk("[%s]: copy_from_user FAIL\n",__func__);
		ret = -EFAULT;
	}
	else
	{
		...
		if( sh_smem != 0 )
		{
			for( cnt = 0; cnt < 4; cnt ++ )
			{
				sh_smem->sh_rgb_scale_a[cnt]		=	als3_work.buf[cnt*3];
				sh_smem->sh_rgb_scale_a[cnt]		<<=	8;
				sh_smem->sh_rgb_scale_a[cnt]		|=	als3_work.buf[cnt*3+1];

				sh_smem->sh_rgb_scale_g[cnt]		=	0x00;
				sh_smem->sh_rgb_scale_g[cnt]		|=	als3_work.buf[cnt*3+2];
			}
		}
		else
		{
...
static int smd_ioctl_get_flag_data(unsigned long arg)
{
	int ret = 0;
	sharp_smem_common_type *sharp_smem;
	uint32_t flag_data;
	...
	if( sharp_smem != 0 )
	{
		flag_data = sharp_smem->shdiag_FlagData;
		if(copy_to_user((uint32_t __user *)arg, &flag_data, sizeof(uint32_t)) != 0)
		{
			printk("[SH]smd_ioctl_get_flag_data: copy_to_user FAILE\n");
			ret = -EFAULT;
		}
	}else{
		printk("[SH]smd_ioctl_get_flag_data: get smem_addr FAILE\n");
		return -EFAULT;
	}
...
/* DIAG_1726 add start */
/* DIAG_1796 add start */
static int smd_ioctl_get_tpvendor(unsigned long arg)
{
	...
	if( sh_smem_addr != 0 )
	{
		tp_vendor = sh_smem_addr->shtps_touchpanel_kind;
		if(copy_to_user((uint32_t __user *)arg, &tp_vendor, sizeof(uint32_t)) != 0)
		{
			printk("[SH]smd_ioctl_get_tpvendor: copy_to_user FAILE\n");
			return -EFAULT;
		}
	}
	else
...
static long smd_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SHDIAG_IOCTL_GET_LCD_UPPER_INFO:
		...
		break;
	case SHDIAG_IOCTL_GET_EDGK:
		ret = smd_ioctl_get_edgk(arg);
		break;
	case SHDIAG_IOCTL_SET_VIB:
		ret = smd_ioctl_set_vib_val(arg);
		break;
	case SHDIAG_IOCTL_SET_WLANNVSW:
		ret = smd_ioctl_set_wlan_val(arg);
		break;
	case SHDIAG_IOCTL_SET_ALS1:
		ret = smd_ioctl_set_als1_val(arg);
		break;
	case SHDIAG_IOCTL_SET_ALS2:
		ret = smd_ioctl_set_als2_val(arg);
		break;
	case SHDIAG_IOCTL_SET_ALS3:
		ret = smd_ioctl_set_als3_val(arg);
		break;
	case SHDIAG_IOCTL_GET_FLAGDATA:
		ret = smd_ioctl_get_flag_data(arg);
		break;
/* DIAG_1726 add start */
...
/* DIAG_1726 add end */
/* DIAG_1796 add start */
	case SHDIAG_IOCTL_GET_TPVENDOR:
		ret = smd_ioctl_get_tpvendor(arg);
		break;
/* DIAG_1796 add end */
	default:
