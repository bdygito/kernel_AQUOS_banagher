...
#ifdef CONFIG_SHARP_SCSI_UFS_SAMSUNG_TW
static inline bool ufshcd_is_samsung_tw_supported(struct ufs_hba *hba)
{
	return !!(hba != NULL ? 
			hba->dev_info.w_manufacturer_id == UFS_VENDOR_SAMSUNG : 
			false);
}
#endif /* CONFIG_SHARP_SCSI_UFS_SAMSUNG_TW */
...
#ifdef CONFIG_SHARP_SCSI_UFS_SAMSUNG_TW
	if (ufshcd_is_samsung_tw_supported(hba))
...
#ifdef CONFIG_SHARP_SCSI_UFS_SAMSUNG_TW
	if (ufshcd_is_samsung_tw_supported(hba))
		if ((idn == QUERY_DESC_IDN_DEVICE && index == 0x00) ||
			(idn == QUERY_DESC_IDN_CONFIGURATION &&
						 	(index >= 0x00 && index <= 0x03)) ||
			(idn == QUERY_DESC_IDN_GEOMETRY && index == 0x00) ||
			(idn == QUERY_DESC_IDN_UNIT &&
						 	(index >= 0x00 && index <= 0x07)))
			selector = 1;
#endif /* CONFIG_SHARP_SCSI_UFS_SAMSUNG_TW */
...
#ifdef CONFIG_SHARP_SCSI_UFS_SAMSUNG_TW
	/* Enable WB only for UFS-3.1 OR if samsung desc len >= 0x57 */
	if ((dev_desc->wspecversion >= 0x310) ||
	    (dev_desc->wmanufacturerid == UFS_VENDOR_SAMSUNG &&
	     dev_desc->wspecversion >= 0x300 &&
	     hba->desc_size.dev_desc >= 0x57)) {
		hba->dev_info.d_ext_ufs_feature_sup =
			desc_buf[DEVICE_DESC_PARAM_EXT_UFS_FEATURE_SUP]
								<< 24 |
			desc_buf[DEVICE_DESC_PARAM_EXT_UFS_FEATURE_SUP + 1]
...
#ifdef CONFIG_SHARP_SCSI_UFS_SAMSUNG_TW
		if (ufshcd_is_samsung_tw_supported(hba)) {
			ret = ufshcd_wb_toggle_flush_during_h8(hba, false);
			if (ret) {
				dev_err(hba->dev,
					"%s: Disable WB flush during H8: failed: %d\n",
					__func__, ret);
				goto enable_gating;
			}
		}
...
static int ufshcd_resume(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	if (ufshcd_is_samsung_tw_supported(hba)) {
		if (!ufshcd_is_runtime_pm(pm_op)) {
			rc = ufshcd_wb_toggle_flush_during_h8(hba, true);
			if (rc) {
				dev_err(hba->dev,
					"%s: Enable WB flush during H8: failed:%d\n",
					__func__, rc);
			}
		}
	}
