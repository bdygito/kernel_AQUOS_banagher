...
struct wled_dbc_param {
	int	x;
	int	y;
};

struct wled_dbc_params {
	int	cnt;
	struct wled_dbc_param *param;
};
#endif /* CONFIG_SHARP_DISPLAY */
...
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00052 */
	struct wled_dbc_params		dbc_params;
#endif /* CONFIG_SHARP_DISPLAY */
};
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00052 */
static u32 wled_brightness_to_dbc(int value, int dbc_brightness,
		struct wled_dbc_params *dbc_params);
static int wled_parse_dbc(struct wled *wled);
#endif /* CONFIG_SHARP_DISPLAY */
...
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00052 */
		if (bl->props.ignore_dbc) {
			pr_debug("%s:ignore_dbc\n", __func__);
		} else {
			brightness = wled_brightness_to_dbc(brightness, bl->props.dbc_brightness, &wled->dbc_params);
		}
#endif /* CONFIG_SHARP_DISPLAY */
		...
		struct wled_dbc_params *dbc_params)
{
	...
	pr_debug("%s:in value:%d,%d\n", __func__, value, dbc_brightness);

	if (value == 0 || dbc_brightness == 0) {
		return 0;
	}

	if (dbc_brightness >= BL_DBC_MAX)
		goto param_err;

	if (!dbc_params) {
		pr_err("%s:dbc_params is null\n", __func__);
		goto param_err;
	}
	...
	for (i = 0;i < cnt;i++) {
		pr_debug("%s:param %d x:%d y:%d\n", __func__,
			i, dbc_params->param[i].x, dbc_params->param[i].y);
	}
	...
	if (in <= dbc_params->param[0].x) {
		lumi = dbc_params->param[0].y * 100;
	} else
	if (in >= dbc_params->param[cnt-1].x) {
		lumi = dbc_params->param[cnt-1].y * 100;
	} else {
		for (i = 1;i < cnt;i++) {
			if (in <= dbc_params->param[i].x) {
				x  = dbc_params->param[i].x;
				x1 = dbc_params->param[i-1].x;
				y  = dbc_params->param[i].y * 100;
				y1 = dbc_params->param[i-1].y * 100;
				lumi = y1 + (in - x1) * (y - y1) / (x - x1);
				break;
			}
		}
	}

	lumi = lumi * dbc_brightness / BL_DBC_MAX;

	if (lumi <= dbc_params->param[0].y * 100) {
		out = dbc_params->param[0].x;
	} else
	if (lumi >= dbc_params->param[cnt-1].y * 100) {
		out = dbc_params->param[cnt-1].x;
	} else {
		for (i = 1;i < cnt;i++) {
			if (lumi <= dbc_params->param[i].y * 100) {
				x  = dbc_params->param[i].x;
				x1 = dbc_params->param[i-1].x;
				y  = dbc_params->param[i].y * 100;
				y1 = dbc_params->param[i-1].y * 100;
				out = x1 + (lumi - y1) * (x - x1) / (y - y1);
				break;
			}
		}
	}
...
param_err:
	pr_debug("%s:out lumi:%d ret:%d\n", __func__, (u32)lumi, (u32)out);
...
static int wled_parse_dbc(struct wled *wled)
{
	...
	wled->dbc_params.cnt = 0;

	if (of_get_property(pdev->dev.of_node,
		"sharp,wled-dbc-params", &size)) {
		cnt = size / sizeof(int);
	} else {
		pr_err("%s:Property dbc-params not available\n", __func__);
		return 0;
	}

	if (!size) {
		pr_err("%s:Property dbc-params size = 0\n", __func__);
		return 0;
	}

	wled->dbc_params.param = devm_kzalloc(&pdev->dev, size, GFP_KERNEL);
	if (!wled->dbc_params.param) {
		pr_err("%s:Failed to alloc mem for dbc-params\n",__func__);
		return 0;
	}

	rc = of_property_read_u32_array(pdev->dev.of_node,
		"sharp,wled-dbc-params",
		(u32 *)wled->dbc_params.param, cnt);
	if (rc) {
		pr_err("%s:Error in reading property: dbc-params\n",
			__func__);
		return 0;
	}
	wled->dbc_params.cnt = cnt / 2;

	pr_debug("%s:dbc_params cnt:%d\n", __func__, wled->dbc_params.cnt);
	for (i = 0;i < wled->dbc_params.cnt;i++) {
		pr_debug("%s:dbc_param %d x:%d y:%d\n", __func__, i,
			wled->dbc_params.param[i].x,
			wled->dbc_params.param[i].y);
	}
