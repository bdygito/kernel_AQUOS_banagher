...
static void backlight_device_init_dbc(void);
				    ...
				    unsigned long brightness)
{
	...
	mutex_lock(&bd->ops_lock);
	if (bd->ops) {
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00052 */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00052 */
		if (!brightness) {
			backlight_device_init_dbc();
		}
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00052 */
static ssize_t dbc_brightness_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct backlight_device *bd = to_backlight_device(dev);

	return sprintf(buf, "%d\n", bd->props.dbc_brightness);
}
...
static void backlight_device_init_dbc(void)
{
	...
	list_for_each_entry(bd, &backlight_dev_list, entry) {
		if (bd->props.type == BACKLIGHT_RAW) {
			bd->props.dbc_brightness = BL_DBC_DEFAULT;
			bd->props.ignore_dbc = false;
		}
	}
	mutex_unlock(&backlight_dev_list_mutex);
}
EXPORT_SYMBOL(backlight_device_init_dbc);

static ssize_t dbc_brightness_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int rc;
	struct backlight_device *bd = to_backlight_device(dev);
	unsigned long dbc_brightness;

	rc = kstrtoul(buf, 0, &dbc_brightness);
	if (rc) {
		pr_err("%s:dbc_brightness param error rc=%d\n", __func__, rc);
		return rc;
	}

	if (dbc_brightness == 0 || dbc_brightness > BL_DBC_MAX) {
		pr_err("%s:dbc_brightness param error dbc_brightness=%d\n", __func__, dbc_brightness);
		return -EINVAL;
	}

	bd->props.dbc_brightness = dbc_brightness;
	if (bd->ops->update_dbc_brightness) {
		bd->ops->update_dbc_brightness(bd, dbc_brightness);
	}

	rc = backlight_device_set_brightness(bd, bd->props.brightness);

	return rc ? rc : count;
}
static DEVICE_ATTR_RW(dbc_brightness);
#endif /* CONFIG_SHARP_DISPLAY */
		...
		struct device_attribute *attr, char *buf)
{
	...
	if ((bd) && (bd->ops) && (bd->ops->get_curr_boost)) {
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((bd) && (bd->ops) && (bd->ops->req_curr_boost)) {
...
static ssize_t backlight_dev_brightness_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct backlight_device *bd = to_backlight_device(dev);
	unsigned int brightness = 0;

	if ((bd) && (bd->ops) && (bd->ops->get_dev_brightness)) {
		bd->ops->get_dev_brightness(bd, &brightness);
	}
...
static DEVICE_ATTR(dev_brightness, 0664,
		backlight_dev_brightness_show, NULL);
#endif /* CONFIG_SHARP_DISPLAY */

#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01014 */
static ssize_t backlight_bl_dimming_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct backlight_device *bd = to_backlight_device(dev);
	int data, rc;
	...
	if ((bd) && (bd->ops) && (bd->ops->req_bl_dimming)) {
		rc = bd->ops->req_bl_dimming(bd, data);
	}
	return count;
}
/* sysfs attributes exported by bl_dimming */
static DEVICE_ATTR(bl_dimming, 0220,
		NULL, backlight_bl_dimming_store);
#endif /* CONFIG_SHARP_DISPLAY */
