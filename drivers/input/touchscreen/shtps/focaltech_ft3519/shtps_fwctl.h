...
/* --------------------------------------------------------------------------- */
#define DEVICE_ERR_CHECK(check, label) \
	if((check)) \
	goto label
/* --------------------------------------------------------------------------- */
...
typedef int (*fwctl_if_ic_deinit_t)(struct shtps_fwctl_info *);
...
typedef int (*fwctl_if_get_gesturetype_t)(struct shtps_fwctl_info *, u8 *, u16 *, u16 *);
...
typedef int (*fwctl_if_get_tm_raw_self_data_t)(struct shtps_fwctl_info *, u8, u8 *);
typedef int (*fwctl_if_get_tm_raw_self_data_wp_t)(struct shtps_fwctl_info *, u8, u8 *);
...
typedef int (*fwctl_if_cmd_tm_raw_self_data_t)(struct shtps_fwctl_info *, u8);
typedef int (*fwctl_if_cmd_tm_raw_self_data_wp_t)(struct shtps_fwctl_info *, u8);
...
typedef int (*fwctl_if_set_lpwg_single_tap_t)(struct shtps_fwctl_info *, u8);
typedef int (*fwctl_if_glove_enable_t)(struct shtps_fwctl_info *);
typedef int (*fwctl_if_glove_disable_t)(struct shtps_fwctl_info *);
typedef int (*fwctl_if_set_ftb_filter_t)(struct shtps_fwctl_info *, u8);

/* --------------------------------------------------------------------------- */
struct shtps_fwctl_functbl {
	fwctl_if_ic_init_t ic_init_f;
	fwctl_if_ic_deinit_t ic_deinit_f;
	...
	fwctl_if_get_tm_raw_self_data_t get_tm_raw_self_data_f;
	fwctl_if_get_tm_raw_self_data_wp_t get_tm_raw_self_data_wp_f;
	...
	fwctl_if_cmd_tm_raw_self_data_t cmd_tm_raw_self_data_f;
	fwctl_if_cmd_tm_raw_self_data_wp_t cmd_tm_raw_self_data_wp_f;
	...
	fwctl_if_set_lpwg_single_tap_t set_lpwg_single_tap_f;
	fwctl_if_glove_enable_t glove_enable_f;
	fwctl_if_glove_disable_t glove_disable_f;
	fwctl_if_set_ftb_filter_t set_ftb_filter_f;
};
/* --------------------------------------------------------------------------- */
#define shtps_fwctl_ic_init(ts) ts->fwctl_p->fwctl_func_p->ic_init_f(ts->fwctl_p)
#define shtps_fwctl_ic_deinit(ts) ts->fwctl_p->fwctl_func_p->ic_deinit_f(ts->fwctl_p)
#define shtps_fwctl_loader_write_pram(ts, pbt_buf, dw_lenth) \
	ts->fwctl_p->fwctl_func_p->loader_write_pram_f(ts->fwctl_p, pbt_buf, dw_lenth)
#define shtps_fwctl_loader_upgrade(ts, pbt_buf, dw_lenth) \
	ts->fwctl_p->fwctl_func_p->loader_upgrade_f(ts->fwctl_p, pbt_buf, dw_lenth)
#define shtps_fwctl_get_device_status(ts, status) \
	ts->fwctl_p->fwctl_func_p->get_device_status_f(ts->fwctl_p, status)
#define shtps_fwctl_soft_reset(ts) ts->fwctl_p->fwctl_func_p->soft_reset_f(ts->fwctl_p)
#define shtps_fwctl_irqclear_get_irqfactor(ts, status) \
	ts->fwctl_p->fwctl_func_p->irqclear_get_irqfactor_f(ts->fwctl_p, status)
#define shtps_fwctl_rezero(ts) ts->fwctl_p->fwctl_func_p->rezero_f(ts->fwctl_p)
#define shtps_fwctl_map_construct(ts, func_check) \
	ts->fwctl_p->fwctl_func_p->map_construct_f(ts->fwctl_p, func_check)
...
#define shtps_fwctl_set_doze_param(ts, param_p, param_size) \
	ts->fwctl_p->fwctl_func_p->set_doze_param_f(ts->fwctl_p, param_p, param_size)
...
#define shtps_fwctl_set_sleepmode_off(ts) \
	ts->fwctl_p->fwctl_func_p->set_sleepmode_off_f(ts->fwctl_p)
#define shtps_fwctl_set_lpwg_mode_on(ts) ts->fwctl_p->fwctl_func_p->set_lpwg_mode_on_f(ts->fwctl_p)
#define shtps_fwctl_set_lpwg_mode_off(ts) \
	ts->fwctl_p->fwctl_func_p->set_lpwg_mode_off_f(ts->fwctl_p)
#define shtps_fwctl_set_lpwg_mode_cal(ts) \
	ts->fwctl_p->fwctl_func_p->set_lpwg_mode_cal_f(ts->fwctl_p)
#define shtps_fwctl_set_low_reportrate_mode(ts, mode) \
	ts->fwctl_p->fwctl_func_p->set_low_reportrate_mode_f(ts->fwctl_p, mode)
#define shtps_fwctl_get_fingermax(ts) ts->fwctl_p->fwctl_func_p->get_fingermax_f(ts->fwctl_p)
#define shtps_fwctl_get_fingerinfo(ts, buf, read_cnt, irqsts, extsts, finger) \
	ts->fwctl_p->fwctl_func_p->get_fingerinfo_f(ts->fwctl_p, buf, read_cnt, irqsts, extsts, finger)
#define shtps_fwctl_get_one_fingerinfo(ts, id, buf, finger) \
	ts->fwctl_p->fwctl_func_p->get_one_fingerinfo_f(ts->fwctl_p, id, buf, finger)
#define shtps_fwctl_get_num_of_touch_fingers(ts, buf) \
	ts->fwctl_p->fwctl_func_p->get_num_of_touch_fingers_f(ts->fwctl_p, buf)
#define shtps_fwctl_get_finger_info_buf(ts, fingerid, fingerMax, buf) \
	ts->fwctl_p->fwctl_func_p->get_finger_info_buf_f(ts->fwctl_p, fingerid, fingerMax, buf)
#define shtps_fwctl_get_finger_state(ts, fingerid, fingerMax, buf) \
	ts->fwctl_p->fwctl_func_p->get_finger_state_f(ts->fwctl_p, fingerid, fingerMax, buf)
#define shtps_fwctl_get_finger_pointid(ts, buf) \
	ts->fwctl_p->fwctl_func_p->get_finger_pointid_f(ts->fwctl_p, buf)
#define shtps_fwctl_get_finger_pos_x(ts, buf) \
	ts->fwctl_p->fwctl_func_p->get_finger_pos_x_f(ts->fwctl_p, buf)
#define shtps_fwctl_get_finger_pos_y(ts, buf) \
	ts->fwctl_p->fwctl_func_p->get_finger_pos_y_f(ts->fwctl_p, buf)
#define shtps_fwctl_get_finger_wx(ts, buf) \
	ts->fwctl_p->fwctl_func_p->get_finger_wx_f(ts->fwctl_p, buf)
#define shtps_fwctl_get_finger_wy(ts, buf) \
	ts->fwctl_p->fwctl_func_p->get_finger_wy_f(ts->fwctl_p, buf)
#define shtps_fwctl_get_finger_z(ts, buf) \
	ts->fwctl_p->fwctl_func_p->get_finger_z_f(ts->fwctl_p, buf)
#define shtps_fwctl_get_gesture(ts, fingerMax, buf, gs1, gs2) \
	ts->fwctl_p->fwctl_func_p->get_gesture_f(ts->fwctl_p, fingerMax, buf, gs1, gs2)
#define shtps_fwctl_get_keystate(ts, status) \
	ts->fwctl_p->fwctl_func_p->get_keystate_f(ts->fwctl_p, status)
#define shtps_fwctl_get_gesturetype(ts, status, coordinate_x, coordinate_y) \
	ts->fwctl_p->fwctl_func_p->get_gesturetype_f(ts->fwctl_p, status, coordinate_x, coordinate_y)
#define shtps_fwctl_get_fwdate(ts, year, month) \
	ts->fwctl_p->fwctl_func_p->get_fwdate_f(ts->fwctl_p, year, month)
#define shtps_fwctl_get_serial_number(ts, buf) \
	ts->fwctl_p->fwctl_func_p->get_serial_number_f(ts->fwctl_p, buf)
...
#define shtps_fwctl_get_tm_frameline(ts, tm_mode, tm_data) \
	ts->fwctl_p->fwctl_func_p->get_tm_frameline_f(ts->fwctl_p, tm_mode, tm_data)
#define shtps_fwctl_get_tm_baseline(ts, tm_mode, tm_data) \
	ts->fwctl_p->fwctl_func_p->get_tm_baseline_f(ts->fwctl_p, tm_mode, tm_data)
#define shtps_fwctl_get_tm_baseline_raw(ts, tm_mode, tm_data) \
	ts->fwctl_p->fwctl_func_p->get_tm_baseline_raw_f(ts->fwctl_p, tm_mode, tm_data)
#define shtps_fwctl_get_tm_raw_self_data(ts, tm_mode, tm_data) \
	ts->fwctl_p->fwctl_func_p->get_tm_raw_self_data_f(ts->fwctl_p, tm_mode, tm_data)
#define shtps_fwctl_get_tm_raw_self_data_wp(ts, tm_mode, tm_data) \
	ts->fwctl_p->fwctl_func_p->get_tm_raw_self_data_wp_f(ts->fwctl_p, tm_mode, tm_data)
#define shtps_fwctl_get_tm_cbdata(ts, tm_mode, tm_data) \
	ts->fwctl_p->fwctl_func_p->get_tm_cbdata_f(ts->fwctl_p, tm_mode, tm_data)
#define shtps_fwctl_cmd_tm_frameline(ts, tm_mode) \
	ts->fwctl_p->fwctl_func_p->cmd_tm_frameline_f(ts->fwctl_p, tm_mode)
#define shtps_fwctl_cmd_tm_baseline(ts, tm_mode) \
	ts->fwctl_p->fwctl_func_p->cmd_tm_baseline_f(ts->fwctl_p, tm_mode)
#define shtps_fwctl_cmd_tm_raw_self_data(ts, tm_mode) \
	ts->fwctl_p->fwctl_func_p->cmd_tm_raw_self_data_f(ts->fwctl_p, tm_mode)
#define shtps_fwctl_cmd_tm_raw_self_data_wp(ts, tm_mode) \
	ts->fwctl_p->fwctl_func_p->cmd_tm_raw_self_data_wp_f(ts->fwctl_p, tm_mode)
#define shtps_fwctl_cmd_tm_baseline_raw(ts, tm_mode) \
	ts->fwctl_p->fwctl_func_p->cmd_tm_baseline_raw_f(ts->fwctl_p, tm_mode)
#define shtps_fwctl_initparam(ts) ts->fwctl_p->fwctl_func_p->initparam_f(ts->fwctl_p)
#define shtps_fwctl_initparam_activemode(ts) \
	ts->fwctl_p->fwctl_func_p->initparam_activemode_f(ts->fwctl_p)
#define shtps_fwctl_initparam_dozemode(ts) \
	ts->fwctl_p->fwctl_func_p->initparam_dozemode_f(ts->fwctl_p)
#define shtps_fwctl_initparam_key(ts) ts->fwctl_p->fwctl_func_p->initparam_key_f(ts->fwctl_p)
#define shtps_fwctl_initparam_lpwgmode(ts) \
	ts->fwctl_p->fwctl_func_p->initparam_lpwgmode_f(ts->fwctl_p)
#define shtps_fwctl_initparam_autorezero(ts) \
	ts->fwctl_p->fwctl_func_p->initparam_autorezero_f(ts->fwctl_p)
#define shtps_fwctl_initparam_reportrate(ts, mode) \
	ts->fwctl_p->fwctl_func_p->initparam_reportrate_f(ts->fwctl_p, mode)
#define shtps_fwctl_initparam_set_custom_report_rate(ts) \
	ts->fwctl_p->fwctl_func_p->initparam_set_custom_report_rate_f(ts->fwctl_p)
...
#define shtps_fwctl_start_testmode(ts, tm_mode) \
	ts->fwctl_p->fwctl_func_p->start_testmode_f(ts->fwctl_p, tm_mode)
#define shtps_fwctl_stop_testmode(ts) ts->fwctl_p->fwctl_func_p->stop_testmode_f(ts->fwctl_p)
#define shtps_fwctl_baseline_offset_disable(ts) \
	ts->fwctl_p->fwctl_func_p->baseline_offset_disable_f(ts->fwctl_p)
#define shtps_fwctl_set_dev_state(ts, state) \
	ts->fwctl_p->fwctl_func_p->set_dev_state_f(ts->fwctl_p, state)
...
#define shtps_fwctl_set_custom_report_rate(ts, rate) \
	ts->fwctl_p->fwctl_func_p->set_custom_report_rate_f(ts->fwctl_p, rate)
#define shtps_fwctl_set_lpwg_sweep_on(ts, enable) \
	ts->fwctl_p->fwctl_func_p->set_lpwg_sweep_on_f(ts->fwctl_p, enable)
#define shtps_fwctl_set_lpwg_double_tap(ts, enable) \
	ts->fwctl_p->fwctl_func_p->set_lpwg_double_tap_f(ts->fwctl_p, enable)
#define shtps_fwctl_set_lpwg_single_tap(ts, enable) \
	ts->fwctl_p->fwctl_func_p->set_lpwg_single_tap_f(ts->fwctl_p, enable)
...
#define shtps_fwctl_set_ftb_filter(ts, enable) \
	ts->fwctl_p->fwctl_func_p->set_ftb_filter_f(ts->fwctl_p, enable)
/* --------------------------------------------------------------------------- */
