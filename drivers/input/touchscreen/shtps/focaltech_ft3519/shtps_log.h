...
#define DBG_PRINTK(log, ...) printk(KERN_DEBUG TPS_ID0 "[%5d] " log, current->pid, ##__VA_ARGS__)
#define ERR_PRINTK(log, ...) printk(KERN_ERR TPS_ID0 "[%5d] " log, current->pid, ##__VA_ARGS__)
...
//#define	DBG_PRINTK(...)	pr_debug(TPS_ID0 " "__VA_ARGS__)
	...
	#define SHTPS_LOG_ERR_PRINT(log, ...) DBG_PRINTK(log, ##__VA_ARGS__)
#else
...
extern u8 gLogOutputEnable;
		...
		#define SHTPS_LOG_DBG_PRINT(log, ...) \
			if((gLogOutputEnable & 0x02) != 0) { \
				DBG_PRINTK(log, ##__VA_ARGS__); \
			}
