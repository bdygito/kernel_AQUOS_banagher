...
#endif
static struct i2c_client *shtps_fts_i2c_client = NULL;
 ...
 *  Return:
 *******************************************************************************/
static int shtps_fts_i2c_read(struct i2c_client *client, char *writebuf, int writelen,
 ...
 *  Return:
 *******************************************************************************/
static int shtps_fts_i2c_write(struct i2c_client *client, char *writebuf, int writelen)
{
	struct shtps_fts *ts = gShtps_fts;
	int power = (ts->state_mgr.state == SHTPS_STATE_IDLE) ? 0 : 1;
 ...
 * Return: data len
 ***********************************************************************/
static ssize_t shtps_fts_debug_write(struct file *filp, const char __user *buff, size_t count,
                                     loff_t *ppos)
{
 ...
 ***********************************************************************/
static ssize_t shtps_fts_debug_read(struct file *filp, char __user *buff, size_t count,
                                    loff_t *ppos)
{
	int ret = 0;
 ...
 * Return: data len
 ***********************************************************************/
static int shtps_fts_debug_write(struct file *filp, const char __user *buff, unsigned long len,
                                 void *data)
{
	unsigned char writebuf[WRITE_BUF_SIZE];
	int buflen = len;
 ...
 * Return: read char number
 ***********************************************************************/
static int shtps_fts_debug_read(char *page, char **start, off_t off, int count, int *eof,
                                void *data)
{
	int ret = 0;
	unsigned char buf[READ_BUF_SIZE];
...
void shtps_fts_release_apk_debug_channel(struct shtps_fts *ts)
{
	...
	if(shtps_fts_proc_entry)
		#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))//2015.5.20 changed
			proc_remove(shtps_fts_proc_entry);
		#else
 ...
 * Return: EPERM
 ***********************************************************************/
static ssize_t shtps_fts_tpfwver_store(struct device *dev, struct device_attribute *attr,
 ...
 * Return: char number
 ***********************************************************************/
static ssize_t shtps_fts_tpdriver_version_show(struct device *dev, struct device_attribute *attr,
                                               char *buf)
{
	ssize_t num_read_chars = 0;
 ...
 * Return: EPERM
 ***********************************************************************/
static ssize_t shtps_fts_tpdriver_version_store(struct device *dev, struct device_attribute *attr,
                                                const char *buf, size_t count)
{
	/* place holder for future use */
	return -EPERM;
 ...
 * Return: char count
 ***********************************************************************/
static ssize_t shtps_fts_tprwreg_store(struct device *dev, struct device_attribute *attr,
                                       const char *buf, size_t count)
{
	...
	if(0 != retval) {
		dev_err(dev,
		        "%s() - ERROR: Could not convert the given input to a number. The given input was: "
		        "\"%s\"\n",
		        __FUNCTION__, buf);
		goto error_return;
	}
 ...
 * Return: char count
 ***********************************************************************/
static ssize_t shtps_fts_fwupdate_store(struct device *dev, struct device_attribute *attr,
                                        const char *buf, size_t count)
{
	#if 1
		struct i2c_client *client = container_of(dev, struct i2c_client, dev);
 ...
 * Return: EPERM
 ***********************************************************************/
static ssize_t shtps_fts_fwupgradeapp_show(struct device *dev, struct device_attribute *attr,
                                           char *buf)
{
	/* place holder for future use */
	return -EPERM;
 ...
 * Return: char count
 ***********************************************************************/
static ssize_t shtps_fts_fwupgradeapp_store(struct device *dev, struct device_attribute *attr,
                                            const char *buf, size_t count)
{
	#if 1
		struct i2c_client *client = container_of(dev, struct i2c_client, dev);
 ...
 * Return: EPERM
 ***********************************************************************/
static ssize_t shtps_fts_getprojectcode_show(struct device *dev, struct device_attribute *attr,
                                             char *buf)
{
 ...
 * Return: EPERM
 ***********************************************************************/
static ssize_t shtps_fts_getprojectcode_store(struct device *dev, struct device_attribute *attr,
                                              const char *buf, size_t count)
{
	/* place holder for future use */
	return -EPERM;
...
static DEVICE_ATTR(ftstpdriverver, S_IRUGO | S_IWUSR, shtps_fts_tpdriver_version_show,
                   shtps_fts_tpdriver_version_store);
/*upgrade from *.i
 *example: echo 1 > ftsfwupdate
 */
static DEVICE_ATTR(ftsfwupdate, S_IRUGO | S_IWUSR, shtps_fts_fwupdate_show,
                   shtps_fts_fwupdate_store);
 ...
 */
static DEVICE_ATTR(ftsfwupgradeapp, S_IRUGO | S_IWUSR, shtps_fts_fwupgradeapp_show,
                   shtps_fts_fwupgradeapp_store);
static DEVICE_ATTR(ftsgetprojectcode, S_IRUGO | S_IWUSR, shtps_fts_getprojectcode_show,
                   shtps_fts_getprojectcode_store);

/*add your attr in here*/
static struct attribute *shtps_fts_attributes[] = {&dev_attr_ftstpfwver.attr,
...
static struct attribute_group shtps_fts_attribute_group = {.attrs = shtps_fts_attributes};
