 ...
 */
	...
	#define SET_TOUCH_DATA_FINGERSTATE(TOUCH_DATA, para) \
		(TOUCH_DATA[0] = (TOUCH_DATA[0] & 0x3F) | (para << 6))
	#define SET_TOUCH_DATA_XPOS(TOUCH_DATA, para) \
		{ \
			TOUCH_DATA[0] = (TOUCH_DATA[0] & 0xF0) | ((para >> 8) & 0x0F); \
			TOUCH_DATA[1] = para & 0xFF; \
		}
	#define SET_TOUCH_DATA_YPOS(TOUCH_DATA, para) \
		{ \
			TOUCH_DATA[2] = (TOUCH_DATA[2] & 0xF0) | ((para >> 8) & 0x0F); \
			TOUCH_DATA[3] = para & 0xFF; \
		}
	#define SET_TOUCH_DATA_ID(TOUCH_DATA, para) \
		(TOUCH_DATA[2] = (TOUCH_DATA[2] & 0x0F) | ((para << 4) & 0xF0))
	#define SET_TOUCH_DATA_WEIGHT(TOUCH_DATA, para) (TOUCH_DATA[4] = para)
	#define SET_TOUCH_DATA_AREA(TOUCH_DATA, para) \
		(TOUCH_DATA[5] = (TOUCH_DATA[5] & 0x0F) | ((para << 4) & 0xF0))
...
static int shtps_touch_emu_read_touchevent_timer_start(unsigned long delay_sec)
{
	...
	schedule_delayed_work(&gShtps_touch_emu_info.touch_emu_read_touchevent_delayed_work,
	                      msecs_to_jiffies(delay_sec * 1000));
...
static void shtps_touch_emu_read_touchevent_delayed_work_function(struct work_struct *work)
{
	...
	while(gShtps_touch_emu_info.state == SHTPS_TOUCH_EMURATOR_STATE_RUNNING) {
		...
		generation_time_us = gShtps_touch_emu_info.touch_list[gShtps_touch_emu_info.current_touch_index]
		                         .generation_time_ms *
		                     1000;
		...
		while(gShtps_touch_emu_info.state == SHTPS_TOUCH_EMURATOR_STATE_RUNNING) {
			...
			specified_time_us =
			    ((current_timeval.tv_sec - gShtps_touch_emu_info.emu_start_timeval.tv_sec) * 1000000) +
			...
			if(generation_time_us > specified_time_us) {
				...
				SHTPS_TOUCH_EMU_LOG_V("generation_time_us=%lu, specified_time_us=%lu, "
				                      "next_event_time_total_us=%lu sleepcount=%d\n",
				                      generation_time_us, specified_time_us, next_event_time_total_us,
				                      sleepcount);
				break;
			}
		}
...
int shtps_touch_emu_set_finger_info(u8 *buf, int bufsize)
{
	...
	specified_time_ms =
	    ((current_timeval.tv_sec - gShtps_touch_emu_info.emu_start_timeval.tv_sec) * 1000) +
	    ((current_timeval.tv_usec - gShtps_touch_emu_info.emu_start_timeval.tv_usec) / 1000);

	for(i = gShtps_touch_emu_info.current_touch_index; i < gShtps_touch_emu_info.touch_list_num;
	    i++) {
		...
		if(touch_list->generation_time_ms <= specified_time_ms) {
			if((set_data_flg != 0) && (set_data_time != touch_list->generation_time_ms)) {
				...
				SHTPS_TOUCH_EMU_LOG_V("touch emu set time=%u [%d] state=%d x=%d y=%d wx=%d wy=%d z=%d\n",
				                      touch_list->generation_time_ms, touch_list->finger_id,
				                      touch_list->finger_state, touch_list->finger_x, touch_list->finger_y,
				                      touch_list->finger_wx, touch_list->finger_wy, touch_list->finger_z);
			}
		}
		else {
			break;
	...
	for(i = 0; i < SHTPS_FINGER_MAX; i++) {
		...
		if(fw_repot_finger_tmp->state != SHTPS_TOUCH_STATE_NO_TOUCH) {
			...
			SET_TOUCH_DATA_FINGERSTATE(
			    TOUCH_DATA, ((fw_repot_finger_tmp->state == SHTPS_TOUCH_STATE_FINGER) ? 0 : 3));
...
//					SET_TOUCH_DATA_XPOS(TOUCH_DATA, (shtps_fwctl_get_maxXPosition(ts) - fw_repot_finger_tmp->x));
//					SET_TOUCH_DATA_YPOS(TOUCH_DATA, (shtps_fwctl_get_maxYPosition(ts) - fw_repot_finger_tmp->y));
//				}
//			#endif /* SHTPS_COORDINATES_POINT_SYMMETRY_ENABLE */
			...
			SET_TOUCH_DATA_AREA(TOUCH_DATA, (fw_repot_finger_tmp->wx > fw_repot_finger_tmp->wy
			                                     ? fw_repot_finger_tmp->wx
			                                     : fw_repot_finger_tmp->wy));
			td_finger_num++;
		}
	}
	buf[0] = 0;
	...
	//	SHTPS_TOUCH_EMU_LOG_V("0x%02X 0x%02X 0x%02X | 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X |
...
static void shtps_touch_emu_recording_data(unsigned char *buffer, ssize_t size)
{
	...
	if(gShtps_touch_emu_info.touchLogFilep) {
		...
		nr_write = gShtps_touch_emu_info.touchLogFilep->f_op->write(
		    gShtps_touch_emu_info.touchLogFilep, buffer, size,
		    &gShtps_touch_emu_info.touchLogFilep->f_pos);
...
int shtps_touch_emu_rec_finger_info(u8 *buf)
{
	...
	do_gettimeofday(&current_timeval);
	recording_time_ms =
	    ((current_timeval.tv_sec - gShtps_touch_emu_info.rec_start_timeval.tv_sec) * 1000) +
...
//				fw_report_info_tmp.fingers[i].x = shtps_fwctl_get_maxXPosition(ts) - fw_report_info_tmp.fingers[i].x;
//				fw_report_info_tmp.fingers[i].y = shtps_fwctl_get_maxYPosition(ts) - fw_report_info_tmp.fingers[i].y;
//			}
//		#endif /* SHTPS_COORDINATES_POINT_SYMMETRY_ENABLE */
	...
	for(i = 0; i < fingerMax; i++) {
		if((fw_report_info_tmp.fingers[i].state !=
		    gShtps_touch_emu_info.fw_report_info.fingers[i].state) ||
		   ...
		   (fw_report_info_tmp.fingers[i].z != gShtps_touch_emu_info.fw_report_info.fingers[i].z)) {
			...
			strsize = sprintf(strbuf, "%lu,%d,%d,%d,%d,%d,%d,%d\n", recording_time_ms,
			                  (fw_report_info_tmp.fingers[i].state), i, (fw_report_info_tmp.fingers[i].x),
			                  (fw_report_info_tmp.fingers[i].y), (fw_report_info_tmp.fingers[i].wx),
			                  (fw_report_info_tmp.fingers[i].wy), (fw_report_info_tmp.fingers[i].z));
...
static int shtps_touch_emu_start_recording(u8 *fileName)
{
	...
	if(!gShtps_touch_emu_info.touchLogFilep) {
		...
		if(IS_ERR(gShtps_touch_emu_info.touchLogFilep)) {
			...
			memset(&gShtps_touch_emu_info.fw_report_info, 0,
			       sizeof(gShtps_touch_emu_info.fw_report_info));
...
static int shtps_get_arguments(char *argStr,   /* [I/O] arguments strings (processed in function) */
...
static int shtps_sysfs_start_touch_emurator(const char *buf, size_t count)
{
	u8 *data;
...

	{
		...
		for(i = 0; i < gShtps_touch_emu_info.touch_list_num; i++) {
			...
			SHTPS_TOUCH_EMU_LOG_DBG_PRINT(
			    "time=%u state[%d] Touch Info[%d] x=%d, y=%d, wx=%d, wy=%d, z=%d\n",
			    list_elem->generation_time_ms, list_elem->finger_state, list_elem->finger_id,
			    list_elem->finger_x, list_elem->finger_y, list_elem->finger_wx, list_elem->finger_wy,
			    list_elem->finger_z);
		}
	}
...
static int shtps_sysfs_stop_touch_emurator(void)
{
...
static ssize_t show_sysfs_emurator_ctrl(struct kobject *kobj, struct kobj_attribute *attr,
                                        char *buf)
{
...
static ssize_t store_sysfs_emurator_ctrl(struct kobject *kobj, struct kobj_attribute *attr,
...
static ssize_t show_sysfs_emurator_data(struct kobject *kobj, struct kobj_attribute *attr,
                                        char *buf)
{
	sprintf(buf, "emurator_data_size = %d byte(list num = %d)\n",
	        gShtps_touch_emu_info.touch_list_buffering_size, gShtps_touch_emu_info.touch_list_num);

	return (strlen(buf));
}

static ssize_t store_sysfs_emurator_data(struct kobject *kobj, struct kobj_attribute *attr,
                                         const char *buf, size_t count)
{
	char *data;
	int copy_size;
	...
	if(gShtps_touch_emu_info.state == SHTPS_TOUCH_EMURATOR_STATE_DISABLE) {
		...
		if(gShtps_touch_emu_info.touch_list == NULL) {
			...
			data =
			    (char *)kmalloc(gShtps_touch_emu_info.touch_list_buffering_size + copy_size, GFP_KERNEL);
			if(data == NULL) {
				return -ENOMEM;
			}
			memcpy(data, gShtps_touch_emu_info.touch_list,
			       gShtps_touch_emu_info.touch_list_buffering_size);
		...
		gShtps_touch_emu_info.touch_list_num = gShtps_touch_emu_info.touch_list_buffering_size /
		                                       sizeof(struct shtps_touch_emurator_list_elem);

		SHTPS_TOUCH_EMU_LOG_DBG_PRINT(
		    "store_sysfs_emurator_data() call end current size = %d byte(list num = %d)\n",
		    gShtps_touch_emu_info.touch_list_buffering_size, gShtps_touch_emu_info.touch_list_num);
	}
	else {
...
static ssize_t store_sysfs_emurator_record_start(struct kobject *kobj, struct kobj_attribute *attr,
                                                 const char *buf, size_t count)
{
	int ret = 0;
	char *argbuf;
...
static ssize_t store_sysfs_emurator_record_stop(struct kobject *kobj, struct kobj_attribute *attr,
                                                const char *buf, size_t count)
{
	int ret = 0;
...
static void shtps_touch_eventlog_recording_data(unsigned char *buffer, ssize_t size)
{
	...
	if(gShtps_touch_eventlog_info.touchLogFilep) {
		...
		nr_write = gShtps_touch_eventlog_info.touchLogFilep->f_op->write(
		    gShtps_touch_eventlog_info.touchLogFilep, buffer, size,
		    &gShtps_touch_eventlog_info.touchLogFilep->f_pos);

		set_fs(fs);
	}
}
int shtps_touch_eventlog_rec_event_info(int state, int finger, int x, int y, int w, int wx, int wy,
                                        int z)
{
	struct timeval current_timeval;
	unsigned long recording_time_ms;
...
static ssize_t store_sysfs_eventlog_record_start(struct kobject *kobj, struct kobj_attribute *attr,
                                                 const char *buf, size_t count)
{
	int ret = 0;
	char *argbuf;
	...
	if(gShtps_touch_eventlog_info.state == SHTPS_TOUCH_EVENTLOG_STATE_DISABLE) {
		if(argc >= 1) {
			...
			SHTPS_TOUCH_EVENTLOG_LOG_DBG_PRINT(
			    "touch eventlog recording not start because few paramters.\n");
			ret = -EINVAL;
		}
	}
	else {
		SHTPS_TOUCH_EVENTLOG_LOG_DBG_PRINT(
		    "touch eventlog recording not start because already running.\n");
		ret = -1;
	}
...
static ssize_t store_sysfs_eventlog_record_stop(struct kobject *kobj, struct kobj_attribute *attr,
                                                const char *buf, size_t count)
{
	int ret = 0;
...
#if defined(SHTPS_ACCUMULATE_FW_REPORT_INFO_ENABLE)
static int shtps_accumulate_fw_report_info_push(struct shtps_accumulate_fw_report_info *mng_p,
                                                struct shtps_touch_info *touch_info,
...
static struct shtps_touch_info *
shtps_accumulate_fw_report_info_pop(struct shtps_accumulate_fw_report_info *mng_p, int *id,
                                    unsigned long *time_us)
{
...
void shtps_accumulate_fw_report_info_add(struct shtps_touch_info *touch_info)
{
	...
	do_gettimeofday(&current_timeval);
	specified_time_us =
	    ((current_timeval.tv_sec - gShtps_accumulate_fw_report_info_p->start_timeval.tv_sec) *
	     1000000) +
	    ((current_timeval.tv_usec - gShtps_accumulate_fw_report_info_p->start_timeval.tv_usec));

	shtps_accumulate_fw_report_info_push(gShtps_accumulate_fw_report_info_p, touch_info,
	                                     specified_time_us);
	return;
}

static ssize_t show_sysfs_fw_report_info(struct kobject *kobj, struct kobj_attribute *attr,
                                         char *buf)
{
	int i;
	int cnt;
	...
	for(cnt = 0; cnt < loopmax; cnt++) {
		info = shtps_accumulate_fw_report_info_pop(gShtps_accumulate_fw_report_info_p,
		                                           &fw_report_info_id, &time_us);
		if(info != NULL) {
			for(i = 0; i < fingerMax; i++) {
				sprintf(workbuf, "[%d] time=%lu,id=%d,state=%d,x=%d,y=%d,wx=%d,wy=%d,z=%d\n",
				        fw_report_info_id, time_us, i, info->fingers[i].state, info->fingers[i].x,
				        info->fingers[i].y, info->fingers[i].wx, info->fingers[i].wy, info->fingers[i].z);
				strcat(buf, workbuf);
			}
		}
	}
...
struct shtps_debug_param_regist_info fw_report_info_sysfs_regist_info[] = {
    {"fw_report_info", SHTPS_DEBUG_PARAM_NAME_ONOFF, &SHTPS_ACCUMULATE_FW_REPORT_INFO_ONOFF, NULL,
     NULL},
...
static struct kobject *shtps_debug_search_kobj(struct kobject *parent_kobj_p,
...
static struct shtps_debug_attr_list_info *shtps_debug_search_attr(struct kobject *parent_kobj_p,
                                                                  const char *search_name)
{
	struct list_head *list_p;
...
static ssize_t shtps_debug_common_store(struct kobject *kobj, struct kobj_attribute *attr,
                                        const char *buf, size_t count)
{
	struct shtps_debug_attr_list_info *find_attr_p = NULL;
	int val;
...
static void shtps_debug_delete_kobj_all(void)
{
	...
	while(list_empty(&(gDegbuInfo->kobj_list)) == 0) {
		item_info_p =
		    list_entry(gDegbuInfo->kobj_list.next, struct shtps_debug_kobj_list_info, kobj_list);
...
static int shtps_debug_create_attr(struct kobject *parent_kobj_p, struct kobj_attribute attr,
                                   int *param_p)
{
...
static void shtps_debug_delete_attr_all(void)
{
	...
	while(list_empty(&(gDegbuInfo->attr_list)) == 0) {
		item_info_p =
		    list_entry(gDegbuInfo->attr_list.next, struct shtps_debug_attr_list_info, attr_list);
...
int shtps_debug_init(struct shtps_debug_init_param *param)
{
	...
	shtps_debug_param_obj_init(param->shtps_root_kobj);
		...
		gShtps_touch_emu_info.state = SHTPS_TOUCH_EMURATOR_STATE_DISABLE;
		INIT_DELAYED_WORK(&gShtps_touch_emu_info.touch_emu_read_touchevent_delayed_work,
		                  shtps_touch_emu_read_touchevent_delayed_work_function);
	...
	#if defined(SHTPS_ACCUMULATE_FW_REPORT_INFO_ENABLE)
		gShtps_accumulate_fw_report_info_p =
		    kzalloc(sizeof(struct shtps_accumulate_fw_report_info), GFP_KERNEL);
		if(gShtps_accumulate_fw_report_info_p != NULL) {
