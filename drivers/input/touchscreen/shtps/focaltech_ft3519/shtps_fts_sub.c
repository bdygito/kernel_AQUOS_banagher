 ...
 */
	...
	#define SHTPS_RECORD_LOG_BUF_SPACING_SIZE \
		(((SHTPS_RECORD_LOG_EVENT_STORE_NUM * \
		   (SHTPS_RECORD_LOG_DATE_STR_LENGTH + SHTPS_RECORD_LOG_EVENT_STR_LENGTH)) * \
		  2))
	...
	const static char *WeekOfDay[] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
	...
	static ssize_t shtps_record_log_kernel_read(struct file *fp, char *buf, size_t size,
	                                            unsigned int offset);
	...
	static int shtps_record_log_add_event_log_store(
	    char *logstr, struct shtps_record_log_event_store_log *store_tbl_p, int *array);
 ...
 */
	...
	static void shtps_perflock_cluster0_enable_check(struct shtps_fts *ts, u8 event,
	                                                 struct shtps_touch_info *info);
	static int shtps_perflock_cluster0_disable_timer_start(struct shtps_fts *ts,
	...
	static void shtps_perflock_cluster1_enable_check(struct shtps_fts *ts, u8 event,
	                                                 struct shtps_touch_info *info);
	static int shtps_perflock_cluster1_disable_timer_start(struct shtps_fts *ts,
	                                                       unsigned long delay_ms);
...
void shtps_performance_check_init(void)
{
		...
		result = gpio_tlmm_config(
		    GPIO_CFG(SHTPS_PERFORMANCE_CHECK_PIN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
		    GPIO_CFG_ENABLE);
		if(result < 0) {
...
void shtps_performance_check(int state)
{
	...
	//	SHTPS_LOG_FUNC_CALL();
		...
		if(state == SHTPS_PERFORMANCE_CHECK_STATE_START) {
			...
			shtps_performance_check_pin_state = (shtps_performance_check_pin_state == 0) ? 1 : 0;
		}
...
void shtps_lpwg_proximity_check_start(struct shtps_fts *ts)
{
	...
	schedule_delayed_work(&ts->proximity_check_delayed_work,
	                      msecs_to_jiffies(SHTPS_LPWG_PROXIMITY_CHECK_PREWAIT));
	...
	#if defined(SHTPS_LPWG_MODE_ENABLE)
	{
		...
		if(ts->lpwg_proximity_get_data == SHTPS_PROXIMITY_NEAR) {
			...
			if(ts->lpwg.is_notified != 0) {
				if(time_after(jiffies, ts->lpwg.notify_time +
				                           msecs_to_jiffies(SHTPS_LPWG_MIN_NOTIFY_CANCEL_INTERVAL))) {
					time = 0;
				}
				else {
					time = jiffies_to_msecs(
					    ((ts->lpwg.notify_time + msecs_to_jiffies(SHTPS_LPWG_MIN_NOTIFY_CANCEL_INTERVAL)) -
					     jiffies) %
...
static void shtps_perflock_cluster0_enable(struct shtps_fts *ts)
{
		...
		for_each_possible_cpu(cpu)
		{
			if(enable_cpu_core_num < SHTPS_PERF_LOCK_CLUSTER0_ENABLE_CORE_NUM) {
				if(cpu < SHTPS_PERF_LOCK_CLUSTER1_TOP_CPU_ID) {
					#if defined(SHTPS_DEF_CTRL_CPU_CLOCK_LINKED_LCD_BRIGHT_ENABLE)
						if(((ts->lpmode_req_state | ts->lpmode_continuous_req_state) &
						    SHTPS_LPMODE_REQ_LCD_BRIGHT) == 0) {
							...
							SHTPS_PERF_LOCK_PRINT("perf_lock <cluster0> set [cpu=%d, freq=%d]\n", cpu,
							                      SHTPS_PERF_LOCK_CLUSTER0_CLOCK_FREQUENCY);
						}
						else {
							per_cpu(perflock_freq, cpu) = SHTPS_PERF_LOCK_CLOCK_FREQUENCY_LCD_LPM;
							SHTPS_PERF_LOCK_PRINT("perf_lock <cluster0> set [cpu=%d, freq=%d]\n", cpu,
							                      SHTPS_PERF_LOCK_CLOCK_FREQUENCY_LCD_LPM);
						}
					#else
						per_cpu(perflock_freq, cpu) = SHTPS_PERF_LOCK_CLUSTER0_CLOCK_FREQUENCY;
						SHTPS_PERF_LOCK_PRINT("perf_lock <cluster0> set [cpu=%d, freq=%d]\n", cpu,
	...
	if(SHTPS_PERF_LOCK_QOS_LATENCY_ENABLE != 0) {
		if(ts->cpu_clock_ctrl_p->qos_cpu_cluster0_latency_state == 0) {
			pm_qos_update_request(&ts->cpu_clock_ctrl_p->qos_cpu_cluster0_latency,
			                      SHTPS_PERF_LOCK_QOS_LATENCY_DEF_VALUE);
...
static void shtps_perflock_cluster1_enable(struct shtps_fts *ts)
{
				...
				if(cpu >= SHTPS_PERF_LOCK_CLUSTER1_TOP_CPU_ID) {
						...
						if(((ts->lpmode_req_state | ts->lpmode_continuous_req_state) &
							...
							SHTPS_PERF_LOCK_PRINT("perf_lock <cluster1> set [cpu=%d, freq=%d]\n", cpu,
							                      SHTPS_PERF_LOCK_CLUSTER1_CLOCK_FREQUENCY);
						}
						else {
							per_cpu(perflock_freq, cpu) = SHTPS_PERF_LOCK_CLOCK_FREQUENCY_LCD_LPM;
							SHTPS_PERF_LOCK_PRINT("perf_lock <cluster1> set [cpu=%d, freq=%d]\n", cpu,
	...
	if(SHTPS_PERF_LOCK_QOS_LATENCY_ENABLE != 0) {
		if(ts->cpu_clock_ctrl_p->qos_cpu_cluster1_latency_state == 0) {
			pm_qos_update_request(&ts->cpu_clock_ctrl_p->qos_cpu_cluster1_latency,
			                      SHTPS_PERF_LOCK_QOS_LATENCY_DEF_VALUE);
...
static int shtps_perflock_cluster0_disable_timer_start(struct shtps_fts *ts, unsigned long delay_ms)
{
	...
	schedule_delayed_work(&ts->cpu_clock_ctrl_p->perf_lock_cluster0_disable_delayed_work,
...
{
	struct delayed_work *dw = container_of(work, struct delayed_work, work);
	struct shtps_cpu_clock_ctrl_info *cpuctrl =
	    container_of(dw, struct shtps_cpu_clock_ctrl_info, perf_lock_cluster0_disable_delayed_work);
...
static int shtps_perflock_cluster1_disable_timer_start(struct shtps_fts *ts, unsigned long delay_ms)
{
	...
	schedule_delayed_work(&ts->cpu_clock_ctrl_p->perf_lock_cluster1_disable_delayed_work,
	                      msecs_to_jiffies(delay_ms));
...
static void shtps_perflock_cluster1_disable_delayed_work_function(struct work_struct *work)
{
	struct delayed_work *dw = container_of(work, struct delayed_work, work);
	struct shtps_cpu_clock_ctrl_info *cpuctrl =
	    container_of(dw, struct shtps_cpu_clock_ctrl_info, perf_lock_cluster1_disable_delayed_work);
	// struct shtps_fts *ts = container_of(cpuctrl, struct shtps_fts, cpu_clock_ctrl_p);
	struct shtps_fts *ts = cpuctrl->ts_p;
                                                 ...
                                                 struct shtps_touch_info *info)
{
	...
	if(SHTPS_PERF_LOCK_ENABLE_ALLEVENTS == 1
			...
			for(i = 0; i < fingerMax; i++) {
				if(ts->cpu_clock_ctrl_p->fingers_hist[0][i].state != SHTPS_TOUCH_STATE_NO_TOUCH) {
					if(ts->cpu_clock_ctrl_p->fingers_hist[1][i].state == SHTPS_TOUCH_STATE_NO_TOUCH) {
						...
						x_diff = SHTPS_ABS_CALC(ts->cpu_clock_ctrl_p->fingers_hist[1][i].x,
						                        ts->cpu_clock_ctrl_p->fingers_hist[0][i].x);
						y_diff = SHTPS_ABS_CALC(ts->cpu_clock_ctrl_p->fingers_hist[1][i].y,
						                        ts->cpu_clock_ctrl_p->fingers_hist[0][i].y);
					}
		...
		if(shtps_fwctl_get_dev_state(ts) == SHTPS_DEV_STATE_ACTIVE
			...
			SHTPS_PERF_LOCK_PRINT(
				"shtps_perflock_disable_timer start (%dms) by %s\n", perf_lock_enable_time,
...
void shtps_perflock_enable_start(struct shtps_fts *ts, u8 event, struct shtps_touch_info *info)
{
	...
	if(event == SHTPS_EVENT_TU) {
		...
		memcpy(ts->cpu_clock_ctrl_p->fingers_hist[1], ts->cpu_clock_ctrl_p->fingers_hist[0],
		       sizeof(ts->cpu_clock_ctrl_p->fingers_hist[0]));
...
void shtps_perflock_init(struct shtps_fts *ts)
{
	...
	shtps_perflock_set_event(ts, SHTPS_EVENT_TU);
	INIT_DELAYED_WORK(&ts->cpu_clock_ctrl_p->perf_lock_cluster0_disable_delayed_work,
	                  shtps_perflock_cluster0_disable_delayed_work_function);
	INIT_DELAYED_WORK(&ts->cpu_clock_ctrl_p->perf_lock_cluster1_disable_delayed_work,
	                  shtps_perflock_cluster1_disable_delayed_work_function);
	...
	pm_qos_add_request(&ts->cpu_clock_ctrl_p->qos_cpu_cluster0_latency, PM_QOS_CPU_DMA_LATENCY,
	                   PM_QOS_DEFAULT_VALUE);
	pm_qos_add_request(&ts->cpu_clock_ctrl_p->qos_cpu_cluster1_latency, PM_QOS_CPU_DMA_LATENCY,
	                   PM_QOS_DEFAULT_VALUE);
...
void shtps_perflock_deinit(struct shtps_fts *ts)
{
	...
	if(ts->cpu_clock_ctrl_p)
		kfree(ts->cpu_clock_ctrl_p);
...
void shtps_cpu_idle_sleep_wake_lock_init(struct shtps_fts *ts)
{
	...
	pm_qos_add_request(&ts->cpu_idle_sleep_ctrl_p->qos_cpu_latency, PM_QOS_CPU_DMA_LATENCY,
	                   PM_QOS_DEFAULT_VALUE);
}
...
void shtps_cpu_idle_sleep_wake_lock_deinit(struct shtps_fts *ts)
{
	...
	if(ts->cpu_idle_sleep_ctrl_p)
		kfree(ts->cpu_idle_sleep_ctrl_p);
...
void shtps_wake_lock_for_fwupdate(struct shtps_fts *ts)
{
	...
	if(ts->cpu_sleep_ctrl_fwupdate_p->wake_lock_for_fwupdate_state == 0) {
		...
		pm_qos_update_request(&ts->cpu_sleep_ctrl_fwupdate_p->qos_cpu_latency_for_fwupdate,
		                      SHTPS_QOS_LATENCY_DEF_VALUE);
...
{
	ts->cpu_sleep_ctrl_fwupdate_p =
	    kzalloc(sizeof(struct shtps_cpu_sleep_ctrl_fwupdate_info), GFP_KERNEL);
	...
	wake_lock_init(&ts->cpu_sleep_ctrl_fwupdate_p->wake_lock_for_fwupdate, ts->ctrl_dev_p,
	               "shtps_wake_lock_for_fwupdate");
	ts->cpu_sleep_ctrl_fwupdate_p->qos_cpu_latency_for_fwupdate.type = PM_QOS_REQ_ALL_CORES;
	pm_qos_add_request(&ts->cpu_sleep_ctrl_fwupdate_p->qos_cpu_latency_for_fwupdate,
...
void shtps_fwupdate_wake_lock_deinit(struct shtps_fts *ts)
{
	...
	if(ts->cpu_sleep_ctrl_fwupdate_p)
		kfree(ts->cpu_sleep_ctrl_fwupdate_p);
...
static void shtps_func_lcd_on(struct shtps_fts *ts)
{
	...
	#if defined(SHTPS_BOOT_FWUPDATE_ENABLE)
	if(ts->boot_fwupdate_request == 1) {
		ts->boot_fwupdate_request = 0;
...
static void shtps_func_setlpwg_doubletap(struct shtps_fts *ts, u8 on)
{
...
static void shtps_func_doubletap_on(struct shtps_fts *ts)
{
	shtps_func_setlpwg_doubletap(ts, 1);
}

static void shtps_func_doubletap_off(struct shtps_fts *ts)
{
...
static void shtps_func_setlpwg_singletap(struct shtps_fts *ts, u8 on)
{
	SHTPS_LOG_FUNC_CALL_INPARAM(on);
	...
	#if defined(SHTPS_POWER_OFF_IN_SLEEP_ENABLE)
	{
		...
		shtps_mutex_lock_ctrl();
		ts->lpwg.lpwg_single_tap_req_state = on;
...
static void shtps_func_singletap_on(struct shtps_fts *ts)
{
	shtps_func_setlpwg_singletap(ts, 1);
}

static void shtps_func_singletap_off(struct shtps_fts *ts)
{
	shtps_func_setlpwg_singletap(ts, 0);
}
...
static void shtps_func_boot_fw_update(struct shtps_fts *ts)
{
	...
	if(shtps_boot_fwupdate_enable_check(ts) != 0) {
		...
		#ifdef SHTPS_BOOT_FWUPDATE_FORCE_UPDATE
			DBG_PRINTK("request force update fw(%04X -> %04X)\n", ver, shtps_fwver_builtin(ts));
			update = 1;
...
int shtps_record_log_api_add_log(int type)
{
	...
	SHTPS_RECORD_LOG_FILE_PRINT("%s() [%s]\n", __func__,
	                            (type == SHTPS_RECORD_LOG_TYPE_LCD_ON)
	                                ? "LCD_ON"
	                                : (type == SHTPS_RECORD_LOG_TYPE_LCD_OFF)
	                                      ? "LCD_OFF"
	                                      : (type == SHTPS_RECORD_LOG_TYPE_REZERO) ? "REZERO"
	                                                                               : "unknown");
...
static int shtps_record_log_file_init_read(void)
{
	...
	memcpy(shtps_dur_info_p->event_init_read_tbl, shtps_dur_info_p->event_store_tbl,
	       sizeof(shtps_dur_info_p->event_init_read_tbl));
	...
	shtps_dur_info_p->write_length = shtps_record_log_kernel_read(
	    fp, shtps_dur_info_p->write_buf, sizeof(shtps_dur_info_p->write_buf), 0);
	...
	for(i = 0; i < shtps_dur_info_p->write_length; i++) {
		if(shtps_dur_info_p->write_buf[i] == '\n') {
			...
			memcpy(shtps_dur_info_p->event_store_tbl[event_log_str_cnt].event_log_str,
			       &(shtps_dur_info_p->write_buf[1 + i - event_log_length_cnt]), event_log_length_cnt);
	...
	if(wk_array == 0) {
		for(i = 0; i < SHTPS_RECORD_LOG_EVENT_STORE_NUM; i++) {
			if(shtps_dur_info_p->event_init_read_tbl[i].event_log_length > 0) {
				memcpy(shtps_dur_info_p->event_store_tbl[event_log_str_cnt].event_log_str,
				       shtps_dur_info_p->event_init_read_tbl[i].event_log_str,
				       shtps_dur_info_p->event_init_read_tbl[i].event_log_length);
				shtps_dur_info_p->event_store_tbl[event_log_str_cnt].event_log_length =
				    shtps_dur_info_p->event_init_read_tbl[i].event_log_length;
				event_log_str_cnt++;
				if(event_log_str_cnt >= SHTPS_RECORD_LOG_EVENT_STORE_NUM) {
					event_log_str_cnt = 0;
...
void shtps_record_log_output(void)
{
	...
	SHTPS_RECORD_LOG_FILE_PRINT("%s() OUTPUT event_store_tbl_array[%d]\n", __func__,
	                            shtps_dur_info_p->event_store_tbl_array);
	for(i = 0; i < SHTPS_RECORD_LOG_EVENT_STORE_NUM; i++) {
		SHTPS_RECORD_LOG_FILE_PRINT(
		    "%s() OUTPUT event_store_tbl[%d] event_log_str[%s] event_log_length[%d]\n", __func__, i,
		    shtps_dur_info_p->event_store_tbl[i].event_log_str,
		    shtps_dur_info_p->event_store_tbl[i].event_log_length);
	}
}
...
static int shtps_record_log_file(void)
{
	...
	if(shtps_dur_info_p->write_length) {
		...
		SHTPS_RECORD_LOG_FILE_PRINT("%s()  write_length: %d write start\n", __func__,
		                            shtps_dur_info_p->write_length);
		shtps_dur_info_p->write_length = shtps_record_log_kernel_write(fp, shtps_dur_info_p->write_buf,
		                                                               shtps_dur_info_p->write_length);
...
static int shtps_record_log_summary_init_file(void)
{
	...
	SHTPS_RECORD_LOG_FILE_PRINT("%s() open file: %s pid=%d tgid=%d comm=%s\n", __func__,
	                            SHTPS_RECORD_LOG_FILE, current->pid, current->tgid, current->comm);
...
{
	int ret = 0;
	ret = shtps_record_log_add_event_log_store(logstr, shtps_dur_info_p->event_store_tbl,
	                                           &(shtps_dur_info_p->event_store_tbl_array));
	return ret;
}
    ...
    char *logstr, struct shtps_record_log_event_store_log *store_tbl_p, int *array)
{
	...
	if(loglen > 0) {
		...
		SHTPS_RECORD_LOG_FILE_PRINT("%s() event log stored length:%d\n", __func__,
		                            store_tbl_p[*array].event_log_length);
		(*array)++;
	...
	if(wk_array == 0) {
		for(i = 0; i < SHTPS_RECORD_LOG_EVENT_STORE_NUM; i++) {
			if(store_tbl_p[i].event_log_length > 0) {
				memcpy(&(shtps_dur_info_p->write_buf[write_buf_cnt]), store_tbl_p[i].event_log_str,
				       store_tbl_p[i].event_log_length);
...
static void shtps_func_wakelock_init(struct shtps_fts *ts)
{
	...
	wake_lock_init(&ts->work_wake_lock, ts->ctrl_dev_p, "shtps_work_wake_lock");
...
static void shtps_func_workq(struct work_struct *work_p)
{
	...
	while(list_empty(&(ts->queue)) == 0) {
		...
		switch(ts->cur_msg_p->event) {
				...
				case SHTPS_FUNC_REQ_BOOT_FW_UPDATE:
					{
						...
						if(driver_state == SHTPS_STATE_ACTIVE) {
							shtps_func_boot_fw_update(ts);
						}
						else {
							ts->boot_fwupdate_request = 1;
						}
					}
					ts->cur_msg_p->status = 0;
					break;
			...
			#if defined(SHTPS_LPWG_SINGLE_TAP_ENABLE)
				case SHTPS_FUNC_REQ_EVENT_SINGLETAP_ON:
					shtps_func_singletap_on(ts);
					ts->cur_msg_p->status = 0;
					break;

				case SHTPS_FUNC_REQ_EVENT_SINGLETAP_OFF:
					shtps_func_singletap_off(ts);
					ts->cur_msg_p->status = 0;
					break;
			#endif /* SHTPS_LPWG_SINGLE_TAP_ENABLE */
...
static char* shtps_func_get_event_name(int event)
{
	switch(event){
		...
		case SHTPS_FUNC_REQ_EVENT_SINGLETAP_ON:		return "SINGLETAP_ON";
		case SHTPS_FUNC_REQ_EVENT_SINGLETAP_OFF:	return "SINGLETAP_OFF";
	}
	return "UnKnown";
}
