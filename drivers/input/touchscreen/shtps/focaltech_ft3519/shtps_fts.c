...
static void shtps_calc_notify(struct shtps_fts *ts, u8 *buf, struct shtps_touch_info *info,
                              u8 *event);
...
const static struct shtps_state_func state_active = {
	...
	.stop_tm = shtps_statef_cmn_error,
	.facetouch_on =
	shtps_statef_active_facetouch_on,
...
const static struct shtps_state_func state_loader = {
	...
	.facetouch_on = shtps_statef_cmn_facetouch_on,
	.facetouch_off =
	shtps_statef_cmn_facetouch_off,
...
const static struct shtps_state_func state_facetouch = {
	...
	.facetouch_off =
	shtps_statef_facetouch_facetouch_off,
...
const static struct shtps_state_func *state_func_tbl[] = {
	...
	static const shtps_deter_suspend_i2c_pending_func_t SHTPS_SUSPEND_PENDING_FUNC_TBL[] = {
	    ...
	    shtps_ioctl_set_high_report_mode_proc, /**<
	                                              SHTPS_DETER_SUSPEND_I2C_PROC_IOCTL_SET_HIGH_REPORT_MODE
	                                            */
	    shtps_ioctl_setlpwg_singletap_proc, /**< SHTPS_DETER_SUSPEND_I2C_PROC_IOCTL_SETLPWG_SINGLETAP */
	};
#endif /* SHTPS_GUARANTEE_I2C_ACCESS_IN_WAKE_ENABLE */
...
void shtps_ioctl_setlpwg_singletap_proc(struct shtps_fts *ts, u8 on)
{
	SHTPS_LOG_FUNC_CALL();

	if(on){
		shtps_func_request_sync(ts, SHTPS_FUNC_REQ_EVENT_SINGLETAP_ON);
	}else{
		shtps_func_request_sync(ts, SHTPS_FUNC_REQ_EVENT_SINGLETAP_OFF);
	}
}
...
int shtps_check_suspend_state(struct shtps_fts *ts, int proc, u8 param)
{
	...
	if(proc == SHTPS_DETER_SUSPEND_I2C_PROC_OPEN || proc == SHTPS_DETER_SUSPEND_I2C_PROC_ENABLE) {
...
static void shtps_exec_suspend_pending_proc_delayed(struct shtps_fts *ts)
{
	...
	SHTPS_LOG_DBG_PRINT("%s() schedule_delayed_work(%d ms)\n", __func__,
	                    SHTPS_SUSPEND_I2C_RESUME_FUNC_DELAY);
...
static void shtps_deter_suspend_i2c_pending_proc_delayed_work_function(struct work_struct *work)
{
	...
	struct shtps_deter_suspend_i2c *dss =
	    container_of(dw, struct shtps_deter_suspend_i2c, pending_proc_work_delay);
...
{
	struct shtps_deter_suspend_i2c *dss =
	    container_of(work, struct shtps_deter_suspend_i2c, pending_proc_work);
	struct shtps_fts *ts = container_of(dss, struct shtps_fts, deter_suspend_i2c);
	int i;
...
static void shtps_facetouch_init(struct shtps_fts *ts)
{
	...
	wake_lock_init(&ts->facetouch.wake_lock, ts->ctrl_dev_p, "shtps_facetouch_wake_lock");
...
static void shtps_event_all_cancel(struct shtps_fts *ts)
{
	...
	for(i = 0; i < fingerMax; i++) {
		if(ts->report_info.fingers[i].state != 0x00) {
			isEvent = 1;
			shtps_report_touch_on(ts, i, SHTPS_TOUCH_CANCEL_COORDINATES_X,
			                      SHTPS_TOUCH_CANCEL_COORDINATES_Y, 1, ts->report_info.fingers[i].wx,
			                      ts->report_info.fingers[i].wy, ts->report_info.fingers[i].z);
		}
	}
	if(isEvent) {
		input_sync(ts->input);
		...
		for(i = 0; i < fingerMax; i++) {
			if(ts->report_info.fingers[i].state != 0x00) {
				isEvent = 1;
				shtps_report_touch_off(ts, i, SHTPS_TOUCH_CANCEL_COORDINATES_X,
				                       SHTPS_TOUCH_CANCEL_COORDINATES_Y, 0, ts->report_info.fingers[i].wx,
				                       ts->report_info.fingers[i].wy, ts->report_info.fingers[i].z);
			}
		}
...
static void shtps_check_facetouch(struct shtps_fts *ts, struct shtps_touch_info *info)
{
	...
	if(SHTPS_FACETOUCH_DETECT_CHECK_MULTITOUCH == 1 &&
	   numOfFingers >= SHTPS_FACETOUCH_DETECT_CHECK_MULTITOUCH_FINGER_NUM) {
...
static void shtps_read_touchevent_infacetouchmode(struct shtps_fts *ts)
{
	...
	*/
	...
	//ts->fw_report_info.finger_num, buf[offset + fingerMax * 5]);
	...
	SHTPS_LOG_FUNC_CALL();
		...
		if(shtps_check_suspend_state(ts, SHTPS_DETER_SUSPEND_I2C_PROC_IRQ, 0) == 0) {
			...
			SHTPS_LOG_DBG_PRINT("[suspend_i2c] irq detect <irq_state:%d><irq_wake_state:%d>\n",
			                    ts->deter_suspend_i2c.suspend_irq_state,
			                    ts->deter_suspend_i2c.suspend_irq_wake_state);
...
void shtps_sleep(struct shtps_fts *ts, int on)
{
	...
	#if defined(SHTPS_LOW_POWER_MODE_ENABLE)
		SHTPS_LOG_DBG_PRINT(
		    "[LPMODE]sleep request recieved. req_state = 0x%02x, continuous_req_state = 0x%02x\n",
	...
	if(SHTPS_LOW_POWER_MODE_CHEK_HW_REV_ENABLE == 0) {
		...
		if(shtps_system_get_hw_type() != SHTPS_HW_TYPE_HANDSET) {
		...
		else {
			{
				...
				if(SHTPS_GET_HW_VERSION_RET_ES_0 == hwrev || SHTPS_GET_HW_VERSION_RET_ES_1 == hwrev) {
					return 0;
				}
			}
		}
		...
		if(type == SHTPS_LPMODE_TYPE_NON_CONTINUOUS) {
			if((ts->lpmode_req_state & req) == 0) {
				...
				SHTPS_LOG_DBG_PRINT("[LPMODE]<ON> type = NON CONTINUOUS, req = %s\n",
					...
					: (req == SHTPS_LPMODE_REQ_LCD_BRIGHT) ? "lcd_bright"
					: "unknown");
			}
		}
		else {
...
void shtps_set_fw_report_rate(struct shtps_fts *ts)
{
	...
	#if defined(SHTPS_DEF_CTRL_FW_REPORT_RATE_LINKED_LCD_BRIGHT_ENABLE)
		if(((ts->lpmode_req_state | ts->lpmode_continuous_req_state) & SHTPS_LPMODE_REQ_LCD_BRIGHT) !=
		   0) {
	...
	if(ts->fw_report_rate_cur == SHTPS_CTRL_FW_REPORT_RATE_PARAM_NORMAL) {
		shtps_report_stflib_kevt(ts, SHTPS_DEF_STFLIB_KEVT_SCANRATE_MODE,
		                         SHTPS_CTRL_FW_REPORT_RATE_STFLIB_KEVT_NORMAL);
	}
	else if(ts->fw_report_rate_cur == SHTPS_CTRL_FW_REPORT_RATE_PARAM_HIGH) {
		shtps_report_stflib_kevt(ts, SHTPS_DEF_STFLIB_KEVT_SCANRATE_MODE,
		                         SHTPS_CTRL_FW_REPORT_RATE_STFLIB_KEVT_HIGH);
	}
	else {
		#if defined(SHTPS_DEF_CTRL_FW_REPORT_RATE_LINKED_LCD_BRIGHT_ENABLE)
			if(ts->fw_report_rate_cur == SHTPS_CTRL_FW_REPORT_RATE_PARAM_LCD_BRIGHT_LOW) {
				shtps_report_stflib_kevt(ts, SHTPS_DEF_STFLIB_KEVT_SCANRATE_MODE,
				                         SHTPS_CTRL_FW_REPORT_RATE_STFLIB_KEVT_LCD_BRIGHT_LOW);
			}
		#endif /* SHTPS_DEF_CTRL_FW_REPORT_RATE_LINKED_LCD_BRIGHT_ENABLE */
	}
}
...
{
	struct delayed_work *dw = container_of(work, struct delayed_work, work);
	struct shtps_lpwg_ctrl *lpwg_p =
	    container_of(dw, struct shtps_lpwg_ctrl, notify_interval_delayed_work);
...
static void shtps_lpwg_notify_interval_start(struct shtps_fts *ts)
{
	...
	schedule_delayed_work(&ts->lpwg.notify_interval_delayed_work,
	                      msecs_to_jiffies(SHTPS_LPWG_MIN_NOTIFY_INTERVAL));
...
static void shtps_lpwg_wakelock_init(struct shtps_fts *ts)
{
	...
	wake_lock_init(&ts->lpwg.wake_lock, ts->ctrl_dev_p, "shtps_lpwg_wake_lock");
	...
	INIT_DELAYED_WORK(&ts->lpwg.notify_interval_delayed_work,
	                  shtps_lpwg_notify_interval_delayed_work_function);

	#if defined(SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE)
		ts->lpwg.notify_wake_lock_state = false;
		wake_lock_init(&ts->lpwg.notify_wake_lock, ts->ctrl_dev_p, "shtps_lpwg_notify_wake_lock");
...
int shtps_is_lpwg_active(struct shtps_fts *ts)
{
	...
	if(ts->lpwg.lpwg_single_tap_req_state != 0) {
		ret |= SHTPS_LPWG_REQ_SINGLE_TAP;
	}
...
int shtps_set_lpwg_sleep_check(struct shtps_fts *ts)
{
	...
	/* --- */
	if((ts->lpwg.lpwg_set_state == SHTPS_LPWG_SET_STATE_OFF) && (on_req != SHTPS_LPWG_REQ_NONE)) {
		#if defined(SHTPS_IRQ_LINKED_WITH_IRQWAKE_ENABLE)
			shtps_irq_enable(ts);
		#else
			shtps_irq_wake_enable(ts);
	...
	/* --- */
	if((on_req & SHTPS_LPWG_REQ_SINGLE_TAP) != 0) {
		SHTPS_LOG_DBG_PRINT("[LPWG] single tap enable\n");
		shtps_fwctl_set_lpwg_single_tap(ts, 1);
		ts->lpwg.lpwg_set_state |= SHTPS_LPWG_SET_STATE_SINGLE_TAP;
	}
	if((off_req & SHTPS_LPWG_REQ_SINGLE_TAP) != 0) {
		SHTPS_LOG_DBG_PRINT("[LPWG] single tap disable\n");
		shtps_fwctl_set_lpwg_single_tap(ts, 0);
		ts->lpwg.lpwg_set_state &= (~SHTPS_LPWG_SET_STATE_SINGLE_TAP);
	}
...
{
	u8 doze_param[3] = {SHTPS_LPWG_DOZE_INTERVAL_DEF, SHTPS_LPWG_DOZE_WAKEUP_THRESHOLD_DEF,
	...
	if(ts->lpwg.lpwg_set_state != SHTPS_LPWG_SET_STATE_OFF) {
		...
		if((ts->lpwg.lpwg_set_state & SHTPS_LPWG_REQ_SINGLE_TAP) != 0) {
			SHTPS_LOG_DBG_PRINT("[LPWG] single tap disable\n");
			shtps_fwctl_set_lpwg_single_tap(ts, 0);
			ts->lpwg.lpwg_set_state &= (~SHTPS_LPWG_REQ_SINGLE_TAP);
		}
...
#if defined(SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE)
void shtps_lpwg_notify_wake_lock(struct shtps_fts *ts, int on)
{
	SHTPS_LOG_FUNC_CALL();
...
static void shtps_lpwg_notify_wake_lock_timer(struct work_struct *work)
{
	...
	struct shtps_fts *ts = container_of(lpwg_p, struct shtps_fts, lpwg);
...
//	SHTPS_LOG_ERR_PRINT("%s\n", saved_command_line);
	for(i = 0; i < SHTPS_MULTI_FW_INFO_SIZE; i++) {
		if(SHTPS_MULTI_FW_INFO_TBL[i].rev_str != NULL) {
			if(strstr(saved_command_line, SHTPS_MULTI_FW_INFO_TBL[i].rev_str) != NULL) {
				ts->multi_fw_type = i;
				break;
			}
		}
...
int shtps_wait_startup(struct shtps_fts *ts)
{
	...
	wait_event_interruptible(ts->wait_start, ts->state_mgr.state == SHTPS_STATE_ACTIVE ||
...
{
	int w = (info->fingers[num].wx >= info->fingers[num].wy) ? info->fingers[num].wx
	                                                         : info->fingers[num].wy;
//	SHTPS_LOG_FUNC_CALL();
...
//			if(ts->fw_report_info.fingers[pointid].state != SHTPS_TOUCH_STATE_NO_TOUCH){
//				ts->fw_report_info.fingers[pointid].x = shtps_fwctl_get_maxXPosition(ts) - ts->fw_report_info.fingers[pointid].x;
//				ts->fw_report_info.fingers[pointid].y = shtps_fwctl_get_maxYPosition(ts) - ts->fw_report_info.fingers[pointid].y;
//			}
		...
		if(ts->fw_report_info.fingers[i].state != SHTPS_TOUCH_STATE_NO_TOUCH) {
			...
			SHTPS_LOG_EVENT(
				DBG_PRINTK("[%s]Touch Info[%d] x=%d, y=%d, wx=%d, wy=%d, z=%d\n",
					...
					: ts->fw_report_info.fingers[i].state == SHTPS_TOUCH_STATE_UNKNOWN ? "Unknown"
					: "Other",
				...
				if(ts->fw_report_info.fingers[i].z == 0) {
					...
					info->fingers[i].z = 1;
					SHTPS_LOG_EVENT(DBG_PRINTK("[warn][TouchEvent][%d] z=0 detected. change to z=1\n", i););
				}
			...
			SHTPS_LOG_EVENT(DBG_PRINTK("[NoTouch]Touch Info[%d] x=%d, y=%d, wx=%d, wy=%d, z=%d\n", i,
			                           ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y,
			                           ts->fw_report_info.fingers[i].wx, ts->fw_report_info.fingers[i].wy,
			                           ts->fw_report_info.fingers[i].z););
		}
	}
	...
	for(i = 0; i < fingerMax; i++) {
		if(info->fingers[i].state != SHTPS_TOUCH_STATE_NO_TOUCH) {
			if(ts->report_info.fingers[i].state != SHTPS_TOUCH_STATE_NO_TOUCH) {
				diff_x =
				    shtps_get_diff(info->fingers[i].x, ts->report_info.fingers[i].x, SHTPS_POS_SCALE_X(ts));
				diff_y =
				    shtps_get_diff(info->fingers[i].y, ts->report_info.fingers[i].y, SHTPS_POS_SCALE_Y(ts));
...
void shtps_report_touch_on(struct shtps_fts *ts, int finger, int x, int y, int w, int wx, int wy,
                           int z)
{
	int lcd_x;
	int lcd_y;
	...
	SHTPS_LOG_EVENT(
	    DBG_PRINTK("[finger]Notify event[%d] touch=100(%d), x=%d(%d), y=%d(%d) w=%d(%d,%d)\n", finger,
	               z, lcd_x, x, lcd_y, y, w, wx, wy););
	_log_msg_sync(LOGMSG_ID__EVENT_NOTIFY, "%d|100|%d|%d|%d|%d|%d|%d|%d|%d", finger, lcd_x, x, lcd_y,
	              y, w, wx, wy, z);
...
void shtps_report_touch_off(struct shtps_fts *ts, int finger, int x, int y, int w, int wx, int wy,
                            int z)
{
	//	SHTPS_LOG_FUNC_CALL();
	input_mt_slot(ts->input, finger);
	...
	SHTPS_LOG_EVENT(
	    DBG_PRINTK("[finger]Notify event[%d] touch=0(%d), x=%d(%d), y=%d(%d) w=%d(%d,%d)\n", finger,
	               z, (x * SHTPS_POS_SCALE_X(ts) / 10000), x, (y * SHTPS_POS_SCALE_Y(ts) / 10000), y,
	               w, wx, wy););
	_log_msg_sync(LOGMSG_ID__EVENT_NOTIFY, "%d|0|%d|%d|%d|%d|%d|%d|%d|%d", finger,
	              (x * SHTPS_POS_SCALE_X(ts) / 10000), x, (y * SHTPS_POS_SCALE_Y(ts) / 10000), y, w,
	              wx, wy, z);
...
#if defined(SHTPS_GLOVE_DETECT_ENABLE)
void shtps_report_touch_glove_on(struct shtps_fts *ts, int finger, int x, int y, int w, int wx,
                                 int wy, int z)
{
	int lcd_x;
	int lcd_y;
...
void shtps_report_touch_glove_off(struct shtps_fts *ts, int finger, int x, int y, int w, int wx,
                                  int wy, int z)
{
	//	SHTPS_LOG_FUNC_CALL();
	input_mt_slot(ts->input, finger);
...
void shtps_event_report(struct shtps_fts *ts, struct shtps_touch_info *info, u8 event)
{
	...
	for(i = 0; i < fingerMax; i++) {
		...
		if(info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER) {
			shtps_report_touch_on(ts, i, info->fingers[i].x, info->fingers[i].y,
			                      shtps_get_fingerwidth(ts, i, info), info->fingers[i].wx,
			                      info->fingers[i].wy, info->fingers[i].z);

		#if defined(SHTPS_GLOVE_DETECT_ENABLE)
			}
			else if(info->fingers[i].state == SHTPS_TOUCH_STATE_GLOVE) {
				shtps_report_touch_glove_on(ts, i, info->fingers[i].x, info->fingers[i].y,
				                            shtps_get_fingerwidth(ts, i, info), info->fingers[i].wx,
				                            info->fingers[i].wy, info->fingers[i].z);
		#endif /* SHTPS_GLOVE_DETECT_ENABLE */
				...
				shtps_report_touch_glove_off(ts, i, info->fingers[i].x, info->fingers[i].y,
				                             shtps_get_fingerwidth(ts, i, info), info->fingers[i].wx,
				                             info->fingers[i].wy, info->fingers[i].z);
		#endif /* SHTPS_GLOVE_DETECT_ENABLE */
		}
		else if(ts->report_info.fingers[i].state != SHTPS_TOUCH_STATE_NO_TOUCH) {
			shtps_report_touch_off(ts, i, info->fingers[i].x, info->fingers[i].y,
			                       shtps_get_fingerwidth(ts, i, info), info->fingers[i].wx,
			                       info->fingers[i].wy, info->fingers[i].z);
		}
	}
...
void shtps_event_force_touchup(struct shtps_fts *ts)
{
	...
	shtps_filter_force_touchup(ts);
		...
		for(i = 0; i < fingerMax; i++) {
			if(ts->report_info.fingers[i].state != 0x00) {
				isEvent = 1;
				shtps_report_touch_on(
				    ts, i, SHTPS_TOUCH_CANCEL_COORDINATES_X, SHTPS_TOUCH_CANCEL_COORDINATES_Y,
				    shtps_get_fingerwidth(ts, i, &ts->report_info), ts->report_info.fingers[i].wx,
				    ts->report_info.fingers[i].wy, ts->report_info.fingers[i].z);
			}
		}
		if(isEvent) {
	...
	for(i = 0; i < fingerMax; i++) {
		if(ts->report_info.fingers[i].state != 0x00) {
			isEvent = 1;
			shtps_report_touch_off(ts, i, ts->report_info.fingers[i].x, ts->report_info.fingers[i].y, 0,
			                       0, 0, 0);
		}
	}
	if(isEvent) {
...
void shtps_report_stflib_kevt(struct shtps_fts *ts, int event, int value)
{
	...
	report_val = ((event & 0xFF) << 24) | (value & 0xFFFFFF);
...
void shtps_send_dummykey_event_delayed_work_function(struct work_struct *work)
{
	struct delayed_work *dw = container_of(work, struct delayed_work, work);
	struct shtps_fts *ts = 
			container_of(dw, struct shtps_fts,
					send_dummykey_event_delayed_work);
	int i;
	...
	for(i = 0; i < SHTPS_SEND_DUMMYKEY_EVENT_INFO_MAX; i++) {
		if(	(ts->send_dummykey_event_info[i].expire_jiffies != 0) &&
			(ts->send_dummykey_event_info[i].expire_jiffies < current_jiffies)) {

			shtps_report_stflib_kevt(ts, SHTPS_DEF_STFLIB_KEVT_TIMEOUT, ts->send_dummykey_event_info[i].id);
...
#if defined(SHTPS_FTB_DELAYED_START_ENABLE)
void shtps_set_ftb_filter(struct shtps_fts *ts, u8 on)
{
	int rc;

	SHTPS_LOG_FUNC_CALL();

	if (ts->ftb_filter_enabled) {
		SHTPS_LOG_ERR_PRINT("FTB filter has been already enabled\n");
		return;
	}

	// skip if touches exist
	if (ts->touch_state.numOfFingers > 0) {
		SHTPS_LOG_ERR_PRINT("enabling FTB filter was skipped\n");
		return;
	}

	rc = shtps_fwctl_set_ftb_filter(ts, on);
	if (rc == 0) {
		SHTPS_LOG_DBG_PRINT("FTB filter enabled\n");
		ts->ftb_filter_enabled = true;
	} else {
		SHTPS_LOG_ERR_PRINT("failed to enable FTB filter\n");
		ts->ftb_filter_enabled = false;
	}
}

void shtps_ftb_delayed_work_function(struct work_struct *work)
{
	struct delayed_work *dw = container_of(work, struct delayed_work, work);
	struct shtps_fts *ts = container_of(dw, struct shtps_fts, ftb_delayed_work);

	SHTPS_LOG_FUNC_CALL();

	shtps_mutex_lock_ctrl();
	shtps_set_ftb_filter(ts, true);
	ts->ftb_work_completed = true;
	shtps_mutex_unlock_ctrl();
}
#endif /* SHTPS_FTB_DELAYED_START_ENABLE */
...
static void shtps_notify_double_tap_event(struct shtps_fts *ts)
{
	...
	sysfs_notify(ts->lpwg.shtpsif_kobj, NULL, ts->lpwg.gesture_notify_attr_name);
#else
...
static void shtps_notify_single_tap_event(struct shtps_fts *ts)
{
	SHTPS_LOG_FUNC_CALL();
	...
	sysfs_notify(ts->lpwg.shtpsif_kobj, NULL, ts->lpwg.gesture_notify_attr_name);
		...
		input_report_key(ts->input_key, KEY_SINGLETAP, 1);
		input_sync(ts->input_key);

		input_report_key(ts->input_key, KEY_SINGLETAP, 0);
		input_sync(ts->input_key);
...
void shtps_read_tmdata(struct shtps_fts *ts, u8 mode)
{
	...
	if(mode == SHTPS_TMMODE_FRAMELINE) {
	...
	else if(mode == SHTPS_TMMODE_RAW_SELF_DATA) {
		shtps_mutex_lock_ctrl();
		rc = shtps_fwctl_cmd_tm_raw_self_data(ts, ts->diag.tm_mode);
		shtps_mutex_unlock_ctrl();
		if(rc == 0) {
			if(shtps_tm_wait_attn(ts) != 0)
				goto tm_read_cancel;
		...
		shtps_mutex_lock_ctrl();
		rc = shtps_fwctl_get_tm_raw_self_data(ts, ts->diag.tm_mode, ts->diag.tm_data);
		shtps_mutex_unlock_ctrl();
		if(rc < 0) {
			goto tm_read_cancel;
		}
	}
	else if(mode == SHTPS_TMMODE_RAW_SELF_DATA_WP) {
		shtps_mutex_lock_ctrl();
		rc = shtps_fwctl_cmd_tm_raw_self_data_wp(ts, ts->diag.tm_mode);
		shtps_mutex_unlock_ctrl();
		if(rc == 0) {
			if(shtps_tm_wait_attn(ts) != 0)
				goto tm_read_cancel;
		...
		shtps_mutex_lock_ctrl();
		rc = shtps_fwctl_get_tm_raw_self_data_wp(ts, ts->diag.tm_mode, ts->diag.tm_data);
		shtps_mutex_unlock_ctrl();
		if(rc < 0) {
			goto tm_read_cancel;
		}
...
static int shtps_irq_resuest(struct shtps_fts *ts)
{
	...
	_log_msg_sync(LOGMSG_ID__IRQ_REQUEST, "%d", ts->irq_mgr.irq);
		...
		rc = request_threaded_irq(ts->irq_mgr.irq, shtps_irq_handler, shtps_irq,
		                          IRQF_TRIGGER_LOW | IRQF_ONESHOT, SH_TOUCH_DEVNAME, ts);
	#else
		rc = request_threaded_irq(ts->irq_mgr.irq, shtps_irq_handler, shtps_irq,
		                          IRQF_TRIGGER_FALLING | IRQF_ONESHOT, SH_TOUCH_DEVNAME, ts);
...
static void shtps_read_touchevent_insleep(struct shtps_fts *ts, int state)
{
	...
	shtps_fwctl_irqclear_get_irqfactor(ts, &irq_sts);
	...
	#if defined(SHTPS_HOST_LPWG_MODE_ENABLE)
		{
			...
			if(shtps_check_host_lpwg_enable() == 1) {
				...
				info.fingers[0].x =
				    (shtps_fwctl_get_finger_pos_x(ts, fingerInfo) * SHTPS_POS_SCALE_X(ts) / 10000);
				info.fingers[0].y =
				    (shtps_fwctl_get_finger_pos_y(ts, fingerInfo) * SHTPS_POS_SCALE_Y(ts) / 10000);

				SHTPS_LOG_DBG_PRINT("[LPWG] TouchInfo <state: %d><x: %d><y: %d>\n", info.fingers[0].state,
				                    info.fingers[0].x, info.fingers[0].y);
				...
				if((info.fingers[0].state == SHTPS_TOUCH_STATE_FINGER) &&
				   (ts->lpwg.pre_info.fingers[0].state == SHTPS_TOUCH_STATE_NO_TOUCH)) {
				...
				else if((info.fingers[0].state == SHTPS_TOUCH_STATE_FINGER) &&
				        (ts->lpwg.pre_info.fingers[0].state == SHTPS_TOUCH_STATE_FINGER)) {
					if(time_after(jiffies, ts->lpwg.swipe_check_time) == 0) {
						...
						if((diff_x > SHTPS_LPWG_SWIPE_DIST_THRESHOLD) ||
						   (diff_y > SHTPS_LPWG_SWIPE_DIST_THRESHOLD)) {
				...
				shtps_fwctl_get_gesturetype(ts, &val, &coordinate_x, &coordinate_y);
			}
		}
	#else
		shtps_fwctl_get_gesturetype(ts, &val, &coordinate_x, &coordinate_y);
	...
	if(val == SHTPS_GESTURE_TYPE_ONE_FINGER_SWIPE) {
		...
		if(ts->lpwg.notify_enable != 0) {
			ts->lpwg.gesture_type = val;
			ts->lpwg.gesture_coordinates_x = coordinate_x;
			ts->lpwg.gesture_coordinates_y = coordinate_y;
	...
	else if(val == SHTPS_GESTURE_TYPE_ONE_FINGER_SINGLE_TAP) {
		if(ts->lpwg.notify_enable != 0) {
			ts->lpwg.gesture_type = val;
			ts->lpwg.gesture_coordinates_x = coordinate_x;
			ts->lpwg.gesture_coordinates_y = coordinate_y;
			...
			#if defined(SHTPS_PROXIMITY_SUPPORT_ENABLE)
			{
				if(SHTPS_LPWG_PROXIMITY_SUPPORT_ENABLE != 0) {
					if(shtps_proximity_state_check(ts) == 0) {
						if(SHTPS_LPWG_PROXIMITY_CHECK_ASYNC_ENABLE != 0) {
							shtps_notify_single_tap_event(ts);
							SHTPS_LOG_DBG_PRINT("[LPWG] proximity check async req\n");
							shtps_lpwg_proximity_check_start(ts);
						}
						else {
	...
	switch(state) {
		...
		default:
			...
			if (event == SHTPS_EVENT_TU) {
				if (ts->ftb_work_completed &&
				    !ts->ftb_filter_enabled) {
					shtps_set_ftb_filter(ts, true);
				}
			}
			#endif /* SHTPS_FTB_DELAYED_START_ENABLE */
...
int request_event(struct shtps_fts *ts, int event, int param)
{
	...
	SHTPS_LOG_DBG_PRINT("event %s(%d) in state %s(%d)\n", shtps_get_event_str(event), event,
	                    shtps_get_state_str(ts->state_mgr.state), ts->state_mgr.state);
...
static int state_change(struct shtps_fts *ts, int state)
{
	...
	SHTPS_LOG_DBG_PRINT("state %s(%d) -> %s(%d)\n", shtps_get_state_str(ts->state_mgr.state),
	                    ts->state_mgr.state, shtps_get_state_str(state), state);
...
{
	SHTPS_LOG_FUNC_CALL();
	if((param == SHTPS_STATE_IDLE) || (param == SHTPS_STATE_SLEEP) ||
	   (param == SHTPS_STATE_BOOTLOADER)) {
		shtps_init_param(ts);
		#if defined(SHTPS_IRQ_LINKED_WITH_IRQWAKE_ENABLE)
			shtps_irq_enable(ts);
		#else
	...
	#if defined(SHTPS_FTB_DELAYED_START_ENABLE)
		ts->ftb_work_completed = false;
		ts->ftb_filter_enabled = false;
		if (SHTPS_FTB_START_DELAY != 0) {
			schedule_delayed_work(&ts->ftb_delayed_work, msecs_to_jiffies(SHTPS_FTB_START_DELAY));
		}
	#endif /* SHTPS_FTB_DELAYED_START_ENABLE */
...
static int shtps_statef_active_stop(struct shtps_fts *ts, int param)
{
	SHTPS_LOG_FUNC_CALL();
	#if defined(SHTPS_FTB_DELAYED_START_ENABLE)
		cancel_delayed_work_sync(&ts->ftb_delayed_work);
	#endif /* SHTPS_FTB_DELAYED_START_ENABLE */
...
static int shtps_statef_sleep_enter(struct shtps_fts *ts, int param)
{
	...
	shtps_report_stflib_kevt(ts, SHTPS_DEF_STFLIB_KEVT_CLEAR_REQUEST,
	                         SHTPS_DEF_STFLIB_KEVT_CLEAR_REQUEST_SLEEP);
...
static void shtps_fts_close(struct input_dev *dev)
{
	...
	if(gShtps_fts != NULL) {
		...
		/* Process synchronously when removing */
...
static int shtps_init_internal_variables(struct shtps_fts *ts)
{
	...
	init_waitqueue_head(&ts->wait_active);

	#if defined(SHTPS_GUARANTEE_I2C_ACCESS_IN_WAKE_ENABLE)
		ts->deter_suspend_i2c.wake_lock_state = 0;
		memset(&ts->deter_suspend_i2c, 0, sizeof(ts->deter_suspend_i2c));
		wake_lock_init(&ts->deter_suspend_i2c.wake_lock, ts->ctrl_dev_p, "shtps_resume_wake_lock");
		ts->deter_suspend_i2c.pm_qos_lock_idle.type = PM_QOS_REQ_ALL_CORES;
		pm_qos_add_request(&ts->deter_suspend_i2c.pm_qos_lock_idle, PM_QOS_CPU_DMA_LATENCY,
		                   PM_QOS_DEFAULT_VALUE);
		INIT_WORK(&ts->deter_suspend_i2c.pending_proc_work,
		          shtps_deter_suspend_i2c_pending_proc_work_function);
		#ifdef SHTPS_DEVELOP_MODE_ENABLE
			INIT_DELAYED_WORK(&ts->deter_suspend_i2c.pending_proc_work_delay,
			                  shtps_deter_suspend_i2c_pending_proc_delayed_work_function);
		#endif /* SHTPS_DEVELOP_MODE_ENABLE */
	#endif /* SHTPS_GUARANTEE_I2C_ACCESS_IN_WAKE_ENABLE */
	...
	shtps_performance_check_init();
		...
		INIT_DELAYED_WORK(&ts->touchkey_inproxymity_delayed_work,
		                  shtps_touchkey_inproxymity_delayed_work_function);
	...
	#if defined(SHTPS_PROXIMITY_SUPPORT_ENABLE)
		wake_lock_init(&ts->wake_lock_proximity, ts->ctrl_dev_p, "shtps_wake_lock_proximity");
		...
		INIT_DELAYED_WORK(&ts->proximity_check_delayed_work,
		                  shtps_lpwg_proximity_check_delayed_work_function);
	#endif /* SHTPS_PROXIMITY_SUPPORT_ENABLE */

	#if defined(SHTPS_SEND_DUMMYKEY_EVENT_ENABLE)
		INIT_DELAYED_WORK(&ts->send_dummykey_event_delayed_work, shtps_send_dummykey_event_delayed_work_function);
	#endif /* SHTPS_SEND_DUMMYKEY_EVENT_ENABLE */

	#if defined(SHTPS_FTB_DELAYED_START_ENABLE)
		ts->ftb_work_completed = false;
		ts->ftb_filter_enabled = false;
		INIT_DELAYED_WORK(&ts->ftb_delayed_work, shtps_ftb_delayed_work_function);
	#endif /* SHTPS_FTB_DELAYED_START_ENABLE */
...
fail_init:
	#if defined(SHTPS_GUARANTEE_I2C_ACCESS_IN_WAKE_ENABLE)
		pm_qos_remove_request(&ts->deter_suspend_i2c.pm_qos_lock_idle);
		wake_lock_destroy(&ts->deter_suspend_i2c.wake_lock);
	#endif /* SHTPS_GUARANTEE_I2C_ACCESS_IN_WAKE_ENABLE */
...
static int shtps_init_inputdev(struct shtps_fts *ts, struct device *ctrl_dev_p, char *modalias)
{
	...
	#endif /* SHTPS_NOTIFY_TOUCH_MINOR_ENABLE */
	input_set_abs_params(ts->input, ABS_MT_POSITION_X, 0, CONFIG_SHTPS_FOCALTECH_LCD_SIZE_X - 1, 0,
	                     0);
	input_set_abs_params(ts->input, ABS_MT_POSITION_Y, 0, CONFIG_SHTPS_FOCALTECH_LCD_SIZE_Y - 1, 0,
	                     0);
...
int shtps_drm_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (evdata && evdata->data) {
		if ((event == DRM_PANEL_EVENT_BLANK) || (event == DRM_PANEL_EARLY_EVENT_BLANK)) {
			...
			SHTPS_LOG_DBG_PRINT("%s() start. event=%d blank=%d\n", __func__, event, *((int*)evdata->data));

			if(	((event == DRM_PANEL_EARLY_EVENT_BLANK) && (blank == DRM_PANEL_BLANK_POWERDOWN)) ||
				((event == DRM_PANEL_EARLY_EVENT_BLANK) && (blank == DRM_PANEL_BLANK_LP)) ){
				#if defined(SHTPS_GUARANTEE_I2C_ACCESS_IN_WAKE_ENABLE)
					if(shtps_check_suspend_state(ts, SHTPS_DETER_SUSPEND_I2C_PROC_SETSLEEP, 1) == 0){
				...
				#if defined(SHTPS_GUARANTEE_I2C_ACCESS_IN_WAKE_ENABLE)
					if(shtps_check_suspend_state(ts, SHTPS_DETER_SUSPEND_I2C_PROC_SETSLEEP, 0) == 0){
				...
				wake_up_interruptible(&ts->wait_active);
			}

			SHTPS_LOG_DBG_PRINT("%s() end. event=%d blank=%d\n", __func__, event, *((int*)evdata->data));
		}
	}
...
int shtps_fts_core_probe(struct device *ctrl_dev_p, struct shtps_ctrl_functbl *func_p, void *ctrl_p,
                         char *modalias, int gpio_irq)
{
	...
	gShtps_fts = ts;
		...
		if(!gpio_is_valid(ts->tpin)) {
			...
			if(IS_ERR_OR_NULL(ts->pinctrl)) {
				...
				ts->tpin_state_active = pinctrl_lookup_state(ts->pinctrl, "touchscreen_test_mode_pull_up");
				if(IS_ERR_OR_NULL(ts->tpin_state_active)) {
					ts->tpin = 0;
				}

				ts->tpin_state_suspend = pinctrl_lookup_state(ts->pinctrl, "touchscreen_test_mode_pull_down");
				if(IS_ERR_OR_NULL(ts->tpin_state_active)) {
					ts->tpin = 0;
				}
			}
	...
	#ifdef CONFIG_OF
	{
		struct shtps_device_ctrl_info device_ctrl_info = {0};

		device_ctrl_info.dev			= ctrl_dev_p;
		device_ctrl_info.rst_pin		= ts->rst_pin;
		device_ctrl_info.pinctrl		= ts->pinctrl;
		device_ctrl_info.tp_int			= of_get_named_gpio(ctrl_dev_p->of_node, "shtps_fts,tp_int", 0);
		device_ctrl_info.int_active		= pinctrl_lookup_state(ts->pinctrl, "shtps_int_active");
		device_ctrl_info.int_standby	= pinctrl_lookup_state(ts->pinctrl, "shtps_int_standby");
		device_ctrl_info.reset_active	= pinctrl_lookup_state(ts->pinctrl, "shtps_reset_active");
		device_ctrl_info.reset_standby	= pinctrl_lookup_state(ts->pinctrl, "shtps_reset_standby");

		result = shtps_device_setup(&device_ctrl_info);
		if(result) {
	...
	#ifdef CONFIG_DRM_PANEL
	{
		...
		count = of_count_phandle_with_args(ctrl_dev_p->of_node, "panel", NULL);
//		SHTPS_LOG_ERR_PRINT("panel count=%d\n", count);
		if (count > 0) {
			for (i = 0; i < count; i++) {
				node = of_parse_phandle(ctrl_dev_p->of_node, "panel", i);
		...
		ts->drm_notifier.notifier_call = shtps_drm_notifier_callback;
		if (ts->active_panel) {
			result = drm_panel_notifier_register(ts->active_panel,
					&ts->drm_notifier);
			if (result < 0) {
				SHTPS_LOG_ERR_PRINT("Failed to register fb notifier client\n");
			}
		}
		else {
			SHTPS_LOG_ERR_PRINT("Failed to get active_panel\n");
		}
	}
	#else
	...
	#if defined(SHTPS_PANEL_KIND_DETECT_ENABLE)
	{
		...
		touchpanel_module_data.dev 							= ctrl_dev_p;
		touchpanel_module_data.pwr_reg_name 				= SHTPS_VREG_ID_VDDH;
		touchpanel_module_data.bus_reg_name 				= SHTPS_VREG_ID_VBUS;
		touchpanel_module_data.pinctrl 						= ts->pinctrl;
		touchpanel_module_data.panel_kind_state_pullup 		= pinctrl_lookup_state(ts->pinctrl, "panel_kind_pull_up");
		touchpanel_module_data.panel_kind_state_pulldown	= pinctrl_lookup_state(ts->pinctrl, "panel_kind_pull_down");
		touchpanel_module_data.panel_kind_gpio 				= of_get_named_gpio(ctrl_dev_p->of_node, "shtps_fts,panel_kind_gpio", 0);

		shtps_touchpanel_module_init();
		shtps_touchpanel_module_set_data(ctrl_dev_p, &touchpanel_module_data);
	}
	#endif /* SHTPS_PANEL_KIND_DETECT_ENABLE */
...
fail_init_shtpsif:
	#ifdef CONFIG_DRM_PANEL
		if (ts->active_panel) {
			drm_panel_notifier_unregister(ts->active_panel, &ts->drm_notifier);
		}
	#else
		#ifdef CONFIG_DRM_MSM
...
int shtps_fts_core_remove(struct shtps_fts *ts, struct device *ctrl_dev_p)
{
	...
	#if defined(SHTPS_PANEL_KIND_DETECT_ENABLE)
		shtps_touchpanel_module_clear_data(ctrl_dev_p);
		shtps_touchpanel_module_exit();
	#endif /* SHTPS_PANEL_KIND_DETECT_ENABLE */
	#ifndef CONFIG_OF
	...
	if(ts) {
		if(on == 1) {
			...
			if(on) {
				...
				shtps_func_request_async(ts, SHTPS_FUNC_REQ_EVENT_LCD_ON);
			}
		#endif /* SHTPS_GUARANTEE_I2C_ACCESS_IN_WAKE_ENABLE */
	}
