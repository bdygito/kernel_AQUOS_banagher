...
#define SHTPS_SEND_DUMMYKEY_EVENT_ENABLE
#if defined(SHTPS_SEND_DUMMYKEY_EVENT_ENABLE)
	#define SHTPS_SEND_DUMMYKEY_EVENT_INFO_MAX	32
...
struct shtps_lpwg_ctrl { //**********	SHTPS_LPWG_MODE_ENABLE
	...
	const u8 *gesture_notify_attr_name;
	u8 gesture_type;
...
enum { //**********	SHTPS_GUARANTEE_I2C_ACCESS_IN_WAKE_ENABLE
	...
	SHTPS_DETER_SUSPEND_I2C_PROC_IOCTL_SETLPWG_SINGLETAP,
...
#if defined(SHTPS_SEND_DUMMYKEY_EVENT_ENABLE)
struct shtps_send_dummykey_event_info{
	unsigned int id;
...
struct shtps_fts {
	...
	struct shtps_offset_info offset;
		...
		int boot_fwupdate_request;
	#endif /* SHTPS_BOOT_FWUPDATE_ENABLE */

	#if defined(SHTPS_ALWAYS_ON_DISPLAY_ENABLE)
		bool first_resume_done;
	#endif /* SHTPS_ALWAYS_ON_DISPLAY_ENABLE */

	#if defined(SHTPS_SEND_DUMMYKEY_EVENT_ENABLE)
		struct shtps_send_dummykey_event_info send_dummykey_event_info[SHTPS_SEND_DUMMYKEY_EVENT_INFO_MAX];
		struct delayed_work send_dummykey_event_delayed_work;
	#endif /* SHTPS_SEND_DUMMYKEY_EVENT_ENABLE */

	#if defined(SHTPS_REJECT_ABANDONED_TOUCH_MALFUNCTIONS_ENABLE)
		int reject_abandoned_touch_malfunctions_check[SHTPS_FINGER_MAX];
		struct timeval reject_abandoned_touch_malfunctions_td_timeval[SHTPS_FINGER_MAX];
	#endif /* SHTPS_REJECT_ABANDONED_TOUCH_MALFUNCTIONS_ENABLE */

	#if defined(SHTPS_FTB_DELAYED_START_ENABLE)
		struct delayed_work ftb_delayed_work;
		bool ftb_work_completed;
		bool ftb_filter_enabled;
...
enum { SHTPS_STARTUP_SUCCESS, SHTPS_STARTUP_FAILED };
...
enum {
	...
	SHTPS_IRQ_ALL = (SHTPS_IRQ_FLASH | SHTPS_IRQ_STATE | SHTPS_IRQ_ABS | SHTPS_IRQ_ANALOG |
	                 SHTPS_IRQ_BUTTON | SHTPS_IRQ_SENSOR),
};
...
enum {
	...
	SHTPS_FUNC_REQ_EVENT_SINGLETAP_ON,
	SHTPS_FUNC_REQ_EVENT_SINGLETAP_OFF,
};
...
enum {
	...
	SHTPS_LPWG_DETECT_GESTURE_TYPE_SINGLE_TAP = 0x04,
};
...
enum {
	...
	SHTPS_LPWG_REQ_SINGLE_TAP = 0x04,
};
...
enum {
	...
	SHTPS_LPWG_SET_STATE_SINGLE_TAP = SHTPS_LPWG_REQ_SINGLE_TAP,
};
