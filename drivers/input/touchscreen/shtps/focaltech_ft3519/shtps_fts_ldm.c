...
#if defined(SHTPS_PANEL_KIND_DETECT_ENABLE)
	#include "shtps_touchpanel_module.h"
#endif /* SHTPS_PANEL_KIND_DETECT_ENABLE */
 ...
 */
#if defined(SHTPS_CREATE_KOBJ_ENABLE)
static ssize_t store_hover_enable(struct kobject *kobj, struct kobj_attribute *attr,
...
static ssize_t store_grip_state(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
                                size_t count)
{
	int on;
	sscanf(buf, "%d", &on);
...
static ssize_t store_cover_state(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
                                 size_t count)
{
	int on;
	sscanf(buf, "%d", &on);
...
static ssize_t show_suspend_i2c_test_state(struct kobject *kobj, struct kobj_attribute *attr,
                                           char *buf)
{
	return snprintf(buf, PAGE_SIZE,
	                "suspend i2c test cmd\n"
	                "  1: set suspend state\n"
	                "  2: clr suspend state\n"
	                "  3: [test request] grip\n"
...
static ssize_t store_suspend_i2c_test_state(struct kobject *kobj, struct kobj_attribute *attr,
                                            const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
	int test_no;
	...
	SHTPS_LOG_DBG_PRINT("test_no=%d param=%d\n", test_no, param);
	...
	switch(test_no) {
		...
		case 5:
...
#if defined(SHTPS_ALWAYS_ON_DISPLAY_ENABLE)
				{
					...
					if(param == 0){
						...
						shtps_drm_notifier_callback(NULL, DRM_PANEL_EVENT_DPMS, &evdata);
					}
					else if(param == 1){
						...
						blank = param % 10;
						shtps_drm_notifier_callback(NULL, (param / 10) % 10, &evdata);
					}
				}
#else
				{
					...
					if(param == 0){
						...
						shtps_drm_notifier_callback(NULL, DRM_PANEL_EVENT_BLANK, &evdata);
					}
					else if(param == 2){
						...
						shtps_drm_notifier_callback(NULL, DRM_PANEL_EARLY_EVENT_BLANK, &evdata);
					}
				}
		...
		#if defined(SHTPS_LPWG_MODE_ENABLE)
			case 6:
				shtps_ioctl_lpwg_enable(ts, param);
				break;
		#endif /* SHTPS_LPWG_MODE_ENABLE */
		case 7:
			shtps_ioctl_hover_enable(ts, param);
			break;
		case 8:
			shtps_ioctl_set_low_power_mode(ts, param);
			break;
		case 9:
			shtps_ioctl_set_continuous_low_power_mode(ts, param);
			break;
		case 10:
			shtps_ioctl_set_lcd_bright_low_power_mode(ts, param);
			break;
		case 11:
		...
		#if defined(SHTPS_COVER_ENABLE)
			case 14:
				msm_tps_set_cover_state(param);
				break;
		#endif /* SHTPS_COVER_ENABLE */
		#if defined(SHTPS_GLOVE_DETECT_ENABLE)
			case 15:
				shtps_ioctl_set_glove_mode(ts, param);
				break;
		#endif /* SHTPS_GLOVE_DETECT_ENABLE */
		#if defined(SHTPS_CTRL_FW_REPORT_RATE)
			case 16:
				shtps_ioctl_set_high_report_mode(ts, param);
				break;
		#endif /* #if defined(SHTPS_CTRL_FW_REPORT_RATE) */
		default:
			break;
...
static int shtps_touch_info_copy_to_user(struct shtps_touch_info *krnl, unsigned long arg,
                                         int isCompat)
{
#ifdef CONFIG_COMPAT
	if(isCompat == 1) {
		int i;
		int err = 0;
		struct shtps_compat_touch_info *usr = (struct shtps_compat_touch_info __user *)arg;
...
#endif /* CONFIG_COMPAT */
	{
		int i;
		int err = 0;
		struct shtps_touch_info *usr = (struct shtps_touch_info __user *)arg;
...
static int shtps_io_ctl_param_copy_from_user(struct shtps_ioctl_param *krnl, unsigned long arg,
                                             int isCompat)
{
#ifdef CONFIG_COMPAT
	if(isCompat == 1) {
		...
		struct shtps_compat_ioctl_param *usr = (struct shtps_compat_ioctl_param __user *)arg;
...
#endif /* CONFIG_COMPAT */
	{
		...
		struct shtps_ioctl_param *usr = (struct shtps_ioctl_param __user *)arg;
...
static int shtps_fw_upgrade_copy_from_user(struct shtps_ioctl_param *krnl, unsigned long arg,
                                           int isCompat)
{
#ifdef CONFIG_COMPAT
	if(isCompat == 1) {
...
static int shtps_io_ctl_param_copy_to_user(struct shtps_ioctl_param *krnl, unsigned long arg,
                                           int isCompat)
{
#ifdef CONFIG_COMPAT
	if(isCompat == 1) {
...
static int shtps_ioctl_reg_write_block(struct shtps_fts *ts, unsigned long arg, int isCompat)
{
	...
	if(M_WRITE_PACKET_FUNC(ts->fwctl_p, param.data[0] << 0x08 | param.data[1], &param.data[2],
	                       (param.size - 2))) {
		shtps_io_ctl_param_release(&param);
		return -EFAULT;
	}
...
static int shtps_ioctl_poll_facetouchoff(struct shtps_fts *ts)
{
		...
		rc = wait_event_interruptible(ts->facetouch.wait_off, (ts->facetouch.detect == 1) ||
		                                                          (ts->facetouch.mode == 0) ||
		                                                          (ts->facetouch.wake_sig == 1));

		_log_msg_recv(LOGMSG_ID__DETECT_FACETOUCH, "%d|%d|%d", ts->facetouch.detect, ts->facetouch.mode,
		              ts->facetouch.wake_sig);
...
static int shtps_ioctl_set_lcd_bright_low_power_mode(struct shtps_fts *ts, unsigned long arg)
{
	...
	#if defined(SHTPS_LOW_POWER_MODE_ENABLE)
		#if defined(SHTPS_GUARANTEE_I2C_ACCESS_IN_WAKE_ENABLE)
			if(shtps_check_suspend_state(ts, SHTPS_DETER_SUSPEND_I2C_PROC_IOCTL_SETLCDBRIGHTLPMODE,
			                             (u8)arg) == 0) {
...
static int shtps_ioctl_set_low_reportrate_mode(struct shtps_fts *ts, unsigned long arg)
{
	...
	if(SHTPS_LOW_REPORTRATE_MODE_ENABLE == 1) {
		...
		if(ts->low_report_rate_mode_state == 0) {
			shtps_report_stflib_kevt(ts, SHTPS_DEF_STFLIB_KEVT_SCANRATE_MODE,
			                         SHTPS_DEF_STFLIB_KEVT_SCANRATE_MODE_NORMAL);
		}
		else {
			shtps_report_stflib_kevt(ts, SHTPS_DEF_STFLIB_KEVT_SCANRATE_MODE,
			                         SHTPS_DEF_STFLIB_KEVT_SCANRATE_MODE_LOW);
		}
...
static int shtps_ioctl_lpwg_double_tap_enable(struct shtps_fts *ts, unsigned long arg)
{
	SHTPS_LOG_FUNC_CALL();
	#if defined(SHTPS_GUARANTEE_I2C_ACCESS_IN_WAKE_ENABLE)
		if(shtps_check_suspend_state(ts, SHTPS_DETER_SUSPEND_I2C_PROC_IOCTL_SETLPWG_DOUBLETAP, (u8)arg) ==
		   0) {
...
static int shtps_ioctl_lpwg_single_tap_enable(struct shtps_fts *ts, unsigned long arg)
{
	SHTPS_LOG_FUNC_CALL();
	#if defined(SHTPS_GUARANTEE_I2C_ACCESS_IN_WAKE_ENABLE)
		if(shtps_check_suspend_state(ts, SHTPS_DETER_SUSPEND_I2C_PROC_IOCTL_SETLPWG_SINGLETAP, (u8)arg) ==
		   0) {
			shtps_ioctl_setlpwg_singletap_proc(ts, (u8)arg);
		}
	#else
		shtps_mutex_lock_ctrl();
		ts->lpwg.lpwg_single_tap_req_state = arg;
		ts->lpwg.notify_enable = 1;
		SHTPS_LOG_DBG_PRINT(" [LPWG] lpwg_single_tap_req_state = %d\n",
		                    ts->lpwg.lpwg_single_tap_req_state);
		shtps_set_lpwg_sleep_check(ts);
		shtps_mutex_unlock_ctrl();
	#endif /* SHTPS_GUARANTEE_I2C_ACCESS_IN_WAKE_ENABLE */
...
static int shtps_ioctl_get_single_tap_enable(struct shtps_fts *ts)
{
	#if defined(SHTPS_LPWG_SINGLE_TAP_ENABLE)
		return 1;
	#else  /* SHTPS_LPWG_SINGLE_TAP_ENABLE */
		return 0;
	#endif /* SHTPS_LPWG_SINGLE_TAP_ENABLE */
}
...
static int shtps_ioctl_set_high_report_mode(struct shtps_fts *ts, unsigned long arg)
{
	...
	#if defined(SHTPS_GUARANTEE_I2C_ACCESS_IN_WAKE_ENABLE)
		if(shtps_check_suspend_state(ts, SHTPS_DETER_SUSPEND_I2C_PROC_IOCTL_SET_HIGH_REPORT_MODE,
		                             (u8)arg) == 0) {
...
{
	SHTPS_LOG_FUNC_CALL();
	shtps_func_request_sync(ts, SHTPS_FUNC_REQ_BOOT_FW_UPDATE);
	return 0;
}

static int shtps_ioctl_wait_active(struct shtps_fts *ts)
{
	...
	ret = wait_event_interruptible(ts->wait_active,
			ts->is_lcd_on == 1);
...
static int shtps_ioctl_get_raw_self_data(struct shtps_fts *ts, unsigned long arg, int isCompat)
{
	SHTPS_LOG_FUNC_CALL();
	shtps_read_tmdata(ts, SHTPS_TMMODE_RAW_SELF_DATA);
	if(shtps_copy_to_user((u8 *)ts->diag.tm_data, arg,
	                      (shtps_get_tm_rxsize(ts) + shtps_get_tm_txsize(ts)) * 2, isCompat)) {
		return -EFAULT;
	}
...
static int shtps_ioctl_get_raw_self_data_wp(struct shtps_fts *ts, unsigned long arg, int isCompat)
{
	SHTPS_LOG_FUNC_CALL();
	shtps_read_tmdata(ts, SHTPS_TMMODE_RAW_SELF_DATA_WP);
	if(shtps_copy_to_user((u8 *)ts->diag.tm_data, arg,
	                      (shtps_get_tm_rxsize(ts) + shtps_get_tm_txsize(ts)) * 2, isCompat)) {
		return -EFAULT;
	}
...
static unsigned int shtpsif_poll(struct file *file, poll_table *wait)
{
	...
	ret = wait_event_interruptible_timeout(ts->diag.wait, ts->diag.event == 1,
...
static long shtpsif_ioctl_cmd(struct shtps_fts *ts, unsigned int cmd, unsigned long arg,
                              int isCompat)
{
	int rc = 0;
	...
	switch(cmd) {
		case TPSDEV_ENABLE:
			rc = shtps_ioctl_enable(ts);
			break;
		case TPSDEV_DISABLE:
			rc = shtps_ioctl_disable(ts);
			break;
		case TPSDEV_RESET:
			rc = shtps_ioctl_reset(ts);
			break;
		case TPSDEV_SOFT_RESET:
			rc = shtps_ioctl_softreset(ts);
			break;
		case TPSDEV_GET_FW_VERSION:
			rc = shtps_ioctl_getver(ts, arg, isCompat);
			break;
		case TPSDEV_GET_TOUCHINFO:
			rc = shtps_ioctl_get_touchinfo(ts, arg, isCompat);
			break;
		case TPSDEV_GET_TOUCHINFO_UNTRANS:
			rc = shtps_ioctl_get_touchinfo_untrans(ts, arg, isCompat);
			break;
		case TPSDEV_SET_TOUCHMONITOR_MODE:
			rc = shtps_ioctl_set_touchinfo_mode(ts, arg);
			break;
		case TPSDEV_READ_REG:
			rc = shtps_ioctl_reg_read(ts, arg, isCompat);
			break;
		case TPSDEV_READ_ALL_REG:
			rc = shtps_ioctl_reg_allread(ts, arg, isCompat);
			break;
		case TPSDEV_WRITE_REG:
			rc = shtps_ioctl_reg_write(ts, arg, isCompat);
			break;
		case TPSDEV_START_TM:
			rc = shtps_ioctl_tm_start(ts, arg);
			break;
		case TPSDEV_STOP_TM:
			rc = shtps_ioctl_tm_stop(ts);
			break;
		case TPSDEV_GET_BASELINE:
			rc = shtps_ioctl_get_baseline(ts, arg, isCompat);
			break;
		case TPSDEV_GET_FRAMELINE:
			rc = shtps_ioctl_get_frameline(ts, arg, isCompat);
			break;
		case TPSDEV_START_FACETOUCHMODE:
			rc = shtps_ioctl_start_facetouchmode(ts);
			break;
		case TPSDEV_STOP_FACETOUCHMODE:
			rc = shtps_ioctl_stop_facetouchmode(ts);
			break;
		case TPSDEV_POLL_FACETOUCHOFF:
			rc = shtps_ioctl_poll_facetouchoff(ts);
			break;
		case TPSDEV_GET_FWSTATUS:
			rc = shtps_ioctl_get_fwstatus(ts, arg, isCompat);
			break;
		case TPSDEV_GET_FWDATE:
			rc = shtps_ioctl_get_fwdate(ts, arg, isCompat);
			break;
		case TPSDEV_DEBUG_REQEVENT:
			rc = shtps_ioctl_debug_reqevent(ts, arg);
			break;
		case TPSDEV_SET_DRAGSTEP:
			rc = shtps_ioctl_set_dragstep(ts, arg);
			break;
		case TPSDEV_SET_FINGERFIXTIME:
			rc = shtps_ioctl_set_fingerfixtime(ts, arg);
			break;
		case TPSDEV_REZERO:
			rc = shtps_ioctl_rezero(ts, arg);
			break;
		case TPSDEV_ACK_FACETOUCHOFF:
			rc = shtps_ioctl_ack_facetouchoff(ts, arg);
			break;
		case TPSDEV_START_TM_F05:
			rc = shtps_ioctl_tmf05_start(ts, arg);
			break;
		case TPSDEV_SET_DRAGSTEP_X:
			rc = shtps_ioctl_set_dragstep_x(ts, arg);
			break;
		case TPSDEV_SET_DRAGSTEP_Y:
			rc = shtps_ioctl_set_dragstep_y(ts, arg);
			break;
		#if defined(SHTPS_LOG_OUTPUT_SWITCH_ENABLE)
			case TPSDEV_LOGOUTPUT_ENABLE:
				rc = shtps_ioctl_log_enable(ts, arg);
				break;
		#endif /* #if defined( SHTPS_LOG_OUTPUT_SWITCH_ENABLE ) */
		#if defined(SHTPS_FWUPDATE_BUILTIN_ENABLE)
			case TPSDEV_GET_FW_VERSION_BUILTIN:
				rc = shtps_ioctl_getver_builtin(ts, arg, isCompat);
				break;
		#endif /* #if defined( SHTPS_FWUPDATE_BUILTIN_ENABLE ) */
		case TPSDEV_SET_INVALID_AREA:
			rc = shtps_ioctl_set_invalid_area(ts, arg);
			break;
		#if defined(SHTPS_SMEM_BASELINE_ENABLE)
			case TPSDEV_GET_SMEM_BASELINE:
				rc = shtps_ioctl_get_smem_baseline(ts, arg, isCompat);
				break;
		#endif /* #if defined( SHTPS_SMEM_BASELINE_ENABLE ) */
		case TPSDEV_SET_LOWPOWER_MODE:
			rc = shtps_ioctl_set_low_power_mode(ts, arg);
			break;
		case TPSDEV_SET_CONT_LOWPOWER_MODE:
			rc = shtps_ioctl_set_continuous_low_power_mode(ts, arg);
			break;
		case TPSDEV_SET_LCD_LOWPOWER_MODE:
			rc = shtps_ioctl_set_lcd_bright_low_power_mode(ts, arg);
			break;
		#if defined(SHTPS_LPWG_MODE_ENABLE)
			case TPSDEV_LPWG_ENABLE:
				rc = shtps_ioctl_lpwg_enable(ts, arg);
				break;
		#endif /* SHTPS_LPWG_MODE_ENABLE */
		case TPSDEV_SET_VEILVIEW_STATE:
			rc = shtps_ioctl_set_veilview_state(ts, arg);
			break;
		case TPSDEV_READ_REG_BLOCK:
			rc = shtps_ioctl_reg_read_block(ts, arg, isCompat);
			break;
		case TPSDEV_WRITE_REG_BLOCK:
			rc = shtps_ioctl_reg_write_block(ts, arg, isCompat);
			break;
		case TPSDEV_GET_VEILVIEW_PATTERN:
			rc = shtps_ioctl_get_veilview_pattern(ts);
			break;
		case TPSDEV_READ_REG_PACKET:
			rc = shtps_ioctl_reg_read_packet(ts, arg, isCompat);
			break;
		case TPSDEV_WRITE_REG_PACKET:
			rc = shtps_ioctl_reg_write_packet(ts, arg, isCompat);
			break;
		case TPSDEV_HOVER_ENABLE:
			rc = shtps_ioctl_hover_enable(ts, arg);
			break;
		case TPSDEV_GET_BASELINE_RAW:
			rc = shtps_ioctl_get_baseline_raw(ts, arg, isCompat);
			break;
		case TPSDEV_BASELINE_OFFSET_DISABLE:
			rc = shtps_ioctl_baseline_offset_disable(ts);
			break;
		#if defined(SHTPS_CHECK_CRC_ERROR_ENABLE)
			case TPSDEV_CHECK_CRC_ERROR:
				rc = shtps_ioctl_check_crc_error(ts);
				break;
		#endif /* if defined( SHTPS_CHECK_CRC_ERROR_ENABLE ) */
		case TPSDEV_SET_NARROW_FRAME_MODE:
			rc = shtps_ioctl_set_narrow_frame_mode(ts, arg);
			break;
		#if defined(SHTPS_LOW_REPORTRATE_MODE)
			case TPSDEV_SET_LOW_REPORTRATE_MODE:
				rc = shtps_ioctl_set_low_reportrate_mode(ts, arg);
				break;
		#endif /* #if defined(SHTPS_LOW_REPORTRATE_MODE) */
		case TPSDEV_GET_SERIAL_NUMBER:
			rc = shtps_ioctl_get_serial_number(ts, arg, isCompat);
			break;
		case TPSDEV_GET_SERIAL_NUMBER_SIZE:
			rc = shtps_ioctl_get_serial_number_size(ts);
			break;
		#if defined(SHTPS_LPWG_DOUBLE_TAP_ENABLE)
			case TPSDEV_LPWG_DOUBLE_TAP_ENABLE:
				rc = shtps_ioctl_lpwg_double_tap_enable(ts, arg);
				break;
		#endif /* SHTPS_LPWG_DOUBLE_TAP_ENABLE */
		#if defined(SHTPS_GLOVE_DETECT_ENABLE)
			case TPSDEV_SET_GLOVE_MODE:
				rc = shtps_ioctl_set_glove_mode(ts, arg);
				break;
		#endif /* SHTPS_GLOVE_DETECT_ENABLE */
		#if defined(SHTPS_CTRL_FW_REPORT_RATE)
			case TPSDEV_SET_HIGH_REPORT_MODE:
				rc = shtps_ioctl_set_high_report_mode(ts, arg);
				break;
		#endif /* SHTPS_CTRL_FW_REPORT_RATE */
		case TPSDEV_GET_CBDATA:
			rc = shtps_ioctl_get_cbdata(ts, arg, isCompat);
			break;
		case TPSDEV_FW_UPGRADE:
			rc = shtps_ioctl_fw_upgrade(ts, arg, isCompat);
			break;
		case TPSDEV_BOOT_FW_UPDATE_REQ:
			rc = shtps_ioctl_boot_fw_update(ts, arg);
			break;
		case TPSDEV_GET_DOUBLE_TAP_ENABLE:
			rc = shtps_ioctl_get_double_tap_enable(ts);
			break;
		#if defined(CONFIG_SHTPS_FOCALTECH_POSITION_OFFSET)
			case TPSDEV_CALIBRATION_PARAM:
				rc = shtps_ioctl_calibration_param(ts, arg, isCompat);
				break;
			case TPSDEV_GET_CALIBRATION_PARAM:
				rc = shtps_ioctl_get_calibration_param(ts, arg, isCompat);
				break;
		#endif /* #if defined(CONFIG_SHTPS_FOCALTECH_POSITION_OFFSET) */
		case TPSDEV_WAIT_ACTIVE:
			rc = shtps_ioctl_wait_active(ts);
			break;
		#if defined(SHTPS_LPWG_SINGLE_TAP_ENABLE)
			case TPSDEV_LPWG_SINGLE_TAP_ENABLE:
				rc = shtps_ioctl_lpwg_single_tap_enable(ts, arg);
				break;
		#endif /* SHTPS_LPWG_SINGLE_TAP_ENABLE */
		case TPSDEV_GET_SINGLE_TAP_ENABLE:
			rc = shtps_ioctl_get_single_tap_enable(ts);
			break;

		case TPSDEV_GET_RAW_SELF_DATA:
			rc = shtps_ioctl_get_raw_self_data(ts, arg, isCompat);
			break;
		case TPSDEV_GET_RAW_SELF_DATA_WP:
			rc = shtps_ioctl_get_raw_self_data_wp(ts, arg, isCompat);
			break;
		default:
...
static long shtpsif_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		case COMPAT_TPSDEV_READ_REG:
			cmd = TPSDEV_READ_REG;
			break;
		case COMPAT_TPSDEV_READ_ALL_REG:
			cmd = TPSDEV_READ_ALL_REG;
			break;
		case COMPAT_TPSDEV_WRITE_REG:
			cmd = TPSDEV_WRITE_REG;
			break;
		case COMPAT_TPSDEV_GET_TOUCHKEYINFO:
			cmd = TPSDEV_GET_TOUCHKEYINFO;
			break;
		case COMPAT_TPSDEV_READ_REG_BLOCK:
			cmd = TPSDEV_READ_REG_BLOCK;
			break;
		case COMPAT_TPSDEV_WRITE_REG_BLOCK:
			cmd = TPSDEV_WRITE_REG_BLOCK;
			break;
		case COMPAT_TPSDEV_READ_REG_PACKET:
			cmd = TPSDEV_READ_REG_PACKET;
			break;
		case COMPAT_TPSDEV_WRITE_REG_PACKET:
			cmd = TPSDEV_WRITE_REG_PACKET;
			break;
		case COMPAT_TPSDEV_GET_TOUCHINFO:
			cmd = TPSDEV_GET_TOUCHINFO;
			break;
		case COMPAT_TPSDEV_GET_TOUCHINFO_UNTRANS:
			cmd = TPSDEV_GET_TOUCHINFO_UNTRANS;
			break;
		case COMPAT_TPSDEV_GET_BASELINE:
			cmd = TPSDEV_GET_BASELINE;
			break;
		case COMPAT_TPSDEV_GET_FRAMELINE:
			cmd = TPSDEV_GET_FRAMELINE;
			break;
		case COMPAT_TPSDEV_GET_SMEM_BASELINE:
			cmd = TPSDEV_GET_SMEM_BASELINE;
			break;
		case COMPAT_TPSDEV_GET_BASELINE_RAW:
			cmd = TPSDEV_GET_BASELINE_RAW;
			break;
		case COMPAT_TPSDEV_GET_SERIAL_NUMBER:
			cmd = TPSDEV_GET_SERIAL_NUMBER;
			break;
		case COMPAT_TPSDEV_GET_HYBRID_ADC:
			cmd = TPSDEV_GET_HYBRID_ADC;
			break;
		case COMPAT_TPSDEV_GET_ADC_RANGE:
			cmd = TPSDEV_GET_ADC_RANGE;
			break;
		case COMPAT_TPSDEV_GET_CBDATA:
			cmd = TPSDEV_GET_CBDATA;
			break;
		case COMPAT_TPSDEV_FW_UPGRADE:
			cmd = TPSDEV_FW_UPGRADE;
			break;
		#if defined(CONFIG_SHTPS_FOCALTECH_POSITION_OFFSET)
			case COMPAT_TPSDEV_CALIBRATION_PARAM:
				cmd = TPSDEV_CALIBRATION_PARAM;
				break;
			case COMPAT_TPSDEV_GET_CALIBRATION_PARAM:
				cmd = TPSDEV_GET_CALIBRATION_PARAM;
				break;
		#endif /* #if defined(CONFIG_SHTPS_FOCALTECH_POSITION_OFFSET) */
		case COMPAT_TPSDEV_GET_RAW_SELF_DATA:
			cmd = TPSDEV_GET_RAW_SELF_DATA;
			break;
		case COMPAT_TPSDEV_GET_RAW_SELF_DATA_WP:
			cmd = TPSDEV_GET_RAW_SELF_DATA_WP;
			break;
	}
...
int shtpsif_init(void)
{
	...
	shtpsif_device =
	    device_create(shtpsif_class, NULL, shtpsif_devid, &shtpsif_cdev, SH_TOUCH_IF_DEVNAME);
...
err_via_cdev_add:
	...
	device_destroy(shtpsif_class, shtpsif_devid);
...
/* -----------------------------------------------------------------------------------
 */
static int shtps_numStrToList(const char *numStr, /* [I  ] num strings */
...
int shtpsif_get_arguments(char *argStr,      /* [I/O] arguments strings (processed in function) */
...
static ssize_t shtpsif_store_common(struct kobject *kobj, struct kobj_attribute *attr,
                                    const char *buf, size_t count)
{
	return -EINVAL;
}
...
static ssize_t shtpsif_store_shtpsiflog(struct kobject *kobj, struct kobj_attribute *attr,
                                        const char *buf, size_t count)
{
	int argc;
	int argv[1];
...
static ssize_t shtpsif_store_enable(struct kobject *kobj, struct kobj_attribute *attr,
                                    const char *buf, size_t count)
{
	int ret;
	struct shtps_fts *ts = gShtps_fts;
...
static ssize_t shtpsif_store_disable(struct kobject *kobj, struct kobj_attribute *attr,
                                     const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
...
static ssize_t shtpsif_store_reset(struct kobject *kobj, struct kobj_attribute *attr,
                                   const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
	int power = (ts->state_mgr.state == SHTPS_STATE_IDLE) ? 0 : 1;
...
static ssize_t shtpsif_store_softreset(struct kobject *kobj, struct kobj_attribute *attr,
                                       const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
	int power = (ts->state_mgr.state == SHTPS_STATE_IDLE) ? 0 : 1;
...
static ssize_t shtpsif_show_fwupdate_buffer(struct kobject *kobj, struct kobj_attribute *attr,
                                            char *buf)
{
	SHTPSIF_LOG_FUNC_CALL();
...
static ssize_t shtpsif_store_fwupdate_buffer(struct kobject *kobj, struct kobj_attribute *attr,
                                             const char *buf, size_t count)
{
	SHTPSIF_LOG_FUNC_CALL();
...
static ssize_t shtpsif_store_clear_fwupdate_buffer(struct kobject *kobj,
...
static ssize_t shtpsif_store_fwupdate(struct kobject *kobj, struct kobj_attribute *attr,
                                      const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
	int rc;
...
static ssize_t shtpsif_show_touchinfo_untrans(struct kobject *kobj, struct kobj_attribute *attr,
                                              char *buf)
{
	struct shtps_fts *ts = gShtps_fts;
...
static ssize_t shtpsif_store_touchinfo_mode(struct kobject *kobj, struct kobj_attribute *attr,
                                            const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
	int argc;
...
static ssize_t shtpsif_store_reg_read(struct kobject *kobj, struct kobj_attribute *attr,
                                      const char *buf, size_t count)
{
	int argc;
	int argv[2];
...
static ssize_t shtpsif_store_reg_write(struct kobject *kobj, struct kobj_attribute *attr,
                                       const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
	unsigned short addr;
...
static ssize_t shtpsif_store_tm_start(struct kobject *kobj, struct kobj_attribute *attr,
                                      const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
	int argc;
...
static ssize_t shtpsif_store_tm_stop(struct kobject *kobj, struct kobj_attribute *attr,
                                     const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
...
static ssize_t shtpsif_show_baseline_raw(struct kobject *kobj, struct kobj_attribute *attr,
                                         char *buf)
{
	struct shtps_fts *ts = gShtps_fts;
	int size;
...
static ssize_t shtpsif_show_poll_touch_info(struct kobject *kobj, struct kobj_attribute *attr,
                                            char *buf)
{
	int ret;
	int i;
...
static ssize_t shtpsif_store_start_facetouchmode(struct kobject *kobj, struct kobj_attribute *attr,
                                                 const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
	int ret;
...
static ssize_t shtpsif_store_stop_facetouchmode(struct kobject *kobj, struct kobj_attribute *attr,
                                                const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
	int ret;
...
static ssize_t shtpsif_show_poll_facetouchoff(struct kobject *kobj, struct kobj_attribute *attr,
                                              char *buf)
{
	int rc;
...
#if defined(CONFIG_SHTPS_FOCALTECH_POSITION_OFFSET)
static ssize_t shtpsif_show_calibration_param(struct kobject *kobj, struct kobj_attribute *attr,
                                              char *buf)
{
	struct shtps_fts *ts = gShtps_fts;
	...
	return snprintf(buf, PAGE_SIZE, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
	                ts->offset.base[0], ts->offset.base[1], ts->offset.base[2], ts->offset.base[3],
	                ts->offset.base[4], ts->offset.diff[0], ts->offset.diff[1], ts->offset.diff[2],
	                ts->offset.diff[3], ts->offset.diff[4], ts->offset.diff[5], ts->offset.diff[6],
	                ts->offset.diff[7], ts->offset.diff[8], ts->offset.diff[9], ts->offset.diff[10],
	                ts->offset.diff[11]);
}
static ssize_t shtpsif_store_calibration_param(struct kobject *kobj, struct kobj_attribute *attr,
                                               const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
	const u8 *data;
...
static ssize_t shtpsif_store_rezero(struct kobject *kobj, struct kobj_attribute *attr,
                                    const char *buf, size_t count)
{
	SHTPSIF_LOG_FUNC_CALL();
	return count;
...
static ssize_t shtpsif_store_ack_facetouchoff(struct kobject *kobj, struct kobj_attribute *attr,
                                              const char *buf, size_t count)
{
	#if defined(CONFIG_SHTPS_FOCALTECH_FACETOUCH_OFF_DETECT)
		struct shtps_fts *ts = gShtps_fts;
...
static ssize_t shtpsif_store_log_enable(struct kobject *kobj, struct kobj_attribute *attr,
                                        const char *buf, size_t count)
{
	int argc;
	int argv[1];
...
#if defined(SHTPS_FWUPDATE_BUILTIN_ENABLE)
static ssize_t shtpsif_show_fwver_builtin(struct kobject *kobj, struct kobj_attribute *attr,
                                          char *buf)
{
	struct shtps_fts *ts = gShtps_fts;
	u16 ver = shtps_fwver_builtin(ts);
...
#if defined(SHTPS_SMEM_BASELINE_ENABLE)
static ssize_t shtpsif_show_smem_baseline(struct kobject *kobj, struct kobj_attribute *attr,
                                          char *buf)
{
	struct shtps_fts *ts = gShtps_fts;
	sharp_smem_common_type *smemdata = sh_smem_get_common_address();
...
static ssize_t shtpsif_store_low_power_mode(struct kobject *kobj, struct kobj_attribute *attr,
                                            const char *buf, size_t count)
{
	#if defined(SHTPS_LOW_POWER_MODE_ENABLE)
		struct shtps_fts *ts = gShtps_fts;
...
static ssize_t shtpsif_store_continuous_low_power_mode(struct kobject *kobj,
                                                       struct kobj_attribute *attr, const char *buf,
                                                       size_t count)
{
	#if defined(SHTPS_LOW_POWER_MODE_ENABLE)
...
SHTPSIF_DEFINE(continuous_low_power_mode, SHTPSIF_SHOW_COMMON,
               shtpsif_store_continuous_low_power_mode);

static ssize_t shtpsif_store_lcd_bright_low_power_mode(struct kobject *kobj,
                                                       struct kobj_attribute *attr, const char *buf,
                                                       size_t count)
{
	#if defined(SHTPS_LOW_POWER_MODE_ENABLE)
...
SHTPSIF_DEFINE(lcd_bright_low_power_mode, SHTPSIF_SHOW_COMMON,
               shtpsif_store_lcd_bright_low_power_mode);

#if defined(SHTPS_LPWG_MODE_ENABLE)
static ssize_t shtpsif_show_lpwg_enable(struct kobject *kobj, struct kobj_attribute *attr,
                                        char *buf)
{
	struct shtps_fts *ts = gShtps_fts;
...
static ssize_t shtpsif_store_lpwg_enable(struct kobject *kobj, struct kobj_attribute *attr,
                                         const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
	int argc;
...
#if defined(SHTPS_LPWG_DOUBLE_TAP_ENABLE)
static ssize_t shtpsif_show_lpwg_double_tap_enable(struct kobject *kobj,
...
static ssize_t shtpsif_store_lpwg_double_tap_enable(struct kobject *kobj,
                                                    struct kobj_attribute *attr, const char *buf,
                                                    size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
...
SHTPSIF_DEFINE(lpwg_double_tap_enable, shtpsif_show_lpwg_double_tap_enable,
               shtpsif_store_lpwg_double_tap_enable);
#endif /* SHTPS_LPWG_DOUBLE_TAP_ENABLE */

#if defined(SHTPS_LPWG_SINGLE_TAP_ENABLE)
static ssize_t shtpsif_show_lpwg_single_tap_enable(struct kobject *kobj,
                                                   struct kobj_attribute *attr, char *buf)
{
	struct shtps_fts *ts = gShtps_fts;
...
static ssize_t shtpsif_store_lpwg_single_tap_enable(struct kobject *kobj,
                                                    struct kobj_attribute *attr, const char *buf,
                                                    size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
	...
	shtps_ioctl_lpwg_single_tap_enable(ts, argv[0]);

	return count;
}
SHTPSIF_DEFINE(lpwg_single_tap_enable, shtpsif_show_lpwg_single_tap_enable,
               shtpsif_store_lpwg_single_tap_enable);
#endif /* SHTPS_LPWG_SINGLE_TAP_ENABLE */

static ssize_t shtpsif_store_veilview_state(struct kobject *kobj, struct kobj_attribute *attr,
                                            const char *buf, size_t count)
{
	SHTPSIF_LOG_FUNC_CALL();
...
static ssize_t shtpsif_show_veilview_pattern(struct kobject *kobj, struct kobj_attribute *attr,
                                             char *buf)
{
	SHTPSIF_LOG_FUNC_CALL();
...
static ssize_t shtpsif_store_veilview_pattern(struct kobject *kobj, struct kobj_attribute *attr,
                                              const char *buf, size_t count)
{
	#if defined(SHTPS_DEBUG_VARIABLE_DEFINES)
		int argc;
...
static ssize_t shtpsif_store_baseline_offset_disable(struct kobject *kobj,
                                                     struct kobj_attribute *attr, const char *buf,
                                                     size_t count)
{
	int ret = -1;
...
#if defined(SHTPS_CHECK_CRC_ERROR_ENABLE)
static ssize_t shtpsif_store_check_crc_error(struct kobject *kobj, struct kobj_attribute *attr,
                                             const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
...
static ssize_t shtpsif_show_device_access_log_enable(struct kobject *kobj,
                                                     struct kobj_attribute *attr, char *buf)
{
	SHTPSIF_LOG_FUNC_CALL();
...
static ssize_t shtpsif_store_device_access_log_enable(struct kobject *kobj,
                                                      struct kobj_attribute *attr, const char *buf,
                                                      size_t count)
{
	int argc;
...
SHTPSIF_DEFINE(device_access_log_enable, shtpsif_show_device_access_log_enable,
               shtpsif_store_device_access_log_enable);
#endif /* #if defined(SHTPS_DEVICE_ACCESS_LOG_ENABLE) */

#if defined(SHTPS_GLOVE_DETECT_ENABLE)
static ssize_t shtpsif_store_glove_enable(struct kobject *kobj, struct kobj_attribute *attr,
                                          const char *buf, size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
	int argc;
...
#if defined(SHTPS_CTRL_FW_REPORT_RATE)
static ssize_t shtpsif_show_high_report_mode_enable(struct kobject *kobj,
                                                    struct kobj_attribute *attr, char *buf)
{
	struct shtps_fts *ts = gShtps_fts;
...
static ssize_t shtpsif_store_high_report_mode_enable(struct kobject *kobj,
                                                     struct kobj_attribute *attr, const char *buf,
                                                     size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
...
SHTPSIF_DEFINE(high_report_mode_enable, shtpsif_show_high_report_mode_enable,
               shtpsif_store_high_report_mode_enable);
#endif /* SHTPS_CTRL_FW_REPORT_RATE */

static ssize_t shtpsif_show_hw_revision(struct kobject *kobj, struct kobj_attribute *attr,
                                        char *buf)
{
		...
		#if defined(SHTPS_TPIN_CHECK_ENABLE)
			return snprintf(
			    buf, PAGE_SIZE, "hw_revision=%s(%d) hw_type=%s(%d) tpin=%d\n",
		#else
			return snprintf(
			    buf, PAGE_SIZE, "hw_revision=%s(%d) hw_type=%s(%d)\n",
		#endif /* SHTPS_TPIN_CHECK_ENABLE */
				...
				: "UNKNOWN")))))))),
			    hw_revision, (hw_type == SHTPS_HW_TYPE_BOARD ? "BOARD" : "HANDSET"), hw_type
		#if defined(SHTPS_TPIN_CHECK_ENABLE)
			    ,
			    tpin
		#endif /* SHTPS_TPIN_CHECK_ENABLE */
...
{
	struct shtps_fts *ts = gShtps_fts;
	char* type = (ts->lpwg.gesture_type == SHTPS_GESTURE_TYPE_ONE_FINGER_SINGLE_TAP) ? "single" : (ts->lpwg.gesture_type == SHTPS_GESTURE_TYPE_ONE_FINGER_DOUBLE_TAP) ? "double" : "other";

	SHTPSIF_LOG_FUNC_CALL();

	return scnprintf(buf, PAGE_SIZE, "%s,%d,%d\n", type, ts->lpwg.gesture_coordinates_x, ts->lpwg.gesture_coordinates_y);
}
...
static ssize_t shtpsif_show_stflib_change_parameter(struct kobject *kobj,
                                                    struct kobj_attribute *attr, char *buf)
{
	struct shtps_fts *ts = gShtps_fts;
...
static ssize_t shtpsif_store_stflib_change_parameter(struct kobject *kobj,
                                                     struct kobj_attribute *attr, const char *buf,
                                                     size_t count)
{
	struct shtps_fts *ts = gShtps_fts;
...
SHTPSIF_DEFINE(stflib_change_parameter, shtpsif_show_stflib_change_parameter,
               shtpsif_store_stflib_change_parameter);
#endif /* SHTPS_DEVELOP_MODE_ENABLE */

#if defined(SHTPS_SEND_DUMMYKEY_EVENT_ENABLE)
static ssize_t shtpsif_store_send_dummykey_event(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	unsigned int input_timer_ms;
	int i;
	long unsigned int current_jiffies = jiffies;
	long unsigned int next_expire_jiffies;

	if(sscanf(buf, "%u %d", &input_event, &input_value) != 2) {
		if(sscanf(buf, "%u", &input_event) != 1) {
			return count;
		}
		else {
			input_value = 0;
	...
	if(input_event != SHTPS_DEF_STFLIB_KEVT_TIMEOUT) {
		shtps_report_stflib_kevt(ts, input_event, input_value);
		return count;
	}

	/* timer event */
	if(sscanf(buf, "%u %u %u", &input_event, &input_id, &input_timer_ms) != 3) {
		if(sscanf(buf, "%u %u", &input_event, &input_id) != 2) {
			return count;
		}
		else {
			input_timer_ms = UINT_MAX;
		}
	}

	cancel_delayed_work(&ts->send_dummykey_event_delayed_work);

	for(i = 0; i < SHTPS_SEND_DUMMYKEY_EVENT_INFO_MAX; i++) {
		if(ts->send_dummykey_event_info[i].expire_jiffies != 0) {
			if(ts->send_dummykey_event_info[i].id == input_id) {
				ts->send_dummykey_event_info[i].expire_jiffies = 0;
			}
		}
	}

	if(input_timer_ms != UINT_MAX) {
		for(i = 0; i < SHTPS_SEND_DUMMYKEY_EVENT_INFO_MAX; i++) {
			if(ts->send_dummykey_event_info[i].expire_jiffies == 0) {
				ts->send_dummykey_event_info[i].id = input_id;
				ts->send_dummykey_event_info[i].expire_jiffies = current_jiffies + msecs_to_jiffies(input_timer_ms);
				break;
			}
		}
	}
	...
	for(i = 0; i < SHTPS_SEND_DUMMYKEY_EVENT_INFO_MAX; i++) {
		if(ts->send_dummykey_event_info[i].expire_jiffies != 0) {
			if(	(next_expire_jiffies == 0) ||
				(next_expire_jiffies > ts->send_dummykey_event_info[i].expire_jiffies)) {
				next_expire_jiffies = ts->send_dummykey_event_info[i].expire_jiffies;
			}
		}
	}
	...
	if(next_expire_jiffies > 0) {
		schedule_delayed_work(&ts->send_dummykey_event_delayed_work, next_expire_jiffies - current_jiffies);
	}

	return count;
}
SHTPSIF_DEFINE(send_dummykey_event,
			NULL,
			shtpsif_store_send_dummykey_event);
#endif /* SHTPS_SEND_DUMMYKEY_EVENT_ENABLE */

static ssize_t shtpsif_show_raw_self_data(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct shtps_fts *ts = gShtps_fts;
	int size;
...
SHTPSIF_DEFINE(raw_self_data, shtpsif_show_raw_self_data, SHTPSIF_STORE_COMMON);

static ssize_t shtpsif_show_raw_self_data_wp(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct shtps_fts *ts = gShtps_fts;
	int size;
...
SHTPSIF_DEFINE(raw_self_data_wp, shtpsif_show_raw_self_data_wp, SHTPSIF_STORE_COMMON);

#if defined(SHTPS_PANEL_KIND_DETECT_ENABLE)
static ssize_t shtpsif_show_update_panel_kind(struct kobject *kobj,
                                                    struct kobj_attribute *attr, char *buf)
{
	int shtps_touchpanel_kind;

	shtps_touchpanel_kind = shtps_touchpanel_module_update_panel_kind();

	return snprintf(buf, PAGE_SIZE, "%d\n", shtps_touchpanel_kind);
}
SHTPSIF_DEFINE(update_panel_kind, shtpsif_show_update_panel_kind,
               NULL);

static ssize_t shtpsif_show_get_panel_kind(struct kobject *kobj,
                                                    struct kobj_attribute *attr, char *buf)
{
	int shtps_touchpanel_kind;

	shtps_touchpanel_kind = shtps_touchpanel_module_get_panel_kind();

	return snprintf(buf, PAGE_SIZE, "%d\n", shtps_touchpanel_kind);
}
SHTPSIF_DEFINE(get_panel_kind, shtpsif_show_get_panel_kind,
               NULL);
#endif /* SHTPS_PANEL_KIND_DETECT_ENABLE */
...
static struct attribute *attrs_shtpsif[] = {
	...
	#endif /* SHTPS_LPWG_DOUBLE_TAP_ENABLE */
	#if defined(SHTPS_LPWG_SINGLE_TAP_ENABLE)
		&shtpsif_lpwg_single_tap_enable.attr,
	#endif /* SHTPS_LPWG_SINGLE_TAP_ENABLE */
	...
	#endif /* SHTPS_DEVELOP_MODE_ENABLE */
	#if defined(SHTPS_SEND_DUMMYKEY_EVENT_ENABLE)
		&shtpsif_send_dummykey_event.attr,
	#endif /* SHTPS_SEND_DUMMYKEY_EVENT_ENABLE */
	&shtpsif_raw_self_data.attr,
	&shtpsif_raw_self_data_wp.attr,
	#if defined(SHTPS_PANEL_KIND_DETECT_ENABLE)
		&shtpsif_update_panel_kind.attr,
		&shtpsif_get_panel_kind.attr,
	#endif /* SHTPS_PANEL_KIND_DETECT_ENABLE */
	NULL
};
	...
	if(ts->kobj == NULL) {
		...
		if(gShtps_kobj_group_shtpsif_p == NULL){
			...
			if(sysfs_create_group(gShtps_kobj_group_shtpsif_p, &shtps_attr_grp_shtpsif)){
				...
				ts->lpwg.gesture_notify_attr_name = shtpsif_wake_event.attr.name;
			#endif /* SHTPS_SUPPORT_GESTURE_NOTIFY */
		}
	}
}
...
void shtps_deinit_io_debugfs(struct shtps_fts *ts)
{
	...
	if(ts->kobj != NULL){
		if(gShtps_kobj_group_shtpsif_p != NULL){
			...
			gShtps_kobj_group_shtpsif_p = NULL;
		}
		kobject_put(ts->kobj);
		ts->kobj = NULL;
	}
