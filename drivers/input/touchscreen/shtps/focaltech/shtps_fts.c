...
int shtps_drm_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (evdata && evdata->data) {
		SHTPS_LOG_DBG_PRINT("start. event=%d blank=%d\n", event, *((int*)evdata->data));
		...
		if (event == DRM_PANEL_EVENT_DPMS) {
			...
			if((ts->first_resume_done == false) && (blank == DRM_PANEL_BLANK_UNBLANK)) {
				ts->first_resume_done = true;
...
#endif /* SHTPS_AMBIENT_DISPLAY_ENABLE */

		SHTPS_LOG_DBG_PRINT("end\n");
	}

	return 0;
}
static void shtps_panel_notifier_register_work(struct work_struct *work)
{
	struct shtps_fts *ts =
			container_of(work, struct shtps_fts,
	...
	SHTPS_LOG_DBG_PRINT("%s: start.\n", __func__);

	for (retry = 0; retry <= SHTPS_PANEL_NOTIFIER_REGISTER_RETRY_MAX; retry++) {
		if (ts->panel_notifier_register_work_flg == false) {
			break;
		}

		count = of_count_phandle_with_args(ts->ctrl_dev_p->of_node, "panel", NULL);
		if (count > 0) {
			for (i = 0; i < count; i++) {
				node = of_parse_phandle(ts->ctrl_dev_p->of_node, "panel", i);
		...
		if (active_panel) {
			...
			rc = drm_panel_notifier_register(active_panel,
					&ts->drm_notifier);
			if (rc < 0) {
				if(retry < SHTPS_PANEL_NOTIFIER_REGISTER_RETRY_MAX) {
					SHTPS_LOG_ERR_PRINT(
							"%s: Failed to register panel notifier client. retry(%d/%d)\n",
							__func__, retry, SHTPS_PANEL_NOTIFIER_REGISTER_RETRY_MAX);

					msleep(SHTPS_PANEL_NOTIFIER_REGISTER_RETRY_INTERVAL_MS);
				}
				else {
					SHTPS_LOG_ERR_PRINT(
	...
	if (evdata && evdata->data) {
		...
		if(evdata->id == MSM_DRM_PRIMARY_DISPLAY){
				...
				if (event == MSM_DRM_EVENT_DPMS) {
					...
					if((ts->first_resume_done == false) && (blank == MSM_DRM_BLANK_UNBLANK)) {
						ts->first_resume_done = true;
                         ...
                         char *modalias, int gpio_irq)
{
	...
	#ifdef CONFIG_DRM_PANEL
		ts->panel_notifier_register_work_flg = true;
		INIT_WORK(&ts->panel_notifier_register_work, shtps_panel_notifier_register_work);
		schedule_work(&ts->panel_notifier_register_work);
	#else
		#ifdef CONFIG_DRM_MSM
...
fail_init_shtpsif:
	#ifdef CONFIG_DRM_PANEL
		ts->panel_notifier_register_work_flg = false;
		cancel_work_sync(&ts->panel_notifier_register_work);
		if (ts->active_panel != NULL) {
