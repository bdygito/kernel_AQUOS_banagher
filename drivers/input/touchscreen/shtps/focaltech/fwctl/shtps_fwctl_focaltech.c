...
/* -------------------------------------------------------------------------- */
static int shtps_Upgrade_ReadPram(struct shtps_fwctl_info *fc_p, unsigned int Addr,
                                                   ...
                                                   u32 dw_lenth)
{
	...
	for(i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if((reg_val[0] == 0x87 && reg_val[1] == 0x07) || (reg_val[0] == 0x87 && reg_val[1] == 0xA7)) {
			msleep(50);
			break;
		}
		else {
	...
	if(nowAddress == dw_lenth) {
		...
		while(1) {
			...
			if(FlashAddr == dw_lenth) {
				...
				if(shtps_Upgrade_ReadPram(fc_p, StartFlashAddr, pCheckBuffer + FlashAddr,
				                          dw_lenth - FlashAddr)) {
                                                ...
                                                u32 dw_lenth)
{
	...
	u32 temp;
	u32 lenght;
	u8 packet_buf[FTS_PACKET_LENGTH_WRITE_APP + 6];
	u8 auc_i2c_write_buf[10];
	u8 bt_ecc;
	int i_ret;
	unsigned char cmd[20];
	unsigned char Checksum = 0;
	u32 uStartAddr = 0x2000;
	...
	for(i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if((reg_val[0] == 0x87 && reg_val[1] == 0xA7) || (reg_val[0] == 0x87 && reg_val[1] == 0xA7)) {
	...
	if((0xF0 != reg_val[0] || 0xAA != reg_val[1]) && (i >= 15)) { /*erase flash fail*/
		SHTPS_LOG_ERR_PRINT("[UPGRADE]: erase app error.reset tp and reload FW!!");
		return -EIO;
	}
	...
	temp = 0;
	packet_number = (dw_lenth) / FTS_PACKET_LENGTH_WRITE_APP;
	packet_buf[0] = 0xbf;

	for(j = 0; j < packet_number; j++) {
		temp = uStartAddr + j * FTS_PACKET_LENGTH_WRITE_APP;
		packet_buf[1] = (u8)(temp >> 16);
		packet_buf[2] = (u8)(temp >> 8);
		packet_buf[3] = (u8)temp;
		uWriteAddr = temp;
		lenght = FTS_PACKET_LENGTH_WRITE_APP;
		packet_buf[4] = (u8)(lenght >> 8);
		packet_buf[5] = (u8)lenght;

		uCheckOff = uWriteAddr / lenght;

		for(i = 0; i < FTS_PACKET_LENGTH_WRITE_APP; i++) {
			packet_buf[6 + i] = pbt_buf[j * FTS_PACKET_LENGTH_WRITE_APP + i];
			bt_ecc ^= packet_buf[6 + i];
		}
		i_ret = M_DIRECT_WRITE_FUNC(fc_p, packet_buf, FTS_PACKET_LENGTH_WRITE_APP + 6);
		if(i_ret < 0) {
		...
		if(i == 30)
			SHTPS_LOG_DBG_PRINT("Query 6a reg time out value: 0x%x, driver set value:0x%x\n", temp,
			                    uCheckOff + uCheckStart);
	}

	if((dw_lenth) % FTS_PACKET_LENGTH_WRITE_APP > 0) {
		temp = uStartAddr + packet_number * FTS_PACKET_LENGTH_WRITE_APP;
		packet_buf[1] = (u8)(temp >> 16);
		packet_buf[2] = (u8)(temp >> 8);
		packet_buf[3] = (u8)temp;
		uWriteAddr = temp;
		temp = (dw_lenth) % FTS_PACKET_LENGTH_WRITE_APP;
		packet_buf[4] = (u8)(temp >> 8);
		packet_buf[5] = (u8)temp;

		uCheckOff = uWriteAddr / temp;

		for(i = 0; i < temp; i++) {
			packet_buf[6 + i] = pbt_buf[packet_number * FTS_PACKET_LENGTH_WRITE_APP + i];
			bt_ecc ^= packet_buf[6 + i];
		}
		i_ret = M_DIRECT_WRITE_FUNC(fc_p, packet_buf, temp + 6);
		if(i_ret < 0) {
