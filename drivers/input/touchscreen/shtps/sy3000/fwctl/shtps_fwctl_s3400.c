...
static int shtps_fwctl_s3400_map_construct_relating_f12_baseaddress(struct shtps_fwctl_info *fc_p);
	...
	for(page = 0; page <= SHTPS_PDT_PAGE_SIZE_MAX; page++){
		...
		for(i = 0xE9;i > 0x0a;i-=sizeof(pdt)){
			...
			case 0x12:
				...
				rc = shtps_fwctl_s3400_map_construct_relating_f12_baseaddress(fc_p);
				if(rc)	goto err_exit;
