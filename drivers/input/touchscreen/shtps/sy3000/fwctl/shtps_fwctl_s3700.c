...
static int shtps_fwctl_s3700_loader_get_partition_table(struct shtps_fwctl_info *fc_p)
{
	...
	partition_table_size = PARTITION_TABLE_READ_SIZE * F34_QUERY_BLOCKSIZE(fc_p->map_p->fn34.query.data);

	partition_table = (u8 *)kzalloc(partition_table_size, GFP_KERNEL);
	if(partition_table == NULL) {
		goto err_exit;
	}

	rc = M_READ_PACKET_FUNC(fc_p, fc_p->map_p->fn34.dataBase + 5, partition_table, partition_table_size);
	if(rc) {
		goto err_exit;
	}
	
	for(i = 0; ((8 * i) + 2) < partition_table_size; i++) {
...
static int shtps_fwctl_s3700_loader_write_config(struct shtps_fwctl_info *fc_p, u8 *fwdata_p, int blockSize)
{
	...
	tx_buf = (u8 *)kzalloc(blockSize, GFP_KERNEL);
	if(tx_buf == NULL) {
		goto err_exit;
	}
	...
	rc = M_WRITE_PACKET_FUNC(fc_p, fc_p->map_p->fn34.dataBase + 5, tx_buf, F34_QUERY_BLOCKSIZE(fc_p->map_p->fn34.query.data));
