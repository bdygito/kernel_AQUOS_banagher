...
static DEVICE_ATTR(calibrate, S_IRUSR | S_IWUSR,
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (dad->panel_scan_data_ppos) {
		pt_debug(dev, DL_DEBUG, "%s: dad->panel_scan_data_ppos %d dad->panel_scan_size %d bytes\n",
			__func__, dad->panel_scan_data_ppos, dad->panel_scan_size);
		if (dad->panel_scan_data_ppos >= dad->panel_scan_size) {
			return_length = 0;
			goto release_mutex;
		}

		if(dad->panel_scan_size - dad->panel_scan_data_ppos > (PAGE_SIZE - 1)) {
			return_length = (PAGE_SIZE - 1);
		}
		else {
			return_length = dad->panel_scan_size - dad->panel_scan_data_ppos;
		}
		memcpy(buf, &buf_out[dad->panel_scan_data_ppos], return_length);
		buf[return_length] = '\0';
		dad->panel_scan_data_ppos += return_length;
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (dad->panel_scan_data_ppos) {
		...
		index = pt_run_and_get_selftest_result(
			...
			dad->panel_scan_data_buf, sizeof(dad->panel_scan_data_buf),
			PT_ST_ID_CM_PANEL, PIP_CMD_MAX_LENGTH,
			PT_ST_GET_RESULTS, PT_ST_PRINT_RESULTS,
			PT_PR_FORMAT_DEFAULT);
		mutex_lock(&dad->sysfs_lock);

		dad->panel_scan_size = index;
		if(dad->panel_scan_size > (PAGE_SIZE - 1)) {
			return_length = (PAGE_SIZE - 1);
		}
		else {
...
static ssize_t pt_cm_panel_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_device_access_data *dad
		= pt_get_device_access_data(dev);
...
static DEVICE_ATTR(cm_panel, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP, 
	pt_cm_panel_show, pt_cm_panel_store);
...
static ssize_t pt_cp_panel_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_device_access_data *dad
		= pt_get_device_access_data(dev);
...
static DEVICE_ATTR(cp_panel, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP, 
	pt_cp_panel_show, pt_cp_panel_store);
