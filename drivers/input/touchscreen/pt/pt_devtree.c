		...
		struct device_node *core_node)
{
	...
	pdata->panel_kind_gpio = of_get_named_gpio(core_node, "parade,panel_kind_gpio", 0);
...
int pt_devtree_create_and_get_pdata(struct device *adap_dev)
{
	...
	for_each_child_of_node(adap_dev->of_node, core_node) {
...
#if defined(PARADE_PANEL_KIND_DETECT_ENABLE)
		{
			...
			ret = of_property_read_string(core_node, "parade,name", &core_name);
			if (!ret) {
				if (strncmp(core_name, "pt_core", strlen("pt_core")) != 0) {
					continue;
				}
			}
			else {
