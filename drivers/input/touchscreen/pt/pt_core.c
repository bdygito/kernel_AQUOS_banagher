...
static int pt_core_wake_(struct pt_core_data *cd)
{
	...
	cd->recover_i2c_error_flg = PARADE_RECOVER_I2C_ERROR_FLG_NORMAL;
...
#if defined(PARADE_RECOVER_I2C_ERROR_ENABLE)
static void pt_recover_i2c_error_work(struct work_struct *work)
{
	struct pt_core_data *cd =
			container_of(work, struct pt_core_data,
					recover_i2c_error_work);
	int rc;

	pt_debug(cd->dev, DL_ERROR, "%s: start.\n", __func__);
	...
	if ((rc < 0) || (cd->recover_i2c_error_flg != PARADE_RECOVER_I2C_ERROR_FLG_NORMAL)) {
		pt_debug(cd->dev, DL_ERROR, "%s: recover I2C error fail. Skip future I2C communication.\n", __func__);
		cd->recover_i2c_error_flg = PARADE_RECOVER_I2C_ERROR_FLG_RECOVER_ERROR;
	}
...
#endif /* PARADE_ASYNC_WAKE_ENABLE */
#if defined(PARADE_RECOVER_I2C_ERROR_ENABLE)
	INIT_WORK(&cd->recover_i2c_error_work, pt_recover_i2c_error_work);
#endif /* PARADE_RECOVER_I2C_ERROR_ENABLE */
...
skip_enum:
	...
	if (pt_active_panel) {
		rc = drm_panel_notifier_register(pt_active_panel,
				&cd->drm_notifier);
		if (rc < 0) {
			pt_debug(dev, DL_ERROR,
		...
		pt_debug(dev, DL_ERROR,
				"%s: Failed to get pt_active_panel\n",
				__func__);
	}
#endif
...
EXPORT_SYMBOL_GPL(pt_set_active_panel);
#endif
