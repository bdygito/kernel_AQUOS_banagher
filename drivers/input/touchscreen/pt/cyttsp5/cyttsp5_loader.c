...
#ifdef CYPRESS_CYTTSP5_SYSFS_BINARY_FW_UPGRADE
				 ...
				 struct cyttsp5_hex_image *row_image)
{
	...
	if (!data) {
		dev_err(dev, "%s: data kzalloc error\n", __func__);
		return -ENOMEM;
	}
#endif
...
#ifdef CYPRESS_CYTTSP5_SYSFS_BINARY_FW_UPGRADE
static ssize_t cyttsp5_fw_data_write(struct file *filp,
		...
		char *buf, loff_t offset, size_t count)
{
	...
	mutex_lock(&data->fw_lock);

	if (!data->fw_loading) {
		mutex_unlock(&data->fw_lock);
		return -ENODEV;
	}

	p = krealloc(data->fw_data, offset + count, GFP_KERNEL);
	if (!p) {
		kfree(data->fw_data);
		data->fw_data = NULL;
		mutex_unlock(&data->fw_lock);
	...
	memcpy(&data->fw_data[offset], buf, count);
	data->fw_size += count;
...
static struct bin_attribute bin_attr_fw_data = {
	...
	.size = 0,
	.write = cyttsp5_fw_data_write,
};

static ssize_t cyttsp5_fw_loading_show(struct device *dev,
...
/*
 * 1: Start loading TT fw
 * 0: End loading TT fw and perform upgrade
 *-1: Exit loading
 */
static ssize_t cyttsp5_fw_loading_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value == 1)
		...
		ld->fw_loading = false;
		if (ld->fw_size == 0) {
			dev_err(dev, "%s: No fw data\n", __func__);
			goto exit_free;
		}

		header_size = ld->fw_data[0];
		if (header_size >= (ld->fw_size + 1)) {
			dev_err(dev, "%s: fw format is invalid\n", __func__);
			goto exit_free;
		}

		rc = cyttsp5_upgrade_firmware(dev, &(ld->fw_data[header_size + 1]),
			ld->fw_size - (header_size + 1));
	}
...
static DEVICE_ATTR(fw_loading, S_IRUSR | S_IWUSR,
	cyttsp5_fw_loading_show, cyttsp5_fw_loading_store);
#endif /* CYPRESS_CYTTSP5_SYSFS_BINARY_FW_UPGRADE */
...
static int cyttsp5_loader_probe(struct device *dev, void **data)
{
	...
	if (rc) {
		dev_err(dev, "%s: Error, could not create fw_loading\n",
				__func__);
		goto error_create_fw_loading;
	}

	rc = device_create_bin_file(dev, &bin_attr_fw_data);
	if (rc) {
		dev_err(dev, "%s: Error, could not create fw_data\n",
				__func__);
		goto error_create_fw_data;
	}
#endif
...
#endif
#ifdef CYPRESS_CYTTSP5_SYSFS_BINARY_FW_UPGRADE
error_create_fw_data:
	device_remove_file(dev, &dev_attr_fw_loading);
error_create_fw_loading:
#endif
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_BINARY_FW_UPGRADE
