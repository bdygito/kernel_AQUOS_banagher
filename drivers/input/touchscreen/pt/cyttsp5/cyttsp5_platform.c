	...
	if (pdata->pinctrl == NULL) {
		...
		if (IS_ERR_OR_NULL(pdata->pinctrl)) {
			...
			pdata->tpin_state_active =
					pinctrl_lookup_state(pdata->pinctrl, "cyttsp5_test_mode_pull_up");
			if (IS_ERR_OR_NULL(pdata->tpin_state_active)) {
				dev_err(dev, "pinctrl lookup failed for cyttsp5_test_mode_pull_up\n");
				pdata->tpin_state_active = NULL;
			}
			pdata->tpin_state_suspend =
					pinctrl_lookup_state(pdata->pinctrl, "cyttsp5_test_mode_pull_down");
			if (IS_ERR_OR_NULL(pdata->tpin_state_suspend)) {
				dev_err(dev, "pinctrl lookup failed for cyttsp5_test_mode_pull_down\n");
				pdata->tpin_state_suspend = NULL;
			}
			pdata->int_state_active =
					pinctrl_lookup_state(pdata->pinctrl, "cyttsp5_int_active");
			if (IS_ERR_OR_NULL(pdata->int_state_active)) {
				dev_err(dev, "pinctrl lookup failed for cyttsp5_int_active\n");
				pdata->int_state_active = NULL;
			}
			pdata->int_state_standby =
					pinctrl_lookup_state(pdata->pinctrl, "cyttsp5_int_standby");
			if (IS_ERR_OR_NULL(pdata->int_state_standby)) {
				dev_err(dev, "pinctrl lookup failed for cyttsp5_int_standby\n");
				pdata->int_state_standby = NULL;
			}
			pdata->int_state_no_pull =
					pinctrl_lookup_state(pdata->pinctrl, "cyttsp5_int_no_pull");
			if (IS_ERR_OR_NULL(pdata->int_state_no_pull)) {
				dev_err(dev, "pinctrl lookup failed for cyttsp5_int_no_pull\n");
				pdata->int_state_no_pull = NULL;
			}
			pdata->reset_state_active =
					pinctrl_lookup_state(pdata->pinctrl, "cyttsp5_reset_active");
			if (IS_ERR_OR_NULL(pdata->reset_state_active)) {
				dev_err(dev, "pinctrl lookup failed for cyttsp5_reset_active\n");
				pdata->reset_state_active = NULL;
			}
			pdata->reset_state_standby =
					pinctrl_lookup_state(pdata->pinctrl, "cyttsp5_reset_standby");
			if (IS_ERR_OR_NULL(pdata->reset_state_standby)) {
				dev_err(dev, "pinctrl lookup failed for cyttsp5_reset_standby\n");
				pdata->reset_state_standby = NULL;
			}
#if defined(CYPRESS_CYTTSP5_PANEL_KIND_DETECT_ENABLE)
			pdata->panel_kind_state_pullup =
					pinctrl_lookup_state(pdata->pinctrl, "cyttsp5_panel_kind_pull_up");
			if (IS_ERR_OR_NULL(pdata->panel_kind_state_pullup)) {
				dev_err(dev, "pinctrl lookup failed for cyttsp5_panel_kind_pull_up\n");
				pdata->panel_kind_state_pullup = NULL;
			}
			pdata->panel_kind_state_pulldown =
					pinctrl_lookup_state(pdata->pinctrl, "cyttsp5_panel_kind_pull_down");
			if (IS_ERR_OR_NULL(pdata->panel_kind_state_pulldown)) {
				dev_err(dev, "pinctrl lookup failed for cyttsp5_panel_kind_pull_down\n");
				pdata->panel_kind_state_pulldown = NULL;
			}
#endif /* CYPRESS_CYTTSP5_PANEL_KIND_DETECT_ENABLE */
		}
	}
