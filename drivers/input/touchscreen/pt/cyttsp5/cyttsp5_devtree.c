		...
		struct device_node *core_node)
{
...
#if 1
	pdata->test_mode_gpio = of_get_named_gpio(core_node, "cy,test_mode_gpio", 0);
#endif
...
#if 1
	rc = of_property_read_string(core_node, "cy,pwr-reg-name", &name);
	if (rc < 0)
		pdata->pwr_reg_name = NULL;
	else
		pdata->pwr_reg_name = name;

	rc = of_property_read_string(core_node, "cy,bus-reg-name", &name);
	if (rc < 0)
	...
	pdata->panel_kind_gpio = of_get_named_gpio(core_node, "cy,panel_kind_gpio", 0);
...
int cyttsp5_devtree_create_and_get_pdata(struct device *adap_dev)
{
...
#ifdef CONFIG_DRM_PANEL
	{
		...
		count = of_count_phandle_with_args(adap_dev->of_node, "panel", NULL);
		if (count > 0) {
			for (i = 0; i < count; i++) {
				node = of_parse_phandle(adap_dev->of_node, "panel", i);
				panel = of_drm_find_panel(node);
				of_node_put(node);
				if (!IS_ERR(panel)) {
					cyttsp5_set_active_panel(panel);
					break;
				}
			}
		}
	...
	for_each_child_of_node(adap_dev->of_node, core_node) {
...
#if defined(CYPRESS_CYTTSP5_PANEL_KIND_DETECT_ENABLE)
		{
			...
			ret = of_property_read_string(core_node, "cy,name", &core_name);
			if (!ret) {
				if (strncmp(core_name, "cyttsp5_core", strlen("cyttsp5_core")) != 0) {
					continue;
				}
			}
			else {
