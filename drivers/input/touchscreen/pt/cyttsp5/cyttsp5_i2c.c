...
#include "cyttsp5_touchpanel_module.h"
...
#if defined(CYPRESS_CYTTSP5_PANEL_KIND_DETECT_ENABLE)
static ssize_t cyttsp5_sysfs_update_panel_kind_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int cyttsp5_touchpanel_kind;

	cyttsp5_touchpanel_kind = cyttsp5_touchpanel_module_update_panel_kind();

	return snprintf(buf, CY_MAX_PRBUF_SIZE, "%d\n", cyttsp5_touchpanel_kind);
}

static ssize_t cyttsp5_sysfs_get_panel_kind_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int cyttsp5_touchpanel_kind;

	cyttsp5_touchpanel_kind = cyttsp5_touchpanel_module_get_panel_kind();

	return snprintf(buf, CY_MAX_PRBUF_SIZE, "%d\n", cyttsp5_touchpanel_kind);
}
static struct device_attribute attributes[] = {
	__ATTR(update_panel_kind, (S_IRUSR | S_IRGRP), cyttsp5_sysfs_update_panel_kind_show, NULL),
	__ATTR(get_panel_kind, S_IRUGO, cyttsp5_sysfs_get_panel_kind_show, NULL),
};
...
int cyttsp5_i2c_probe(struct i2c_client *client,
	const struct i2c_device_id *i2c_id)
{
...

	{
		...
		struct cyttsp5_core_platform_data *cpdata = pdata->core_pdata;
		...
		cyttsp5_touchpanel_module_init();
		cyttsp5_touchpanel_module_set_data(dev, &touchpanel_module_data);
	}
...
#if defined(CYPRESS_CYTTSP5_PANEL_KIND_DETECT_ENABLE)
EXPORT_SYMBOL_GPL(cyttsp5_i2c_probe);
#endif /* CYPRESS_CYTTSP5_PANEL_KIND_DETECT_ENABLE */

int cyttsp5_i2c_remove(struct i2c_client *client)
{
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_DEVICETREE_SUPPORT
	struct device *dev = &client->dev;
	const struct of_device_id *match;
	...
	cyttsp5_touchpanel_module_clear_data(dev);
	cyttsp5_touchpanel_module_exit();
...
#if defined(CYPRESS_CYTTSP5_PANEL_KIND_DETECT_ENABLE)
EXPORT_SYMBOL_GPL(cyttsp5_i2c_remove);
#endif /* CYPRESS_CYTTSP5_PANEL_KIND_DETECT_ENABLE */
