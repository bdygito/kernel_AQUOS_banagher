...
//#define FTS_DEVELOP_MODE_ENABLE

#if defined( FTS_DEVELOP_MODE_ENABLE )
	#if defined( FTS_CORE )
		#define FTS_PARAM_DEF(name, val) \
			int name = val; \
			module_param(name, int, S_IRUGO | S_IWUSR);
	#else
		#define FTS_PARAM_DEF(name, val) 	extern int name;
	#endif /* FTS_CORE */
#else
	#if defined( FTS_CORE )
		#define FTS_PARAM_DEF(name, val) 	const int name = val;
	#else
		#define FTS_PARAM_DEF(name, val) 	extern const int name;
	#endif /* FTS_CORE */
#endif /* FTS_DEVELOP_MODE_ENABLE */



#define FTS_FW_UPDATE_CUSTOM_IF_ENABLE

#ifdef CONFIG_TOUCHSCREEN_TOUCHEVENT_NOTIFIER
	#define FTS_TOUCHEVENT_NOTIFIER_ENABLE
#endif
#ifdef CONFIG_SHARP_SHTERM
	#define FTS_SEND_SHTERM_EVENT_ENABLE
#endif

#define FTS_POWER_OFF_IN_SUSPEND_ENABLE
//#define FTS_USE_THREADED_IRQ_ENABLE
#define FTS_LPWG_ENABLE
#define FTS_CTRL_FW_REPORT_RATE_ENABLE
#define FTS_CONTROL_INPUT_SYNC_TIMING
#define FTS_CONTROL_EVENT_READ_NUM_AT_ONCE
#define	FTS_GLOVE_ENABLE
#define FTS_NOTIFIER_DIAG_TOUCH_ENABLE

#if defined(FTS_LPWG_ENABLE)
	#define FTS_LPWG_WAKE_LOCK_TIMEOUT_MS		2000

	FTS_PARAM_DEF(FTS_LPWG_PARAM_GESTURE_CONTROL_SETTING_D_TAP,				0x05)
	FTS_PARAM_DEF(FTS_LPWG_PARAM_GESTURE_CONTROL_SETTING_D_DOUBLETAP,		0x13)
	FTS_PARAM_DEF(FTS_LPWG_PARAM_GESTURE_CONTROL_SETTING_T_TAP_MAX,			0x14)
	FTS_PARAM_DEF(FTS_LPWG_PARAM_GESTURE_CONTROL_SETTING_T_DOUBLETAP_MIN,	0x04)
	FTS_PARAM_DEF(FTS_LPWG_PARAM_GESTURE_CONTROL_SETTING_T_DOUBLETAP_MAX,	0x28)
	FTS_PARAM_DEF(FTS_LPWG_PARAM_GESTURE_AREA_SETTING_CLIP_X,				0)
	FTS_PARAM_DEF(FTS_LPWG_PARAM_GESTURE_AREA_SETTING_CLIP_Y,				0)

	#define FTS_PROXIMITY_SUPPORT_ENABLE

	#if defined(FTS_PROXIMITY_SUPPORT_ENABLE)
		#include <misc/proximity.h>
		#define FTS_PROXIMITY_NEAR				SH_PROXIMITY_NEAR
		#define FTS_PROXIMITY_FAR				SH_PROXIMITY_FAR
	#endif /* FTS_PROXIMITY_SUPPORT_ENABLE */
#endif /* FTS_LPWG_ENABLE */
#if defined(FTS_CTRL_FW_REPORT_RATE_ENABLE)
	#define FTS_CTRL_FW_REPORT_RATE_SPEED_MID	0
	#define FTS_CTRL_FW_REPORT_RATE_SPEED_HIGH	1
	#define FTS_CTRL_FW_REPORT_RATE_SPEED_LOW	2
	#define FTS_CTRL_FW_REPORT_RATE_SPEED_UNKNOWN	255
#endif /* FTS_CTRL_FW_REPORT_RATE_ENABLE */

#define FTS_RESET_GPIO_MIN_MS		2
#define FTS_RESET_GPIO_MAX_MS		5
...
/* **** PANEL SPECIFICATION **** */
#define X_AXIS_MAX	1079	/* /< Max X coordinate of the display */
#define X_AXIS_MIN	0	/* /< min X coordinate of the display */
#define Y_AXIS_MAX	2339	/* /< Max Y coordinate of the display */
...
#define PRESSURE_MAX	511	/* /< Max value of pressure reported */
#else
#define PRESSURE_MAX	255	/* /< Max value of pressure reported */
...
#define TOUCH_ID_MAX	5	/* /< Max number of simoultaneous touches
				 * reported */
...
struct fts_hw_platform_data {
	...
	int dvdd_gpio;  /* /< number of the gpio associated to the dvdd pin */
	int avdd_gpio;  /* /< number of the gpio associated to the avdd pin */
...
struct fts_ts_info {
	...
	struct workqueue_struct  *event_wq;	/* /< Workqueue used for event
						 ...
						 * work threads */
	struct workqueue_struct  *suspend_resume_wq;	/* /< Workqueue used for suspend and resume
													 * work threads */
	...
	unsigned long touch_id;	/* /< Bitmask for touch id (mapped to input
				 * slots) */
	unsigned long is_palm_touch_id;
#ifdef STYLUS_MODE
	...
	bool dvdd_gpio_configured;
	bool avdd_gpio_configured;
	...
	bool is_firmware_loader_enabled;

#if defined(FTS_LPWG_ENABLE)
	unsigned short gesture_type;
	unsigned short gesture_coordinates_x;
	unsigned short gesture_coordinates_y;
	...
	unsigned int fw_report_rate_cur_state;
#endif /* FTS_CTRL_FW_REPORT_RATE_ENABLE */
};
