				     ...
				     size_t count, loff_t *pos)
{
	...
	pbuf = (char *)kmalloc(count * sizeof(char), GFP_KERNEL);
	if (pbuf == NULL) {
		logError(1,
			"%s %s: can not allocate memory for pbuf ERROR %08X",
			tag,
	...
	if (count > MESSAGE_MIN_HEADER_SIZE - 1 && p[0] == MESSAGE_START_BYTE) {
		...
		logError(0, "%s functionToTest[0] = %02X cmd[0]= %02X\n", tag,
			 funcToTest[0], cmd[0]);
		switch (funcToTest[0]) {
		case CMD_GETFWFILE:
	...
	logError(0, "%s Number of Parameters = %d\n", tag, numberParam);
...
int fts_proc_init(void)
{
	...
	entry = proc_create(DRIVER_TEST_FILE_NODE, 0660, fts_dir,
			    &fts_driver_test_ops);
