...
struct mutex fts_int_mutex;	/* /< mutex to controll the access to the
			 * disable_irq_counter */
...
int setScanMode(u8 mode, u8 settings)
{
	...
	if (mode == SCAN_MODE_LOW_POWER) {
		size = 2;
		ret = fts_write_wait_echo(cmd, size);
	}
	else {
		ret = fts_write(cmd, size);
					/* use write instead of writeFw because
...
int setFeatures(u8 feat, u8 *settings, int size)
{
	...
	if (cmd == NULL) {
		logError(1,
			"%s %s: can not allocate memory for cmd ERROR %08X",
			tag,
...
//	logError(0, "%s %s: Settings = ", tag, __func__);
	logError(0, "%s %s: Settings = %02X%s\n", tag, __func__, settings[0], (size > 1 ? " ..." : ""));
	for (i = 0; i < size; i++) {
		cmd[2 + i] = settings[i];
...
int writeSysCmd(u8 sys_cmd, u8 *sett, int size)
{
	...
	cmd = (u8 *)kmalloc((2 + size) * sizeof(u8), GFP_KERNEL);
	if (cmd == NULL) {
		logError(1, "%s %s: can not allocate cmd ERROR %08X",
...
int readSysInfo(int request)
{
	...
	/* logError(0, "%s Die Info =  ", tag); */
	for (i = 0; i < DIE_INFO_SIZE; i++)
		systemInfo.u8_dieInfo[i] = data[index++];
	/* logError(0, "%02X ", systemInfo.u8_dieInfo[i]); */
	/* logError(0, "\n"); */
	...
	/* logError(0, "%s Release Info =  ", tag); */
	for (i = 0; i < RELEASE_INFO_SIZE; i++)
		systemInfo.u8_releaseInfo[i] = data[index++];
	/* logError(0, "%02X ", systemInfo.u8_releaseInfo[i]); */
	/* logError(0, "\n"); */
...
int setFWReportRate(int rate)
{
	u8 cmd[4] = { 0xC0, 0x00, 0x00, 0x00 };
	int ret, size = 4;

	if(rate == FTS_CTRL_FW_REPORT_RATE_SPEED_MID){
		cmd[2] = 0x01;
	}
	else if(rate == FTS_CTRL_FW_REPORT_RATE_SPEED_HIGH){
		cmd[2] = 0x00;
	}
	else if(rate == FTS_CTRL_FW_REPORT_RATE_SPEED_LOW){
		cmd[2] = 0x02;
	}

	logError(0, "%s %s: Setting Dynamic report rate: param = %02X !\n",
		 tag, __func__, cmd[2]);
	...
	logError(0, "%s %s: Setting Dynamic report rate OK!\n", tag, __func__);
	return OK;
}
