...
static void flash_firmware_cont(const struct firmware *fw, void *context)
{
	struct device *dev = NULL;
	...
	if (!fw)
		goto flash_firmware_cont_exit;

	if (!fw->data || !fw->size) {
		logError(1, "%s %s: No firmware received\n", tag, __func__);
		goto flash_firmware_cont_release_exit;
	}

	Fw.data = NULL;
	logError(0, "%s %s: Reading Fw file...\n", tag, __func__);

	// get FW data
	orig_size = fw->size;
	orig_data = (u8 *)kmalloc((orig_size) * sizeof(u8), GFP_KERNEL);
	if (orig_data == NULL) {
		logError(1,
	...
	memcpy(orig_data, (u8 *)fw->data, (orig_size));

	res = parseBinFile(orig_data, orig_size, &Fw, keep_cx); // orig_data is freed here
	if (res < OK) {
	...
	logError(0, "%s %s: Fw file read COMPLETED!\n", tag, __func__);

	logError(0, "%s %s: Starting flashing procedure...\n", tag, __func__);
	res = flash_burn(Fw, force, keep_cx);
	if (res < OK && res != (ERROR_FW_NO_UPDATE | ERROR_FLASH_BURN_FAILED)) {
	...
	logError(0, "%s %s: flashing procedure Finished!\n", tag, __func__);
	kfree(Fw.data);

flash_firmware_cont_release_exit:
	release_firmware(fw);

flash_firmware_cont_exit:
...
int requestFlashLoader(int force, int keep_cx)
{
	...
	res = request_firmware_nowait(THIS_MODULE, FW_ACTION_NOHOTPLUG,
			"fts_firmware_update", dev, GFP_KERNEL, (void *)flag, flash_firmware_cont);
	if (res < 0) {
		logError(1, "%s %s: Fail request firmware class file load\n", tag, __func__);
		return ERROR_OP_NOT_ALLOW;
	}
...
int flash_firmware_custom(const u8 *data, size_t size, int keep_cx)
{
	...
	if (dev == NULL) {
		logError(1,
			 ...
			 ERROR_OP_NOT_ALLOW);
		res = -1;
		goto flash_firmware_custom_exit;
	}
	info = dev_get_drvdata(dev);
	...
	memcpy(orig_data, (u8 *)data, (orig_size));
	...
	logError(1, "%s %s: Starting flashing procedure...\n", tag, __func__);
	res = flash_burn(Fw, force, keep_cx);
	if (res < OK && res != (ERROR_FW_NO_UPDATE | ERROR_FLASH_BURN_FAILED)) {
		logError(1, "%s %s: ERROR %08X\n", tag,
			 __func__, ERROR_FLASH_PROCEDURE);
	...
	logError(1, "%s %s: flashing procedure Finished!\n", tag, __func__);
	kfree(Fw.data);

	res = 0;

flash_firmware_custom_exit:
	info->is_firmware_loader_enabled = 0;
