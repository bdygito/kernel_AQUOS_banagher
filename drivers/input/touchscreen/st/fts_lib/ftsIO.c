...
static bool					check_write_echo_flg = false;
static int					check_write_echo;
static char					check_write_echo_value[4];
static int					check_write_echo_len;
static wait_queue_head_t	check_write_echo_wait;
static struct mutex			wait_write_echo_mutex;
#define FTS_CHECK_WRITE_ECHO_TIMEOUT_MS		100
...
int openChannel(void *clt)
{
	...
	mutex_init(&wait_write_echo_mutex);
	init_waitqueue_head(&check_write_echo_wait);
...
#if I2C_MEMORY_ALIGNED
	alloc_size = sizeof(u8) * byteToRead;
	alloc_buf = (u8 *)kzalloc(alloc_size, GFP_KERNEL);
	if(alloc_buf == NULL){
		logError(1, "%s %s: alloc error. size = %d\n", tag, __func__, alloc_size);
		return -ENOMEM;
	}
#endif
...
#if I2C_MEMORY_ALIGNED
		if (ret >= OK) {
			memcpy(outBuf, alloc_buf, I2CMsg[0].len);
		}
#endif
#else
...
#ifdef FTS_DEVELOP_MODE_ENABLE
	logCommunication(I2C_IO_READ, outBuf, byteToRead);
#endif
...
#if I2C_MEMORY_ALIGNED
		if (ret >= OK) {
			memcpy(outBuf, alloc_buf, I2CMsg[1].len);
		}
#endif
#else
		ret = spi_sync(getClient(), &msg);
...
#ifdef FTS_DEVELOP_MODE_ENABLE
	logCommunication(I2C_IO_WRITE, cmd, cmdLength);
	logCommunication(I2C_IO_READ, outBuf, byteToRead);
#endif
	ret = OK;
...
#if I2C_MEMORY_ALIGNED
		if (ret >= OK) {
			memcpy(outBuf, alloc_buf, I2CMsg[2].len);
		}
#endif
#else
		ret = spi_sync(getClient(), &msg);
...
#ifdef FTS_DEVELOP_MODE_ENABLE
	logCommunication(I2C_IO_WRITE, writeCmd1, writeCmdLength);
	logCommunication(I2C_IO_WRITE, readCmd1, readCmdLength);
	logCommunication(I2C_IO_READ, outBuf, byteToRead);
#endif
	ret = OK;
		  ...
		  dataSize)
{
	...
	finalCmd = (u8 *)kmalloc((1 + addrSize + WRITE_CHUNK) * sizeof(u8),
				GFP_KERNEL);
	if (finalCmd == NULL) {
		logError(1,
			"%s %s: can not allocate memory for finalCmd ERROR %08X",
			tag,
		      ...
		      byteToRead, int hasDummyByte)
{
	...
	finalCmd = (u8 *)kmalloc((1 + addrSize) * sizeof(u8), GFP_KERNEL);
	if (finalCmd == NULL) {
		logError(1, "%s %s: can not allocate finalCmd ERROR %08X",
			       ...
			       addrSize2, u64 address, u8 *data, int dataSize)
{
	...
	finalCmd1 = (u8 *)kmalloc((1 + addrSize1) * sizeof(u8), GFP_KERNEL);
	if (finalCmd1 == NULL) {
		logError(1, "%s %s: can not allocate finalCmd1 ERROR %08X",
			tag, __func__, ERROR_ALLOC);
		return ERROR_ALLOC;
	}

	finalCmd2 = (u8 *)kmalloc((1 + addrSize2 + WRITE_CHUNK) * sizeof(u8), GFP_KERNEL);
	if (finalCmd2 == NULL) {
		logError(1, "%s %s: can not allocate finalCmd2 ERROR %08X",
			tag, __func__, ERROR_ALLOC);
		kfree(finalCmd1);
		return ERROR_ALLOC;
	}
				   ...
				   int byteToRead, int hasDummyByte)
{
	...
	finalCmd2 = (u8 *)kmalloc((1 + addrSize2) * sizeof(u8), GFP_KERNEL);
	if (finalCmd2 == NULL) {
		logError(1, "%s %s: can not allocate finalCmd2 ERROR %08X",
			tag, __func__, ERROR_ALLOC);
		kfree(finalCmd1);
...
{
	if(check_write_echo_flg == true) {
#if 0
		logError(1,
			 "%s %s Check Echo event = %02X %02X %02X %02X\n",
			 tag, __func__, echo[0], echo[1], echo[2], echo[3]);
#endif
		if(memcmp(check_write_echo_value, echo, check_write_echo_len) == 0) {
			check_write_echo = 1;
			wake_up_interruptible(&check_write_echo_wait);
		}
	}
}
...
int fts_write_wait_echo(u8 *cmd, int cmdLength)
{
	...
	mutex_lock(&wait_write_echo_mutex);

	check_write_echo_len = cmdLength > 4 ? 4 : cmdLength;
	for(i = 0; i < check_write_echo_len; i++) {
		check_write_echo_value[i] = cmd[i];
	}
#if 0
	logError(1,
		 "%s %s set check echo event = %02X %02X %02X %02X\n",
		 tag, __func__, check_write_echo_value[0], check_write_echo_value[1], check_write_echo_value[2], check_write_echo_value[3]);
#endif

	while (retry < I2C_RETRY && (ret < OK || ret2 < OK)) {
		check_write_echo = 0;
		check_write_echo_flg = true;
		...
		if (ret >= 0) {
			...
			logCommunication(I2C_IO_WRITE, cmd, cmdLength);
#endif
			ret2 = wait_event_interruptible_timeout(check_write_echo_wait,
					check_write_echo == 1,
					msecs_to_jiffies(FTS_CHECK_WRITE_ECHO_TIMEOUT_MS));
			check_write_echo = 0;
	...
	check_write_echo_flg = false;

	mutex_unlock(&wait_write_echo_mutex);
