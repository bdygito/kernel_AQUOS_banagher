...
#include "ftsCompensation.h"
	...
	int log_get_log_enable(void) { return log_enable; }

	static int log_event_enable = 0;
	module_param(log_event_enable, int, S_IRUGO | S_IWUSR);
	int log_get_log_event_enable(void) { return log_event_enable; }

	static int log_frame_enable = 0;
	module_param(log_frame_enable, int, S_IRUGO | S_IWUSR);
	int log_get_log_frame_enable(void) { return log_frame_enable; }
...
void logCommunication(int io, u8 *buf, int size)
{
	...
	if(log_communication_enable == 1){
		...
		if(strbuf == NULL){
			logError(1, "%s() alloc error. size = %d\n", __func__, allocSize);
			return;
		}

		sprintf(strbuf, "%s[I2C] %s :", tag, (io == I2C_IO_WRITE ? "W " : " R"));
