...
int enableCustomGesture(u8 gesture_flag)
{
	u8 cmd[10];
	int ret;
	short gesture_area[4];
	u8 custom_gesture_mask = 0x00;
	u8 mask_singletap = 0x01;
	u8 mask_doubletap = 0x02;
	...
	if (gesture_flag & 0x01) {
		/* double-tap */
		custom_gesture_mask |= mask_doubletap;
	}
	if ((gesture_flag >> 1) & 0x01) {
		/* single-tap */
		custom_gesture_mask |= mask_singletap;
	}

	/* write Tap gesture control setting */
	cmd[0] = 0xC1;
	cmd[1] = 0x05;
	cmd[2] = FTS_LPWG_PARAM_GESTURE_CONTROL_SETTING_D_TAP;
	cmd[3] = FTS_LPWG_PARAM_GESTURE_CONTROL_SETTING_D_DOUBLETAP;
	cmd[4] = FTS_LPWG_PARAM_GESTURE_CONTROL_SETTING_T_TAP_MAX;
	cmd[5] = FTS_LPWG_PARAM_GESTURE_CONTROL_SETTING_T_DOUBLETAP_MIN;
	cmd[6] = FTS_LPWG_PARAM_GESTURE_CONTROL_SETTING_T_DOUBLETAP_MAX;
	ret = fts_write_wait_echo(cmd, 7);
	if (ret < OK) {
		logError(1, "%s %s: write Tap gesture control setting failed...ERROR %08X !\n", tag,
			 __func__, ret);
		goto END;
	}

	/* write Tap gesture area setting */
	gesture_area[0] = FTS_LPWG_PARAM_GESTURE_AREA_SETTING_CLIP_X;
	gesture_area[1] = FTS_LPWG_PARAM_GESTURE_AREA_SETTING_CLIP_Y;
	gesture_area[2] = X_AXIS_MAX - FTS_LPWG_PARAM_GESTURE_AREA_SETTING_CLIP_X;
	gesture_area[3] = Y_AXIS_MAX - FTS_LPWG_PARAM_GESTURE_AREA_SETTING_CLIP_Y;

	cmd[0] = 0xC1;
	cmd[1] = 0x06;
	cmd[2] = (gesture_area[0] >> 8) & 0xFF;
	cmd[3] = gesture_area[0] & 0xFF;
	cmd[4] = (gesture_area[1] >> 8) & 0xFF;
	cmd[5] = gesture_area[1] & 0xFF;
	cmd[6] = (gesture_area[2] >> 8) & 0xFF;
	cmd[7] = gesture_area[2] & 0xFF;
	cmd[8] = (gesture_area[3] >> 8) & 0xFF;
	cmd[9] = gesture_area[3] & 0xFF;
	ret = fts_write_wait_echo(cmd, 10);
	if (ret < OK) {
		logError(1, "%s %s: write Tap gesture area setting failed...ERROR %08X !\n", tag,
			 __func__, ret);
		goto END;
	}

	ret = setFeatures(FEAT_SEL_GESTURE, &custom_gesture_mask, 1);
	if (ret < OK) {
		logError(1, "%s enable custom gestures: ERROR %08X\n", tag,
			 ret);
		goto END;
	}
