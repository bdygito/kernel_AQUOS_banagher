...
#define LOGx(log, ...) \
	printk(KERN_DEBUG "[%5d] " log, current->pid, ##__VA_ARGS__)

#define LOGy(log, ...) \
	printk(KERN_DEBUG "[%5d]%s (line %d): " log, current->pid, __func__, __LINE__, ##__VA_ARGS__)

#define LOGE(log, ...) \
	printk(KERN_ERR "[%5d] " log, current->pid, ##__VA_ARGS__)

#ifdef FTS_DEVELOP_MODE_ENABLE
	int log_get_log_enable(void);
	int log_get_log_event_enable(void);
	int log_get_log_frame_enable(void);

	#define logError(force, log, ...)				\
			do {									\
				if(force != 0) {					\
					LOGE(log, ##__VA_ARGS__);		\
				}									\
				else if(log_get_log_enable()) { 	\
					LOGx(log, ##__VA_ARGS__);		\
				}									\
			} while(0)

	#define logTouchEvent(log, ...)					\
			do {									\
				if(log_get_log_event_enable()) {	\
					LOGx(log, ##__VA_ARGS__);		\
				}									\
			} while(0)

	#define logFrame(force, log, ...)					\
			do {										\
				if(force != 0) {						\
					LOGx(log, ##__VA_ARGS__);			\
				}										\
				else if(log_get_log_frame_enable()) {	\
					LOGx(log, ##__VA_ARGS__);			\
				}										\
			} while(0)
#else
...
#endif
#else
void logTouchEvent(const char *msg, ...);
void logError(int force, const char *msg, ...);
#endif
void logCommunication(int io, u8 *buf, int size);
