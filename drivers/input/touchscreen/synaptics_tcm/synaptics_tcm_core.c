...
{
#if defined(SYNAPTICS_TCM_POWER_CONTROL_ENABLE)
	return 0;
#else
	int retval;
	const struct syna_tcm_board_data *bdata = tcm_hcd->hw_if->bdata;
...
static int syna_tcm_enable_regulator(struct syna_tcm_hcd *tcm_hcd, bool en)
{
	...
	if (tcm_hcd->regulator_enable == true) {
		LOGI(tcm_hcd->pdev->dev.parent,
				"already regulator is enabled\n");
		return 0;
	}
	...
	tcm_hcd->regulator_enable = true;

	return 0;

disable_pwr_reg:
	if (tcm_hcd->regulator_enable == false) {
		LOGI(tcm_hcd->pdev->dev.parent,
				"already regulator is disabled\n");
		return 0;
	}
...
disable_bus_reg:
	...
	tcm_hcd->regulator_enable = false;
...
#if defined(SYNAPTICS_TCM_PANEL_CONNECT_CHECK_ENABLE)
static void syna_tcm_panel_check_panel_connected(struct syna_tcm_hcd *tcm_hcd)
{
	if (tcm_hcd->panel_connected == -1) {
		int retval;
		unsigned char *resp_buf;
		unsigned int resp_buf_size;
		unsigned int resp_length;
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Failed to write command %s. pannel not connected.\n",
					STR(CMD_IDENTIFY));

			tcm_hcd->panel_connected = 0;
		}
		else{
			tcm_hcd->panel_connected = 1;
		}
...
#endif /* SYNAPTICS_TCM_PANEL_CONNECT_CHECK_ENABLE */
...
#if defined(SYNAPTICS_TCM_PANEL_CONNECT_CHECK_ENABLE)
	if (tcm_hcd->panel_connected == 0) {
		LOGI(tcm_hcd->pdev->dev.parent,
				"syna_tcm_resume() panel not connected. end\n");
		return 0;
	}
#endif /* SYNAPTICS_TCM_PANEL_CONNECT_CHECK_ENABLE */
...
//		LOGI(tcm_hcd->pdev->dev.parent, "changed irq-gpio state active\n");
...
#if defined(SYNAPTICS_TCM_PANEL_CONNECT_CHECK_ENABLE)
	syna_tcm_panel_check_panel_connected(tcm_hcd);
	...
	if (tcm_hcd->panel_connected == 0) {
		...
		if (tcm_hcd->hw_if->bdata->reset_gpio >= 0) {
			gpio_set_value(tcm_hcd->hw_if->bdata->reset_gpio, tcm_hcd->hw_if->bdata->reset_on_state);
		}
#endif
...
static int syna_tcm_tpin_enable_check(struct syna_tcm_hcd *tcm_hcd)
{
	#ifdef CONFIG_OF
		const struct syna_tcm_board_data *bdata = tcm_hcd->hw_if->bdata;
		int val;

		if(bdata->test_mode_gpio == -1){
			LOGE(tcm_hcd->pdev->dev.parent,
					"test_mode_gpio not set.\n");
			return -1;
		}

		gpio_request(bdata->test_mode_gpio, "synaptics_tcm_tpin");
		pinctrl_select_state(tcm_hcd->pinctrl, tcm_hcd->tpin_state_active);
		mb();
		udelay(10);
		val = gpio_get_value(bdata->test_mode_gpio);
		LOGI(tcm_hcd->pdev->dev.parent,
				"gpio_get_value() val = %d\n", val);
		pinctrl_select_state(tcm_hcd->pinctrl, tcm_hcd->tpin_state_suspend);
		gpio_free(bdata->test_mode_gpio);
...
static int syna_tcm_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(tcm_hcd->pinctrl)) {
		...
		tcm_hcd->tpin_state_active =
				pinctrl_lookup_state(tcm_hcd->pinctrl, "test_mode_pull_up");
		if (IS_ERR_OR_NULL(tcm_hcd->tpin_state_active)) {
			LOGE(&pdev->dev, "pinctrl lookup failed for test_mode_pull_up\n");
			tcm_hcd->tpin_state_active = NULL;
		}
		tcm_hcd->tpin_state_suspend =
				pinctrl_lookup_state(tcm_hcd->pinctrl, "test_mode_pull_down");
		if (IS_ERR_OR_NULL(tcm_hcd->tpin_state_suspend)) {
			LOGE(&pdev->dev, "pinctrl lookup failed for test_mode_pull_down\n");
			tcm_hcd->tpin_state_suspend = NULL;
		}
	}
	...
	#endif
	#if defined(CONFIG_TOUCHSCREEN_SYNAPTICS_TCM_ZEROFLASH)
		zeroflash_module_init();
	#endif
	#if defined(CONFIG_TOUCHSCREEN_SYNAPTICS_TCM_DIAGNOSTICS)
		diag_module_init();
	#endif
	#if defined(CONFIG_TOUCHSCREEN_SYNAPTICS_TCM_ROMBOOT)
		romboot_module_init();
	#endif
...
#if 1
	if (syna_tcm_tpin_enable_check(tcm_hcd) == 0) {
		syna_tcm_resume(&tcm_hcd->pdev->dev);
	}
#if defined(SYNAPTICS_TCM_PANEL_CONNECT_CHECK_ENABLE)
	else{
