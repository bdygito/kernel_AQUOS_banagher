...
static int parse_dt(struct device *dev, struct syna_tcm_board_data *bdata)
{
	...
	prop = of_find_property(np, "synaptics,power-delay-us", NULL);
	if (prop && prop->length) {
		retval = of_property_read_u32(np, "synaptics,power-delay-us",
				&value);
		if (retval < 0) {
			LOGE(dev,
					"Failed to read synaptics,power-delay-us property\n");
			return retval;
		} else {
			bdata->power_delay_us = value;
		}
	} else {
		bdata->power_delay_us = 0;
	}

	prop = of_find_property(np, "synaptics,bus-delay-us", NULL);
	if (prop && prop->length) {
		retval = of_property_read_u32(np, "synaptics,bus-delay-us",
				&value);
		if (retval < 0) {
			LOGE(dev,
					"Failed to read synaptics,bus-delay-us property\n");
			return retval;
		} else {
			bdata->bus_delay_us = value;
		}
	} else {
		bdata->bus_delay_us = 0;
	}
	...
	prop = of_find_property(np, "synaptics,test-mode-gpio", NULL);
	if (prop && prop->length) {
		bdata->test_mode_gpio = of_get_named_gpio_flags(np,
				"synaptics,test-mode-gpio", 0, NULL);
	} else {
		bdata->test_mode_gpio = -1;
	}
