...
#ifdef CYPRESS_CYTTSP5_SEND_SHTERM_EVENT_ENABLE
	#include <misc/shterm_k.h>
#endif /* CYPRESS_CYTTSP5_SEND_SHTERM_EVENT_ENABLE */

#ifdef CONFIG_DRM_PANEL
struct drm_panel *cyttsp5_active_panel = NULL;
#endif
...
#if defined(CYPRESS_CYTTSP5_LPMODE_ENABLE)
#define CYPRESS_CYTTSP5_LPMODE_TYPE_NON_CONTINUOUS	0
#define CYPRESS_CYTTSP5_LPMODE_TYPE_CONTINUOUS		1

#define CYPRESS_CYTTSP5_LPMODE_REQ_NONE				0x00
#define CYPRESS_CYTTSP5_LPMODE_REQ_COMMON			0x01
#define CYPRESS_CYTTSP5_LPMODE_REQ_ECO				0x02

static void cyttsp5_set_lpmode_init(struct cyttsp5_core_data *cd);
static void cyttsp5_lpmode_set_state(struct cyttsp5_core_data *cd, int type, int req, int on);
#endif /* CYPRESS_CYTTSP5_LPMODE_ENABLE */
#if defined(CYPRESS_CYTTSP5_GLOVE_ENABLE)
static int cyttsp5_set_glove_detect_enable(struct cyttsp5_core_data *cd);
static int cyttsp5_set_glove_detect_disable(struct cyttsp5_core_data *cd);
static int cyttsp5_set_glove_state(struct cyttsp5_core_data *cd, int on);
static int cyttsp5_set_glove_detect_init(struct cyttsp5_core_data *cd);
#endif /* CYPRESS_CYTTSP5_GLOVE_ENABLE */
#if defined(CYPRESS_CYTTSP5_LPWG_ENABLE)
static void cyttsp5_set_lpwg_on(struct cyttsp5_core_data *cd);
static void cyttsp5_set_lpwg_off(struct cyttsp5_core_data *cd);
static u8 cyttsp5_get_lpwg_state(struct cyttsp5_core_data *cd);
static int cyttsp5_lpwg_set_state(struct cyttsp5_core_data *cd, int on);
#endif /* CYPRESS_CYTTSP5_LPWG_ENABLE */
#if defined(CYPRESS_CYTTSP5_CHARGER_ARMOR_ENABLE)
static void cyttsp5_set_charger_armor_on(struct cyttsp5_core_data *cd);
static void cyttsp5_set_charger_armor_off(struct cyttsp5_core_data *cd);
static int cyttsp5_set_charger_armor_state(struct cyttsp5_core_data *cd, int on);
static void cyttsp5_set_charger_armor_init(struct cyttsp5_core_data *cd);
#endif /* CYPRESS_CYTTSP5_CHARGER_ARMOR_ENABLE */
		...
		u16 row_number, u16 length, u8 ebid, u8 *read_buf, u16 *crc)
{
	...
	if (length >= read_length)
		memcpy(read_buf, &cd->response_buf[10], read_length);
	else
		return -EPROTO;
#else
	if (length < read_length)
...
#if 1
	cancel_delayed_work(&cd->watchdog_timer);
	schedule_delayed_work(&cd->watchdog_timer, 
...
static void cyttsp5_watchdog_timer(struct work_struct *work)
{
	struct delayed_work *dw = container_of(work, struct delayed_work, work);
	struct cyttsp5_core_data *cd = container_of(dw, struct cyttsp5_core_data, watchdog_timer);
...
#if defined(CYPRESS_CYTTSP5_LPMODE_ENABLE)
	cd->lpmode_req_state = CYPRESS_CYTTSP5_LPMODE_REQ_NONE;
#endif /* CYPRESS_CYTTSP5_LPMODE_ENABLE */
#if defined(CYPRESS_CYTTSP5_GLOVE_ENABLE)
	if(cd->glove_enable_state != 0){
		cyttsp5_set_glove_detect_disable(cd);
	}
#endif /* CYPRESS_CYTTSP5_GLOVE_ENABLE */
#if defined(CYPRESS_CYTTSP5_LPWG_ENABLE)
	if (cd->lpwg_req_state != 0) {
		cyttsp5_set_lpwg_on(cd);
	}
#endif /* CYPRESS_CYTTSP5_LPWG_ENABLE */
	...
	if (!IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture)) {
		if (cd->sleep_state == SS_SLEEP_ON) {
			...
			if (!dev->power.is_suspended) {
				parade_debug(dev, DEBUG_LEVEL_2, "%s: dev->power.is_suspended=false\n",
					__func__);
				goto read;
			}
			parade_debug(dev, DEBUG_LEVEL_2, "%s: wait_event_timeout start\n",
				__func__);
			t = wait_event_timeout(cd->wait_q,
					(cd->lpwg_system_suspended == false),
					msecs_to_jiffies(2000));
			if (IS_TMO(t)) {
				parade_debug(dev, DEBUG_LEVEL_2, "%s: wait_event_timeout timeout end\n",
					__func__);
				return -EAGAIN;
			}
			parade_debug(dev, DEBUG_LEVEL_2, "%s: wait_event_timeout end\n",
				__func__);
#else
			if (!dev->power.is_suspended)
				goto read;
...
static int cyttsp5_core_wake_(struct cyttsp5_core_data *cd)
{
	...
	if (cd->lpwg_wake_lock_on == true) {
		...
		parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: wake_unlock(lpwg_wake_lock)\n",
			__func__);
	...
	if (cd->lpwg_cur_state != 0) {
		cyttsp5_set_lpwg_off(cd);
	}
#endif /* CYPRESS_CYTTSP5_LPWG_ENABLE */
#if defined(CYPRESS_CYTTSP5_GLOVE_ENABLE)
	cyttsp5_set_glove_detect_init(cd);
#endif /* CYPRESS_CYTTSP5_GLOVE_ENABLE */
#if defined(CYPRESS_CYTTSP5_CHARGER_ARMOR_ENABLE)
	cyttsp5_set_charger_armor_init(cd);
#endif /* CYPRESS_CYTTSP5_CHARGER_ARMOR_ENABLE */
...
static int cyttsp5_core_suspend(struct device *dev)
{
	...
	if (!IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture)) {
		parade_debug(cd->dev, DEBUG_LEVEL_2, "%s: set cd->lpwg_system_suspended = true\n",
			__func__);
...
static int cyttsp5_core_resume(struct device *dev)
{
	...
	if (!IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture)) {
		parade_debug(cd->dev, DEBUG_LEVEL_2, "%s: set cd->lpwg_system_suspended = false\n",
			__func__);
		...
		parade_debug(cd->dev, DEBUG_LEVEL_2, "%s: wake_up(&cd->wait_q)\n",
			__func__);
		wake_up(&cd->wait_q);
	}
#endif /* CYPRESS_CYTTSP5_LPWG_ENABLE */
...
#if defined(CYPRESS_CYTTSP5_LPMODE_ENABLE)
static void cyttsp5_set_lpmode_on(struct cyttsp5_core_data *cd)
{
	int retval = 0;

	retval |= cyttsp5_hid_output_set_param_(cd, 0x1C, 0x0064, 2);
	retval |= cyttsp5_hid_output_set_param_(cd, 0x06, 0x01, 1);
	if (retval != 0) {
		dev_err(cd->dev, "%s: Failed to set doze mode ON\n",
				__func__);
	}
}
static void cyttsp5_set_lpmode_off(struct cyttsp5_core_data *cd)
{
	int retval = 0;

	retval |= cyttsp5_hid_output_set_param_(cd, 0x06, 0x00, 1);
	if (retval != 0) {
		dev_err(cd->dev, "%s: Failed to set doze mode OFF\n",
				__func__);
	}
}
static u8 cyttsp5_get_lpmode_state(struct cyttsp5_core_data *cd)
{
	u8 req = (cd->lpmode_req_state | cd->lpmode_continuous_req_state);

	if(req == CYPRESS_CYTTSP5_LPMODE_REQ_NONE){
		return 1;
	}
	return 0;
}
static void cyttsp5_set_lpmode_init(struct cyttsp5_core_data *cd)
{
	if(cyttsp5_get_lpmode_state(cd) != 0){
		cyttsp5_set_lpmode_off(cd);
	}else{
		cyttsp5_set_lpmode_on(cd);
	}
}
static void cyttsp5_lpmode_set_state(struct cyttsp5_core_data *cd, int type, int req, int on)
{
	...
	if(on){
		if(type == CYPRESS_CYTTSP5_LPMODE_TYPE_NON_CONTINUOUS){
			if((cd->lpmode_req_state & req) == 0){
				cd->lpmode_req_state |= req;
				changed = 1;
				parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: [LPMODE]<ON> type = NON CONTINUOUS, req = %s\n",
										__func__,
										(req == CYPRESS_CYTTSP5_LPMODE_REQ_COMMON)? "common" :
										(req == CYPRESS_CYTTSP5_LPMODE_REQ_ECO)? "eco" : "unknown");
			}
		}else{
			if((cd->lpmode_continuous_req_state & req) == 0){
				cd->lpmode_continuous_req_state |= req;
				changed = 1;
				parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: [LPMODE]<ON> type = CONTINUOUS, req = %s\n",
										__func__,
										(req == CYPRESS_CYTTSP5_LPMODE_REQ_COMMON)? "common" :
										(req == CYPRESS_CYTTSP5_LPMODE_REQ_ECO)? "eco" : "unknown");
			}
		...
		if(type == CYPRESS_CYTTSP5_LPMODE_TYPE_NON_CONTINUOUS){
			if((cd->lpmode_req_state & req) != 0){
				cd->lpmode_req_state &= ~req;
				changed = 1;
				parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: [LPMODE]<OFF> type = NON CONTINUOUS, req = %s\n",
										__func__,
										(req == CYPRESS_CYTTSP5_LPMODE_REQ_COMMON)? "common" :
										(req == CYPRESS_CYTTSP5_LPMODE_REQ_ECO)? "eco" : "unknown");
			}
			...
			if((cd->lpmode_continuous_req_state & req) != 0){
				cd->lpmode_continuous_req_state &= ~req;
				changed = 1;
				parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: [LPMODE]<OFF> type = CONTINUOUS, req = %s\n",
										__func__,
										(req == CYPRESS_CYTTSP5_LPMODE_REQ_COMMON)? "common" :
										(req == CYPRESS_CYTTSP5_LPMODE_REQ_ECO)? "eco" : "unknown");
			}
...
#if defined(CYPRESS_CYTTSP5_GLOVE_ENABLE)
static int cyttsp5_fwctl_glove_enable(struct cyttsp5_core_data *cd)
{
	int retval;

	retval = cyttsp5_hid_output_set_param_(cd, 0x02, 0x03, 1);
	if (retval != 0) {
		dev_err(cd->dev,
				"Failed to set glove mode ON");
	}
	if (CYPRESS_CYTTSP5_GLOVE_GRIP_ENABLE != 0) {
		cyttsp5_hid_output_set_param_(cd, 0x0E, CYPRESS_CYTTSP5_GLOVE_GRIP_XEDG, 2);
		cyttsp5_hid_output_set_param_(cd, 0x0F, CYPRESS_CYTTSP5_GLOVE_GRIP_XEDG, 2);
		cyttsp5_hid_output_set_param_(cd, 0x16, 0, 1);
		cyttsp5_hid_output_set_param_(cd, 0x18, 1, 1);
	}

	return retval;
}
static int cyttsp5_fwctl_glove_disable(struct cyttsp5_core_data *cd)
{
	int retval;

	retval = cyttsp5_hid_output_set_param_(cd, 0x02, 0x01, 1);
	if (retval != 0) {
		dev_err(cd->dev,
				"Failed to set glove mode OFF");
	}
	if (CYPRESS_CYTTSP5_GLOVE_GRIP_ENABLE != 0) {
		cyttsp5_hid_output_set_param_(cd, 0x18, 0, 1);
	}
...
static int cyttsp5_set_glove_detect_enable(struct cyttsp5_core_data *cd)
{
	...
	if(req == 1){
		rc = cyttsp5_fwctl_glove_enable(cd);
...
static int cyttsp5_set_glove_detect_disable(struct cyttsp5_core_data *cd)
{
	...
	rc = cyttsp5_fwctl_glove_disable(cd);
	return rc;
}
...
static int cyttsp5_set_glove_state(struct cyttsp5_core_data *cd, int on)
{
	...
	if(on){
		if(cd->glove_enable_state == 0){
			...
			if (cd->sleep_state == SS_SLEEP_OFF) {
				cyttsp5_set_glove_detect_enable(cd);
			}
		}
	}
	else{
...
static int cyttsp5_set_glove_detect_init(struct cyttsp5_core_data *cd)
{
	...
	if(cd->glove_enable_state == 0){
		rc = cyttsp5_set_glove_detect_disable(cd);
	}else{
		rc = cyttsp5_set_glove_detect_enable(cd);
	}
...
static void cyttsp5_set_lpwg_on(struct cyttsp5_core_data *cd)
{
	...
	retval = 0;
	cd->easy_wakeup_gesture = 0x01;
	if (retval != 0) {
		dev_err(cd->dev, "%s: Failed to set lpwg mode ON\n", __func__);
	}
	else{
		cd->lpwg_cur_state = 1;
	}
...
static void cyttsp5_set_lpwg_off(struct cyttsp5_core_data *cd)
{
	...
	if (retval != 0) {
		dev_err(cd->dev, "%s: Failed to set lpwg mode OFF\n", __func__);
	}
	else{
		cd->lpwg_cur_state = 0;
	}
...
static int cyttsp5_lpwg_set_state(struct cyttsp5_core_data *cd, int on)
{
	...
	if(on){
		if(cd->lpwg_req_state == 0){
			cd->lpwg_req_state = 1;
			changed = 1;
			parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: [lpwg]<ON>\n",
				__func__);
		}
	}
	else{
		if(cd->lpwg_req_state != 0){
			cd->lpwg_req_state = 0;
			changed = 1;
			parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: [lpwg]<OFF>\n",
				__func__);
		}
	}
	...
	if(changed){
		if (cd->sleep_state == SS_SLEEP_ON) {
			if(cyttsp5_get_lpwg_state(cd) == 0 && cd->lpwg_req_state == 1){
				cyttsp5_core_wake_(cd);
				cyttsp5_core_sleep_(cd);
			}
			else if(cyttsp5_get_lpwg_state(cd) != 0 && cd->lpwg_req_state == 0){
				cyttsp5_set_lpwg_off(cd);
	...
	parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: [lpwg]lpwg request recieved. req_state = 0x%02x\n",
...
static void cyttsp5_lpwg_wake_lock_timer(struct work_struct *work)
{
	struct delayed_work *dw = container_of(work, struct delayed_work, work);
	struct cyttsp5_core_data *cd = container_of(dw, struct cyttsp5_core_data, lpwg_wake_lock_timer);
...
static void cyttsp5_set_charger_armor_on(struct cyttsp5_core_data *cd)
{
	...
	retval = cyttsp5_hid_output_set_param_(cd, 0x04, 0x01, 1);
	if (retval != 0) {
		dev_err(cd->dev,
...
static void cyttsp5_set_charger_armor_off(struct cyttsp5_core_data *cd)
{
	...
	retval = cyttsp5_hid_output_set_param_(cd, 0x04, 0x00, 1);
	if (retval != 0) {
		dev_err(cd->dev,
...
static int cyttsp5_set_charger_armor_state(struct cyttsp5_core_data *cd, int on)
{
	...
	if(on){
		if(cd->charger_armor_req_state == 0){
			...
			parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: [charger_armor]<ON>\n",
				__func__);

			if (cd->sleep_state == SS_SLEEP_OFF) {
				cyttsp5_set_charger_armor_on(cd);
			}
		}
	}
	else{
		if(cd->charger_armor_req_state != 0){
			...
			parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: [charger_armor]<OFF>\n",
				__func__);

			if (cd->sleep_state == SS_SLEEP_OFF) {
				cyttsp5_set_charger_armor_off(cd);
			}
		}
	}
...
#if defined(CYPRESS_CYTTSP5_LPMODE_ENABLE)
static ssize_t cyttsp5_sysfs_lpmode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int input;
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);

	parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: start\n", __func__);

	if (sscanf(buf, "%u", &input) != 1)
		return -EINVAL;

	cyttsp5_lpmode_set_state(cd, CYPRESS_CYTTSP5_LPMODE_TYPE_NON_CONTINUOUS, CYPRESS_CYTTSP5_LPMODE_REQ_COMMON, input);

	parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: end\n", __func__);

	return count;
}
static ssize_t cyttsp5_sysfs_lpmode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
...
static ssize_t cyttsp5_sysfs_continuous_lpmode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int input;
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	...
	cyttsp5_lpmode_set_state(cd, CYPRESS_CYTTSP5_LPMODE_TYPE_CONTINUOUS, CYPRESS_CYTTSP5_LPMODE_REQ_ECO, input);

	parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: end\n", __func__);

	return count;
}
static ssize_t cyttsp5_sysfs_continuous_lpmode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
...
#if defined(CYPRESS_CYTTSP5_GLOVE_ENABLE)
static ssize_t cyttsp5_sysfs_glove_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	unsigned long value;
	...
	rc = cyttsp5_set_glove_state(cd, value);
	if (rc < 0) {
		dev_err(dev, "%s: error. rc=%d\n", __func__, rc);
		return -EINVAL;
	}
...
static ssize_t cyttsp5_sysfs_glove_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
...
#if defined(CYPRESS_CYTTSP5_LPWG_ENABLE)
static ssize_t cyttsp5_sysfs_lpwg_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	unsigned long value;
	...
	rc = cyttsp5_lpwg_set_state(cd, value);
	if (rc < 0) {
		dev_err(dev, "%s: error. rc=%d\n", __func__, rc);
		return -EINVAL;
	}
...
static ssize_t cyttsp5_sysfs_lpwg_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
...
#ifdef CYPRESS_CYTTSP5_SEND_SHTERM_EVENT_ENABLE
static ssize_t cyttsp5_sysfs_battlog_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int input;
...
#if defined(CYPRESS_CYTTSP5_CHARGER_ARMOR_ENABLE)
static ssize_t cyttsp5_sysfs_charger_armor_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	unsigned long value;
	...
	rc = cyttsp5_set_charger_armor_state(cd, value);
	if (rc < 0) {
		dev_err(dev, "%s: error. rc=%d\n", __func__, rc);
		return -EINVAL;
	}
...
static ssize_t cyttsp5_sysfs_charger_armor_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
...
#if defined(CYPRESS_CYTTSP5_LPMODE_ENABLE)
	__ATTR(lpmode, (S_IRUGO | S_IWUSR | S_IWGRP), cyttsp5_sysfs_lpmode_show,
		cyttsp5_sysfs_lpmode_store),
	__ATTR(continuous_lpmode, (S_IRUGO | S_IWUSR | S_IWGRP), cyttsp5_sysfs_continuous_lpmode_show,
		cyttsp5_sysfs_continuous_lpmode_store),
#endif /* CYPRESS_CYTTSP5_LPMODE_ENABLE */
#if defined(CYPRESS_CYTTSP5_GLOVE_ENABLE)
	__ATTR(glove, (S_IRUGO | S_IWUSR | S_IWGRP), cyttsp5_sysfs_glove_show,
		cyttsp5_sysfs_glove_store),
#endif /* CYPRESS_CYTTSP5_GLOVE_ENABLE */
#if defined(CYPRESS_CYTTSP5_LPWG_ENABLE)
	__ATTR(lpwg, (S_IRUGO | S_IWUSR | S_IWGRP), cyttsp5_sysfs_lpwg_show,
		cyttsp5_sysfs_lpwg_store),
#endif /* CYPRESS_CYTTSP5_LPWG_ENABLE */
#ifdef CYPRESS_CYTTSP5_SEND_SHTERM_EVENT_ENABLE
	__ATTR(battlog, (S_IWUSR | S_IWGRP), NULL,
		cyttsp5_sysfs_battlog_store),
#endif /* CYPRESS_CYTTSP5_SEND_SHTERM_EVENT_ENABLE */
#if defined(CYPRESS_CYTTSP5_CHARGER_ARMOR_ENABLE)
	__ATTR(charger_armor, (S_IRUGO | S_IWUSR | S_IWGRP), cyttsp5_sysfs_charger_armor_show,
		cyttsp5_sysfs_charger_armor_store),
#endif /* CYPRESS_CYTTSP5_CHARGER_ARMOR_ENABLE */
};
...
#if 1
static int cyttsp5_tpin_enable_check(struct cyttsp5_core_platform_data *pdata, struct device *dev)
{
	#ifdef CONFIG_OF
		int val;
		...
		if(pdata->test_mode_gpio == -1){
			...
			gpio_request(pdata->test_mode_gpio, "cyttsp5_tpin");
...
#ifdef CONFIG_DRM_PANEL
int cyttsp5_drm_notifier_cb(struct notifier_block *self, unsigned long event, void *data)
{
	struct drm_panel_notifier *evdata = data;
	int blank;
	struct cyttsp5_core_data *cd =
			container_of(self, struct cyttsp5_core_data, drm_notifier);
...
#if defined(CYPRESS_CYTTSP5_ALWAYS_ON_DISPLAY_ENABLE)
		if (event == DRM_PANEL_EVENT_DPMS) {
			parade_debug(cd->dev, DEBUG_LEVEL_0, "%s: start. event=%d blank=%d\n", __func__, event, *(int*)evdata->data);
			...
			parade_debug(cd->dev, DEBUG_LEVEL_0, "%s: end. event=%d blank=%d\n", __func__, event, *(int*)evdata->data);
		}
#else
...
#endif /* CYPRESS_CYTTSP5_ALWAYS_ON_DISPLAY_ENABLE */
	}
		...
		u16 irq, size_t xfer_buf_size)
{
...
#if 1
	rc = cyttsp5_tpin_enable_check(cd->cpdata, cd->dev);
	if (rc) {
		rc = -ENODEV;
		goto error_detect;
	}
...
#if 1
	INIT_DELAYED_WORK(&cd->watchdog_timer, cyttsp5_watchdog_timer);
#else
	...
	cd->lpwg_wake_lock_on = false;
	rc = wake_lock_init(&cd->lpwg_wake_lock, cd->dev, "cyttsp5_lpwg_wake_lock");
	if (rc) {
		dev_err(dev, "%s: Error, could not setup lpwg_wake_lock\n", __func__);
		goto error_setup_lpwg_wake_lock;
	}
	INIT_DELAYED_WORK(&cd->lpwg_wake_lock_timer, cyttsp5_lpwg_wake_lock_timer);
#endif /* CYPRESS_CYTTSP5_LPWG_ENABLE */
...
#ifdef CONFIG_DRM_PANEL
	cd->drm_notifier.notifier_call = cyttsp5_drm_notifier_cb;
	if (cyttsp5_active_panel) {
		rc = drm_panel_notifier_register(cyttsp5_active_panel,
				&cd->drm_notifier);
		if (rc < 0) {
			dev_err(dev,
		...
		dev_err(dev,
				"%s: Failed to get cyttsp5_active_panel\n",
				__func__);
	}
#endif
...
#ifdef CONFIG_DRM_PANEL
	if (cyttsp5_active_panel) {
		drm_panel_notifier_unregister(cyttsp5_active_panel, &cd->drm_notifier);
	}
#endif
...
#ifdef CONFIG_DRM_PANEL
int cyttsp5_set_active_panel(struct drm_panel *panel)
{
	cyttsp5_active_panel = panel;
	return 0;
}
EXPORT_SYMBOL_GPL(cyttsp5_set_active_panel);
#endif
