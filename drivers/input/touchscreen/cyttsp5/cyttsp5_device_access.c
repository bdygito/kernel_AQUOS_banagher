...
#ifdef CYPRESS_CYTTSP5_SYSFS_CALIBRATE
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	length = cyttsp5_ic_parse_input(dad->dev, buf, size,
...
put_pm_runtime:
	...
	if (cd->show_timestamp) {
		...
		index += scnprintf(buf, PAGE_SIZE,
			"[%u] ", jiffies_to_msecs(jiffies));
		length = prepare_print_buffer(status,
			&dad->ic_buf[5], length, &buf[index],
			PAGE_SIZE-index);
	} else {
		length = prepare_print_buffer(status,
			&dad->ic_buf[5], length, buf,
			PAGE_SIZE);
	}
...
static DEVICE_ATTR(panel_scan, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP, cyttsp5_panel_scan_show, cyttsp5_panel_scan_store);

static ssize_t cyttsp5_cm_panel_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_device_access_data *dad
		= cyttsp5_get_device_access_data(dev);
	...
	index = cyttsp5_run_and_get_selftest_result(
		dev, buf, PAGE_SIZE,
		CY_ST_ID_CM_PANEL, PIP_CMD_MAX_LENGTH, true);
...
static DEVICE_ATTR(cm_panel, S_IRUSR | S_IRGRP,
	cyttsp5_cm_panel_show, NULL);

static ssize_t cyttsp5_cp_panel_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_device_access_data *dad
		= cyttsp5_get_device_access_data(dev);
...
static DEVICE_ATTR(cp_panel, S_IRUSR | S_IRGRP,
	cyttsp5_cp_panel_show, NULL);
#endif /* CYPRESS_CYTTSP5_SYSFS_CALIBRATE */
...
static int cyttsp5_setup_debugfs(struct device *dev)
{
	int rc;
	struct cyttsp5_device_access_data *dad
		= cyttsp5_get_device_access_data(dev);
	...
	if (IS_ERR_OR_NULL(dad->base_dentry)) {
		dev_err(dev, "%s: Error, could not create debugfs base directory\n",
				__func__);
		goto exit;
	}
