...
#ifdef CONFIG_SENSORS_FPRINT_SHARP_EGIS_NAVIGATION_SUPPORT
...
#endif /* CONFIG_SENSORS_FPRINT_SHARP_EGIS_NAVIGATION_SUPPORT */
...
long egisfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FP_POWER_ONOFF:
		...
		DEBUG_PRINT(" %s : FP_POWER_ONOFF is disabled \n", __func__);
#else
		if (copy_from_user(&data, (int __user *)arg, sizeof(data)))
		{
			return -EFAULT;
...
int egisfp_platforminit(struct egisfp_dev_t *egis_dev)
{
	...
	if (egis_dev != NULL)
	{
		if (!egis_dev->pars_dtsi_done)
		{
			ERROR_PRINT(" %s : need pars dtsi first \n", __func__);
			return -EIO;
		}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	INFO_PRINT(" %s : count=%d\n", __func__, (int)count);
	if (!strncmp(buf, "enable", strlen("enable"))) {
		ioctl_data.int_mode = 1;
		ret = do_egisfp_power_onoff(egis_dev, &ioctl_data);
	} else if (!strncmp(buf, "disable", strlen("disable"))) {
		ioctl_data.int_mode = 0;
		ret = do_egisfp_power_onoff(egis_dev, &ioctl_data);
	} else {
		ERROR_PRINT(" %s : -EINVAL\n", __func__);
		return -EINVAL;
	}
...
#ifdef CONFIG_SENSORS_FPRINT_SHARP_PRE_EVT
	struct egisfp_ioctl_cmd_t ioctl_data = {0};
#endif /* CONFIG_SENSORS_FPRINT_SHARP_PRE_EVT */
...
/* Initialize the driver data */
	...
	/* platform init */
	status = egisfp_platforminit(egis_dev);
	if (status < 0)
	{
		ERROR_PRINT(" %s : egisfp_platforminit fail, status = %d \n", __func__, status);
	}
#endif /* CONFIG_SENSORS_FPRINT_SHARP_CUSTOMIZE */
	/*
	...
	if (status) {
		ERROR_PRINT(" %s : could not create sysfs \n");
		goto egistec_probe_failed;
	}
#endif /* CONFIG_SENSORS_FPRINT_SHARP_CUSTOMIZE */
...
#ifdef CONFIG_SENSORS_FPRINT_SHARP_PRE_EVT
	if (of_property_read_bool(egis_dev->dd->dev.of_node, "egistec,enable-on-boot")) {
		INFO_PRINT(" %s : Enabling hardware\n", __func__);
		ioctl_data.int_mode = 1;
		do_egisfp_power_onoff(egis_dev, &ioctl_data);
	}
	msleep(5);
	do_egisfp_reset(egis_dev);
#endif /* CONFIG_SENSORS_FPRINT_SHARP_PRE_EVT */
