...
//#define STMVL53L1_SLAVE_ADDR        0x52
/*
...
static void stmvl53l1_release_xsdn(struct tof_ctrl_t *tof_ctrl)
{
	if (tof_ctrl->io_flag.xsdn_owned) {
		("release xsdn_gpio %d", tof_ctrl->xsdn_gpio);
		gpio_free(tof_ctrl->xsdn_gpio);
		tof_ctrl->io_flag.xsdn_owned = 0;
	}
...
static int stmvl53l1_request_pwren(struct tof_ctrl_t *tof_ctrl)
{
	...
	("request pwren_gpio %d", tof_ctrl->pwren_gpio);
...
static void stmvl53l1_release_pwren(struct tof_ctrl_t *tof_ctrl)
{
	if (tof_ctrl->io_flag.pwr_owned) {
		("release pwren_gpio %d", tof_ctrl->pwren_gpio);
...
#else
/*avdd get OK*/
	pr_err("%s %d nsq regulator_get 1 avdd\n",__func__,__LINE__);
...
/*iovdd*/
	...
	/*iovdd*/
    	...
    	//rc = of_property_read_string(client->dev.of_node, "iovdd-supply", &name);
	if (rc == -EINVAL)
		iovdd_reg_name = NULL;
	else if (rc < 0)
		return rc;
    ...
    if ((iovdd_reg_name != NULL) && (*iovdd_reg_name != 0)) {
        ...
        rc = regulator_set_voltage(bus_reg,
                1800000, 1800000);
...
static int32_t stmvl53l1_platform_probe(struct platform_device *pdev)
{
	...
	pr_err("%s %d nsq Enter,call cam_cci_get_subdev\n",__func__,__LINE__);
	if (!cam_cci_get_subdev(0)){
		pr_err("%s %d nsq,cam_cci_get_subdev\n",__func__,__LINE__);
		return -EPROBE_DEFER;
	}
	...
	*/
	...
	cci_client->sid = STMVL53L1_SLAVE_ADDR;
	...
	*/
	...
	/* setup other stuff */
	pr_err("%s %d nsq call stmvl53l1_setup\n",__func__,__LINE__);
	rc = stmvl53l1_setup(vl53l1_data);
	if (rc) {
		pr_err("%s %d nsq stmvl53l1_setup,rc:%d\n", __func__,__LINE__,rc);
		goto release_gpios;
	}else
		pr_err("%s %d nsq stmvl53l1_setup,rc:%d\n", __func__,__LINE__,rc);
...
int stmvl53l1_power_up_cci(void *object)
{
	...
	if (tof_ctrl->power_supply) {
		pr_err("%s %d regulator_enable power_supply\n",__func__,__LINE__);
		...
		if (rc) {
			pr_err("%s %d fail to turn on regulator power_supply,rc:%d\n",__func__,__LINE__,rc);
		}
		pr_err("%s %d regulator_enable cci_supply\n",__func__,__LINE__);
		rc |= regulator_enable(tof_ctrl->cci_supply);
		if (rc) {
			pr_err("%s %d fail to turn on regulator cci_supply,rc:%d\n",__func__,__LINE__,rc);
			return rc;
		}
	} 
#ifdef xxx
		...
		pr_err("%s %d slow power on\n",__func__,__LINE__);
	}
#endif
	else {
		pr_err("%s %d no power control\n",__func__,__LINE__);
 	}

	pr_err("%s %d call camera_io_init master_info\n",__func__,__LINE__);
	rc = camera_io_init(&tof_ctrl->io_master_info);
	if (rc < 0){
		pr_err("%s %d cci init failed: rc:%d\n",__func__,__LINE__,rc);
	}
...
int stmvl53l1_power_down_cci(void *cci_object)
{
	...
	if (tof_ctrl->power_supply) {
		...
		rc = regulator_disable(tof_ctrl->power_supply);
		if (rc){
			pr_err("%s %d power_supply isable failed. rc=%d\n",
				__func__,__LINE__,rc);
		}
		rc = regulator_disable(tof_ctrl->cci_supply);
		if (rc){
			pr_err("%s %d cci_supply disable failed. rc=%d\n",
				__func__,__LINE__,rc);
		}
	}
	#ifdef xxx 
	...
	pr_err("%s %d call camera_io_release: rc:%d\n",__func__,__LINE__,rc);
...
static void stmvl53l1_work_handler_cci(struct work_struct *work)
{
	...
	pr_err("stmvl53l1_work_handler_cci %d", rc);
...
int stmvl53l1_reset_release_cci(void *object)
{
	...
	pr_err("%s %d call VL53L1_WaitDeviceBooted 1,rc:%d\n",__func__,__LINE__,rc);
/*NSQ add start*/
	//rc=0;
/*NSQ add end*/
	pr_err("%s %d call VL53L1_WaitDeviceBooted 2,rc:%d\n",__func__,__LINE__,rc);
	...
	if (rc) {
		...
		#endif
		pr_err("%s %d boot fail with error rc:%d\n", __func__,__LINE__,rc);
