...
static uint64_t suspend_total_time = 0;
static uint64_t suspend_enter_time = 0;
...
static int lpm_suspend_prepare(void)
{
	...
	getnstimeofday(&ts);
	suspend_enter_time = timespec_to_ns(&ts);
#endif /* CONFIG_SHARP_PNP_SLEEP_SLEEPLOG */
...
static void lpm_suspend_wake(void)
{
	...
	getnstimeofday(&ts);
	suspend_total_time += (timespec_to_ns(&ts) - suspend_enter_time);
#endif /* CONFIG_SHARP_PNP_SLEEP_SLEEPLOG */
