...
static void phy_shutdown(struct usbpd *pd)
{
	...
	np = of_find_compatible_node(NULL, NULL,"qcom,gpio2");
	if (np){
		gpio2_vconn = of_get_named_gpio(np,"vconn-gpios", 0);
		if (gpio_is_valid(gpio2_vconn))
			gpio_request(gpio2_vconn, "GPIO2");
		}

	if (pd->vconn_enabled) {
		regulator_disable(pd->vconn);
		if (np && gpio_is_valid(gpio2_vconn))
		{
			usleep_range(1000, 2000);
			gpio_direction_output(gpio2_vconn, 0);
		}
...
static void vconn_swap(struct usbpd *pd)
{
	...
	if (pd->vconn_enabled) {
		...
		if (np && gpio_is_valid(gpio2_vconn))
		{
			gpio_direction_output(gpio2_vconn, 1);
			usleep_range(1000, 2000);
		}
