...
#ifdef KERNEL_SHLOCAL_KONA
extern int32_t shcampdaf_fw_init(uint32_t index);
extern int32_t sh_ois_fw_wait_complete(uint32_t index);
extern int32_t sh_ois_exit(uint32_t index);
extern struct mutex shcampdaf_mutex_00;
extern struct mutex shcampdaf_mutex_01;
extern uint8_t *shcam_eeprom_data_00;
extern uint8_t *shcam_eeprom_data_01;
...
/* SHLOCAL_CAMERA_DRIVERS-> */
#ifdef KERNEL_SHLOCAL_KONA
	rc = shcampdaf_fw_init(o_ctrl->soc_info.index);
	if (rc < 0){
...
/* SHLOCAL_CAMERA_DRIVERS-> */
#ifdef KERNEL_SHLOCAL_KONA
	sh_ois_fw_wait_complete(soc_info->index);
#endif
/* SHLOCAL_CAMERA_DRIVERS<- */
...
/* SHLOCAL_CAMERA_DRIVERS-> */
#ifdef KERNEL_SHLOCAL_KONA
			{
				...
				CAM_ERR(CAM_OIS, "cmm_hdr->cmd_type=%d", cmm_hdr->cmd_type);
				if(cmm_hdr->cmd_type == CAMERA_SENSOR_CMD_TYPE_INVALID) { 
					break;
				} else {
...
/* SHLOCAL_CAMERA_DRIVERS-> */
#ifdef KERNEL_SHLOCAL_KONA
		if(o_ctrl->soc_info.index == 0){
			if((shcam_eeprom_data_00 != NULL) && (shcam_diag_data != NULL)){
				CAM_DBG(CAM_OIS, "shcam_eeprom_data_00[0x28]=0x%0x shcam_eeprom_data_00[0x29]=0x%0x", shcam_eeprom_data_00[0x28], shcam_eeprom_data_00[0x29]);
				if(((shcam_eeprom_data_00[0x28] == 0x00) && (shcam_eeprom_data_00[0x29] == 0x00)) || ((shcam_eeprom_data_00[0x28] == 0xFF) && (shcam_eeprom_data_00[0x29] == 0xFF))){
					return rc;
				}
				...
				CAM_DBG(CAM_OIS, "shcam_eeprom_data_00=%p shcam_diag_data=%p", shcam_eeprom_data_00, shcam_diag_data);
				return rc;
			}
		} else if(o_ctrl->soc_info.index == 1){
			if((shcam_eeprom_data_01 != NULL) && (shcam_diag_data != NULL)){
				CAM_DBG(CAM_OIS, "shcam_eeprom_data_01[0x28]=0x%0x shcam_eeprom_data_01[0x29]=0x%0x", shcam_eeprom_data_01[0x28], shcam_eeprom_data_01[0x29]);
				if(((shcam_eeprom_data_01[0x28] == 0x00) && (shcam_eeprom_data_01[0x29] == 0x00)) || ((shcam_eeprom_data_01[0x28] == 0xFF) && (shcam_eeprom_data_01[0x29] == 0xFF))){
					return rc;
				}
				CAM_DBG(CAM_OIS, "shcam_diag_data[0x00]=0x%0x shcam_diag_data[0x01]=0x%0x", shcam_diag_data[0x00], shcam_diag_data[0x01]);
				if((shcam_diag_data[0x00] != 0x00) || (shcam_diag_data[0x01] != 0x55)){
				...
				CAM_DBG(CAM_OIS, "shcam_eeprom_data_01=%p shcam_diag_data=%p", shcam_eeprom_data_01, shcam_diag_data);
				return rc;
			}
		}
		...
		sh_ois_fw_wait_complete(o_ctrl->soc_info.index);
#endif
/* SHLOCAL_CAMERA_DRIVERS<- */
