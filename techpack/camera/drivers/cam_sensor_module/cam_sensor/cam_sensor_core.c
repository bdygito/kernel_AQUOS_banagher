...
extern void sh_ois_read_eeprom_0(struct cam_sensor_ctrl_t *s_ctrl);
extern void sh_ois_read_eeprom_1(struct cam_sensor_ctrl_t *s_ctrl);
#endif
/* SHLOCAL_CAMERA_DRIVERS<- */
...
/* SHLOCAL_CAMERA_DRIVERS-> */
#ifdef KERNEL_SHLOCAL_KONA
	if(slave_info->sensor_id == 0x0563) {
		sh_ois_read_eeprom_0(s_ctrl);
	} else if(slave_info->sensor_id == 0x1341) {
		sh_ois_read_eeprom_1(s_ctrl);
	}
#endif
#ifdef CONFIG_SHARP_SHTERM
	{
...
/* SHLOCAL_CAMERA_DRIVERS-> */
	{
		...
		if(slave_info->sensor_id == 0x0563) {
			sh_ois_fw_wait_complete(0);
			sh_ois_exit(0);
		} else if(slave_info->sensor_id == 0x1341) {
			sh_ois_fw_wait_complete(1);
			sh_ois_exit(1);
		}
#endif
