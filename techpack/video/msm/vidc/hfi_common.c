...
static atomic_t pwr_collapsed = ATOMIC_INIT(0);
...
static int __get_pwr_collapse_delay(struct venus_hfi_device *device)
{
	int val;

	if (atomic_read(&pwr_collapsed) == 0) {
		val = device->res->msm_vidc_pwr_collapse_delay * 2;
	} else {
		val = device->res->msm_vidc_pwr_collapse_delay;
	}
	...
	if (!__write_queue(q_info, (u8 *)pkt, requires_interrupt, sid)) {
		if (device->res->sw_power_collapsible) {
			...
			if (!queue_delayed_work(device->venus_pm_workq,
				&venus_hfi_pm_work,
				msecs_to_jiffies(
				__get_pwr_collapse_delay(device)))) {
