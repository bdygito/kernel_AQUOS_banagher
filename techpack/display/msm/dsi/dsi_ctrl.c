...
static void dsi_message_tx_dump(const struct mipi_dsi_msg *msg)
{
	...
	if ((pos != NULL) &&
		...
		if ((int)msg->tx_len == (int)1) {
			DRM_DEBUG_DSI("%s: type = %02x(%s/%s), addr = %02x", __func__,
						msg->type,
						(msg->flags & MIPI_DSI_MSG_USE_LPM) ? "LP" : "HS",
						(msg->flags & MIPI_DSI_MSG_LASTCOMMAND) ? "last" : "none",
						*pos);
		} else {
			...
			DRM_DEBUG_DSI("%s: type = %02x(%s/%s), addr = %02x, value =%s", __func__,
						msg->type,
						(msg->flags & MIPI_DSI_MSG_USE_LPM) ? "LP" : "HS",
						(msg->flags & MIPI_DSI_MSG_LASTCOMMAND) ? "last" : "none",
						*pos, str);
