...
#include <linux/err.h>
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00046 */
#ifdef CONFIG_SHARP_SHTERM
#include <misc/shterm_k.h>
#endif /* CONFIG_SHARP_SHTERM */
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00057 */ /* CUST_ID_00210 */
...
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00203 */
#include <video/mipi_display.h>
#endif /* CONFIG_SHARP_DISPLAY */
...
extern void msm_atomic_notify_mode_dpms(struct drm_connector *connector,
	int power_mode);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00209 */
static int dsi_host_alloc_cmd_tx_buffer(struct dsi_display *display);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00210 */
void dsi_display_notify_mode_poms(struct dsi_display *display,
		unsigned long event);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00205 */
...
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01020 */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00205 */
static uint test_err_count = 0;
#ifdef CONFIG_DEBUG_FS
module_param(test_err_count, uint, 0600);
#endif /* CONFIG_DEBUG_FS */
#endif /* CONFIG_SHARP_DISPLAY */

#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00031 */
static int dsi_display_bl_notify_wait = 8000;
module_param(dsi_display_bl_notify_wait, int, 0600);
static int dsi_display_bl_redraw_wait = 32000;
module_param(dsi_display_bl_redraw_wait, int, 0600);
#endif /*  CONFIG_SHARP_DISPLAY */
		...
		void *display, u32 bl_lvl)
{
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00031 */
	bl_down = false;
	DSI_DEBUG("%s:bl_lvl=%d,last_req=%d\n",__func__ ,bl_lvl, bl_last_req);

	if (bl_last_req != 0 && bl_lvl > bl_last_req) {
		if ((dsi_display->ctrl[0].ctrl == NULL) ||
		    (dsi_display->ctrl[0].ctrl->pdev == NULL)) {
			DSI_ERR("%s:pdev is null\n", __func__);
		} else {
			dev = &dsi_display->ctrl[0].ctrl->pdev->dev;
			if (dev){
				sysfs_notify(&dev->kobj, "display", "duty");
				usleep_range(dsi_display_bl_notify_wait,
							 (dsi_display_bl_notify_wait + 100));
				DSI_DEBUG("%s:brightness_wait bl_notify done\n",__func__);

				drm_redraw_event_notify();
				usleep_range(dsi_display_bl_redraw_wait,
							 (dsi_display_bl_redraw_wait + 100));
				DSI_DEBUG("%s:brightness_wait bl_redraw done\n",__func__);
			}
		}
	}else if (bl_lvl > 0 && bl_last_req > bl_lvl) {
		bl_down = true;
	}
	...
	if ((panel->power_mode == SDE_MODE_DPMS_LP1) ||
		...
		DSI_DEBUG("ignore bc setting between LP1, LP2\n");
		goto error;
	}
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00031 */
	panel->bl_config.bl_down = bl_down;
#endif /* CONFIG_SHARP_DISPLAY */
	rc = dsi_panel_set_backlight(panel, (u32)bl_temp);
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00031 */
	panel->bl_config.bl_down = false;
#endif /* CONFIG_SHARP_DISPLAY */
	if (rc)
...
#ifdef CONFIG_SHARP_DISPLAY_1HZ_CUST /* CUST_ID_01027 */
	if (rc == 0) {
...
static int dsi_display_cmd_engine_enable(struct dsi_display *display)
{
	...
	if (display->tx_cmd_buf == NULL) {
		DSI_DEBUG("[%s] display->tx_cmd_buf Is NULL\n", __func__);

		rc = dsi_host_alloc_cmd_tx_buffer(display);
		if (rc) {
			DSI_ERR("[%s] failed to allocate cmd tx buffer memory, rc=%d\n",
				   display->name, rc);
		}
	}
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00205 */
static bool dsi_display_err_num_read(struct dsi_panel *panel)
{
	int ret = 0;
	unsigned char read_cmd_addr[1] = {0x05};
	...
	ret = drm_cmn_dsi_dcs_read(display, read_cmd_addr[0], 1, &r_buf);
	if (ret >= 0) {
		DSI_DEBUG("%s: read: 0x%02x\n", __func__, r_buf);
	} else {
		DSI_ERR("%s: failed to read\n", __func__);
		return false;
	}
	if (test_err_count > 0) {
		r_buf = test_err_count;
		test_err_count--;
	}
	if (r_buf != 0) {
		DSI_ERR("%s:error num %d\n", __func__, r_buf);
		return false;
	}
	return true;
}

static bool dsi_display_err_num_read_lock(struct dsi_panel *panel)
{
	bool ret = true;
	...
	if (drm_cmn_get_panel_type() != DRM_PANEL_STEIN) {
		pr_debug("%s:not support panel\n", __func__);
		return true;
	}

	dsi_panel_acquire_panel_lock(panel);
	ret = dsi_display_err_num_read(panel);
...
static bool dsi_display_validate_reg_read(struct dsi_panel *panel)
{
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00203 */
	j = 0;
	/* Glance Mode(DSI_OP_CMD_MODE) Check */
	if (panel->cur_mode->panel_mode == DSI_OP_CMD_MODE) {
		j = 1;
	}
	...
	for (i = 0; i < len; ++i) {
		...
		DSI_DEBUG("%s panel status %d-%d read: 0x%02x", __func__,
							j, i, config->return_buf[i]);
	}
...
} dsi_display_status_events[] = {
	{ BIT(2)|BIT(4), SHBATTLOG_EVENT_DISP_ERR_DRVOFF	},
	{ BIT(4),		 SHBATTLOG_EVENT_DISP_ERR_SLPIN		},
	{ BIT(2),		 SHBATTLOG_EVENT_DISP_ERR_DISPOFF	},
	// other ((bit(3) , bit(6) , bit(7))
};

static void dsi_display_output_panel_status_event(struct dsi_panel *panel)
{
	int i = 0;
	int arysize = ARRAY_SIZE(dsi_display_status_events);
	...
	reg = config->return_buf[0];

	memset(&info, 0, sizeof(info));
	info.event_num = SHBATTLOG_EVENT_DISP_ERR_CHK_NG;

	for (i = 0; i != arysize; i++) {
		//if (dsi_display_status_events[i].reg == reg) {
		if ((dsi_display_status_events[i].chkbit & reg) == 0) {
			info.event_num = dsi_display_status_events[i].event_num;
			break;
		}
	}

	DSI_ERR("%s: status error. reg = 0x%02x, event=%d\n", __func__, reg, info.event_num);
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00205 */
	struct dsi_display *display = NULL;
#ifdef CONFIG_SHARP_SHTERM /* CUST_ID_00046 */
	shbattlog_info_t info;
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00046 */
#ifdef CONFIG_SHARP_SHTERM
		info.event_num = SHBATTLOG_EVENT_DISP_ERR_READ_ERROR;
		DSI_ERR("%s: status register read error. event=%d\n", __func__, info.event_num);
		shterm_k_set_event(&info);
#endif /* CONFIG_SHARP_SHTERM */
		display->recovery_cnt++;
		DSI_WARN("%s: recovery_cnt=%d\n", __func__, display->recovery_cnt);
#endif /* CONFIG_SHARP_DISPLAY */
		goto exit;
	} else {
		/*
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00046 */
#ifdef CONFIG_SHARP_SHTERM
			dsi_display_output_panel_status_event(panel);
#endif /* CONFIG_SHARP_SHTERM */
#endif /* CONFIG_SHARP_DISPLAY */
			rc = -EINVAL;
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00204 */ /* CUST_ID_00205 */ /* CUST_ID_00210 */
			display->recovery_cnt++;
			DSI_WARN("%s: recovery_cnt=%d\n", __func__, display->recovery_cnt);
		} else {
			display->recovery_cnt = 0;
			if (drm_cmn_get_panel_type() == DRM_PANEL_STEIN) {
				rc = dsi_display_err_num_read(panel);
				if (!rc) {
#ifdef CONFIG_SHARP_SHTERM /* CUST_ID_00046 */
					info.event_num = SHBATTLOG_EVENT_DISP_ERR_MIPI_ERROR;
					DSI_ERR("%s: error_count error. event=%d\n", __func__,
												info.event_num);
					shterm_k_set_event(&info);
#endif /* CONFIG_SHARP_SHTERM */
					rc = -EINVAL;
...
exit:
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00204 */
	DSI_DEBUG("%s:out. rc=%d, recovery_cnt=%d\n",
				__func__, rc, display->recovery_cnt);
#endif /* CONFIG_SHARP_DISPLAY */
	return rc;
}
...
int dsi_display_wait_for_te(struct dsi_display *display)
{
	int rc = 0;
	...
	rc = dsi_display_status_check_te(display);
		...
		int power_mode, void *disp)
{
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00054 */ /* CUST_ID_01005 */
		if (!display->panel->panel_mode_switch_enabled) {
#ifdef CONFIG_ARCH_RECOA
			display->panel->ulps_feature_enabled = true;
#else
			dsi_panel_aod_enable(display->panel, true);
	...
	case SDE_MODE_DPMS_ON:
		...
		if (!display->panel->panel_mode_switch_enabled) {
#ifdef CONFIG_ARCH_RECOA
			display->panel->ulps_feature_enabled = false;
#else
			dsi_panel_aod_enable(display->panel, false);
#endif /* CONFIG_ARCH_RECOA */
		}
#endif /* CONFIG_SHARP_DISPLAY */
	...
	case SDE_MODE_DPMS_OFF:
		...
		if (display->panel->power_mode == SDE_MODE_DPMS_LP1 ||
			display->panel->power_mode == SDE_MODE_DPMS_LP2) {
			rc = dsi_panel_exit_aod(display->panel);
		}
#endif /* CONFIG_ARCH_RECOA */
		break;
#endif /* CONFIG_SHARP_DISPLAY */
				  ...
				  loff_t *ppos)
{
	...
	char *buf;
	int rc = 0;
#ifndef CONFIG_SHARP_DISPLAY /* CUST_ID_00050 */
...
#ifndef CONFIG_SHARP_DISPLAY_DMS_CUST /* CUST_ID_00030 */
		mode->timing.h_skew /= display->ctrl_count;
#endif /* CONFIG_SHARP_DISPLAY_DMS_CUST */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00208 */
	DSI_DEBUG("configuring seamless dynamic fps = %d\n", dsi_mode->timing.refresh_rate);
#else
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00210 */
	display->new_fps = dsi_mode->timing.refresh_rate;
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00032 */ /* CUST_ID_01018 */
	rc = drm_notify_create_sysfs(&display->ctrl[0].ctrl->pdev->dev);
	if (rc) {
		DSI_ERR("%s: failed to drm_notify_create_sysfs, rc=%d\n",
								__func__, rc);
	}
	rc = drm_drive_mode_add_sysfs(display);
	if (rc) {
		DSI_ERR("%s: failed to drm_drive_mode_add_sysfs, rc=%d\n",
								__func__, rc);
		rc = 0;
	}
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01021 */
	rc = drm_bias_add_sysfs(display);
	if (rc) {
		DSI_ERR("%s: failed to drm_bias_add_sysfs, rc=%d\n",
								__func__, rc);
		rc = 0;
	}
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01022 */
	rc = drm_peak_nits_add_sysfs(display);
	if (rc) {
		DSI_ERR("%s: failed to drm_peak_nits_create_sysfs, rc=%d\n",
								__func__, rc);
		rc = 0;
	}
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00203 */
	display->aod_offset = AOD_OFFSET_CENTER;
	display->aod_direction = true;
#endif /* CONFIG_SHARP_DISPLAY*/
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00210 */
	display->new_fps = 0;
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01021 */
	drm_bias_remove_sysfs(display);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01022 */
	drm_peak_nits_remove_sysfs(display);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00032 */ /* CUST_ID_01018 */
			  ...
			  struct dsi_display_mode **out_modes)
{
	...
	*out_modes = NULL;
	...
	for (mode_idx = 0; mode_idx < timing_mode_count; mode_idx++) {
		...
		num_dfps_rates = ((!dfps_caps.dfps_support || display_mode.priv_info->mipiclkhalf ||
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00206 */
			if (display_mode.priv_info->mipiclkhalf)
				continue;
#endif /* CONFIG_SHARP_DISPLAY */
...
int dsi_display_get_panel_vfp(void *dsi_display,
	int h_active, int v_active, int clock)
#else
int dsi_display_get_panel_vfp(void *dsi_display,
...
#endif /* CONFIG_SHARP_DISPLAY */
{
	...
	dsi_panel_get_dfps_caps(display->panel, &dfps_caps);
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00206 */
	if (dfps_caps.dfps_support && !display->panel->cur_mode->priv_info->mipiclkhalf)
#else
	if (dfps_caps.dfps_support)
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00030 */
	if (display->panel->dms_mode) {
		refresh_rate = timing.h_skew;
	} else {
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	if (display->is_cont_splash_enabled) {
		...
		rc = drm_gmm_ctrl_gamma_table(display);
		if (rc) {
	...
	if (mode->dsi_mode_flags & DSI_MODE_FLAG_DMS) {
		...
		if(dsi_panel_is_type_oled(display->panel)) {
			/* Nothing return code. because to freeze if error code return. */
			drm_drive_mode_send_timing_switch(display->panel);
		} else {
		...
		if (display->panel->dms_mode) {
			if (mode->timing.h_skew > DRM_BASE_MAX_FPS) {
				fpslow_base = DRM_BASE_MAX_FPS;
			} else {
				fpslow_base = mode->timing.h_skew;
			}
			DSI_DEBUG("%s:refresh_rate(%d), fpslow_base(%d)\n", __func__,
						mode->timing.h_skew, fpslow_base);
			msm_set_fps_low_base(fpslow_base);
		}
#else /* CONFIG_SHARP_DISPLAY */
		rc = dsi_panel_switch(display->panel);
		if (rc)
...
#if defined(CONFIG_SHARP_DISPLAY) && defined(CONFIG_SHARP_BOOT) /* CUST_ID_00057 */
static int dsi_display_mfr_raven_30hz(struct dsi_display *display)
{
	int ret = 0;
#if defined(CONFIG_ARCH_BANAGHER)
	int cmd_cnt;
	struct dsi_cmd_desc *cmds;
...
int dsi_display_post_enable(struct dsi_display *display)
{
	...
	if (display->panel->cur_mode->dsi_mode_flags & DSI_MODE_FLAG_POMS) {
		...
		if (display->config.panel_mode == DSI_OP_CMD_MODE) {
			dsi_display_err_num_read_lock(display->panel);
			dsi_panel_mode_switch_to_cmd(display->panel);
			dsi_display_aod_disp_move(display);
		}
#else /* CONFIG_SHARP_DISPLAY */
		if (display->config.panel_mode == DSI_OP_CMD_MODE)
			dsi_panel_mode_switch_to_cmd(display->panel);
...
#ifdef CONFIG_SHARP_DISPLAY_DMS_CUST /* CUST_ID_00030 */ /* CUST_ID_01005 */
		mode = display->panel->cur_mode;
		DSI_DEBUG("%s: mode_flags(0x%x)\n", __func__, mode->dsi_mode_flags);
		if ((mode) && (!(mode->dsi_mode_flags & DSI_MODE_FLAG_DMS))) {
#ifdef CONFIG_ARCH_RECOA
			DSI_DEBUG("%s: power_mode(%d)\n", __func__,
								display->panel->power_mode);
			if (display->panel->power_mode == SDE_MODE_DPMS_LP1 ||
				display->panel->power_mode == SDE_MODE_DPMS_LP2) {
				int rc2 = 0;

				rc2 = dsi_panel_aod_setting(display->panel);
				if (rc2)
					DSI_ERR("[%s] panel aod-setting failed, rc=%d\n",
					       display->name, rc2);
				rc |= rc2;

				rc2 = dsi_panel_aod_start(display->panel);
				if (rc2)
					DSI_ERR("[%s] panel aod-start failed, rc=%d\n",
					       display->name, rc2);
				rc |= rc2;
			} else
#endif /* CONFIG_ARCH_RECOA */
...
#else /* CONFIG_SHARP_DISPLAY_DMS_CUST */
		rc = dsi_panel_post_enable(display->panel);
		if (rc)
			DSI_ERR("[%s] panel post-enable failed, rc=%d\n",
				display->name, rc);
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00210 */
	if (display->panel->cur_mode->dsi_mode_flags & DSI_MODE_FLAG_POMS) {
		dsi_display_notify_mode_poms(display,
				DRM_PANEL_EVENT_SWITCH_GLANCE_MODE);
	}
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00007 */ /* CUST_ID_00046 */  /* CUST_ID_00203 */
	mode = display->panel->cur_mode;
	if ((mode) && (!(mode->dsi_mode_flags & (DSI_MODE_FLAG_DMS | DSI_MODE_FLAG_POMS)))) {
		if (bl_tmp) {
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00024 */ /* CUST_ID_00203 */
	priv = display->drm_dev->dev_private;
	if ((!priv) || (!priv->panel_is_connected)) {
		return rc;
	}
...
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00032 */ /* CUST_ID_00046 */ /* CUST_ID_00203 */
	if ((mode) && (!(mode->dsi_mode_flags & (DSI_MODE_FLAG_DMS | DSI_MODE_FLAG_POMS)))) {
		if (!display->panel->panel_mode_switch_enabled) {
...
int dsi_display_pre_disable(struct dsi_display *display)
{
	...
	if ((priv) && (priv->panel_is_connected)) {
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00032 */ /* CUST_ID_00203 */
...
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY  /* CUST_ID_00203 */ /* CUST_ID_00204 */
	panel = display->panel;
	if (panel) {
		if ((panel->esd_config.esd_enabled) && (!display->poms_pending)) {
			if (display->recovery_cnt >= DSI_DISPLAY_RETRY_RECOVERY_CNT) {
	...
	if (display->poms_pending) {
		dsi_display_notify_mode_poms(display,
				DRM_PANEL_EVENT_SWITCH_GLANCE_MODE_PRE);
	}
#endif /* CONFIG_SHARP_DISPLAY */
...
exit:
	mutex_unlock(&display->display_lock);
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00046 */ /* CUST_ID_00203 */
#ifdef CONFIG_SHARP_SHTERM
	if (!display->poms_pending) {
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00203 */ /* CUST_ID_00210 */
static int dsi_display_aod_disp_position_stein(struct dsi_display *display)
{
	int aod_offset = 0;
	int ret = 0;
	char data[] = {0x84, 0x00, 0x00, 0x00, 0x00};
	struct dsi_cmd_desc cmd[] = {
		{{0, MIPI_DSI_GENERIC_LONG_WRITE , MIPI_DSI_MSG_USE_LPM | MIPI_DSI_MSG_UNICAST, 0, 0, sizeof(data), data, 0, NULL}, 1, 0},
	};
	...
	aod_offset = display->aod_offset;
	DSI_DEBUG("%s:in aod_offset=%d(0x%04X)\n", __func__, aod_offset, aod_offset);

	data[1] = ((aod_offset >> 8) & 0x0F);
	data[2] = (aod_offset & 0xFF);
	data[3] = (((aod_offset + AOD_DROW_HEIGHT) >> 8) & 0x0F);
	data[4] = ((aod_offset + AOD_DROW_HEIGHT) & 0xFF);

	ret = drm_cmn_panel_cmds_transfer(display, cmd, ARRAY_SIZE(cmd));
	if (ret) {
		DSI_ERR("[%s] aod disp position error:%d\n", __func__, (int)ret);
		return ret;
	}
...
static int dsi_display_aod_disp_position_sinanju(struct dsi_display *display)
{
	int aod_offset = 0;
	int ret = 0;
	char data[] = {0x30, 0x00, 0x00, 0x00, 0x00};
	struct dsi_cmd_desc cmd[] = {
		{{0, MIPI_DSI_DCS_LONG_WRITE , MIPI_DSI_MSG_USE_LPM | MIPI_DSI_MSG_UNICAST, 0, 0, sizeof(data), data, 0, NULL}, 1, 0},
	};
	...
	data[1] = ((aod_offset >> 8) & 0x0F);
	data[2] = (aod_offset & 0xFF);
	data[3] = (((aod_offset + AOD_DROW_HEIGHT - 1) >> 8) & 0x0F);
	data[4] = ((aod_offset + AOD_DROW_HEIGHT - 1) & 0xFF);
...
void dsi_display_aod_disp_move(void *disp)
{
	struct dsi_display *display;
	struct dsi_panel *panel = NULL;
	int ret = 0;
	...
	panel = display->panel;
	if (!panel) {
		SDE_ERROR("%s: dsi_panel is NULL\n", __func__);
		return;
	}

	if (!panel->panel_mode_switch_enabled) {
		SDE_DEBUG("%s: mode switch disabled\n", __func__);
		return;
	}

	DSI_DEBUG("%s: offset(%d) direction(%d)\n", __func__,
			display->aod_offset, display->aod_direction);
	if (drm_cmn_get_panel_type() == DRM_PANEL_STEIN) {
		ret = dsi_display_aod_disp_position_stein(display);
	} else {
		ret = dsi_display_aod_disp_position_sinanju(display);
	}
	if (ret < 0) {
		return;
	}

	if (display->aod_direction) {
		display->aod_offset += AOD_OFFSET_STEP;
		if (display->aod_offset >= AOD_OFFSET_CENTER + AOD_OFFSET_MAX) {
			display->aod_direction = false;
		}
	} else {
		display->aod_offset -= AOD_OFFSET_STEP;
		if (display->aod_offset <= AOD_OFFSET_CENTER - AOD_OFFSET_MAX) {
	...
	DSI_DEBUG("%s: next offset(%d) direction(%d)\n", __func__,
			display->aod_offset, display->aod_direction);
...
int dsi_display_set_drive_mode(void *display, enum msm_drive_mode mode)
{
	...
	rc = drm_drive_mode_set(dsi_display, mode);
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00210 */
int dsi_display_update_dfps(void *display)
{
	struct dsi_display *dsi_display = (struct dsi_display *)display;
	int rc = 0;
	int panel_type = DRM_PANEL_UNKNOWN;
	...
	panel_type = drm_cmn_get_panel_type();
	if (panel_type != DRM_PANEL_SINANJU) {
		DSI_DEBUG("Set dfps not support panel(%d)", panel_type);
		return rc;
	}

	rc = dsi_panel_set_dfps(dsi_display->panel, dsi_display->new_fps);

	DSI_DEBUG("Set dfps %d %s", dsi_display->new_fps,
			rc ? "failed" : "successful");
		...
		unsigned long event)
{
	...
	if (!connector) {
		DSI_ERR("%s: drm_connector is NULL\n", __func__);
		return;
	}

	drm_panel = dsi_display_get_drm_panel(display);
	if (!drm_panel) {
		DSI_ERR("%s: drm_panel is NULL\n", __func__);
		return;
	}

	switch (display->config.panel_mode) {
	case DSI_OP_VIDEO_MODE:
		if (event == DRM_PANEL_EVENT_SWITCH_GLANCE_MODE_PRE) {
			poms_mode = DRM_PANEL_GLANCE_SWITCH_TO_CMD_MODE;
		} else {
			poms_mode = DRM_PANEL_GLANCE_SWITCH_TO_VIDEO_MODE;
		}
		break;
	case DSI_OP_CMD_MODE:
		if (event == DRM_PANEL_EVENT_SWITCH_GLANCE_MODE_PRE) {
		...
		break;
	default:
		DSI_ERR("invalid panel_mode(%d)\n", display->config.panel_mode);
		return;
	}
	DSI_DEBUG("event=0x%02X,poms_mode=%d\n", event, poms_mode);

	memset(&notifier_data, 0x00, sizeof(notifier_data));
	notifier_data.data = &poms_mode;
	notifier_data.id = connector->base.id;

	drm_panel_notifier_call_chain(drm_panel, event,
					    &notifier_data);
}
#endif /* CONFIG_SHARP_DISPLAY */
