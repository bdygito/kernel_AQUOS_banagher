				...
				struct dsi_display_mode *dsi_mode)
{
	...
	if (!drm_cmn_is_diag_mode()) {
		dsi_mode->timing.refresh_rate = DRM_BASE_MAX_FPS;
		DSI_DEBUG("%s:set h_skew(%d)\n", __func__,
							dsi_mode->timing.h_skew);
	} else {
				...
				struct drm_display_mode *drm_mode)
{
	...
	if (!drm_cmn_is_diag_mode()) {
		...
		DSI_DEBUG("%s:set hskew(%d)\n", __func__,  drm_mode->hskew);
	} else {
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00207 */
	if (crtc_state->crtc) {
		...
		struct msm_display_kickoff_params *params)
{
	...
	dsi_panel_apply_boost_pre(connector, display);
#endif /* CONFIG_SHARP_DISPLAY */
