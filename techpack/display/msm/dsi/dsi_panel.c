...
#include "sde_dbg.h"
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00034 */ /* CUST_ID_00037 */
#include "sharp/drm_diag.h"
#include "../sharp/drm_gmm.h"
#endif /* CONFIG_SHARP_DISPLAY */

#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00039 */ /* CUST_ID_00053 */
#include "../sharp/drm_cmn.h"
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01015 */
#include "../sharp/drm_voltage_control.h"
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00041 */ /* CUST_ID_00053 */
#include "sde_connector.h"
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01018 */
#include "../sharp/drm_drive_mode.h"
#endif /* CONFIG_SHARP_DISPLAY */
...
#include "../sharp/drm_peak_nits.h"
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01027 */
...
#define DSI_PANEL_DIMMING_WAIT	(34 * 1000)
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01019 */
#define DRM_SVEN_EVT_BKL_MAX_LIMIT 0xA00
#define DRM_SVEN_EVT_BKL_MIN_LIMIT 0x100
#endif /* CONFIG_SHARP_DISPLAY */

#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00002 */
static int dsi_panel_reset_on(struct dsi_panel *panel);
#ifdef CONFIG_ARCH_BANAGHER
...
static unsigned int cancel_delay_ms[DSI_BOOST_MODE_MAX] = {10 * 1000, 10 * 1000};
...
static void dsi_panel_curr_boost_panel_mutex_unlock(enum dsi_backlight_type type,
										struct dsi_panel *panel);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01011 */ /* CUST_ID_01030 */
static int dsi_panel_apply_boost(struct dsi_panel *panel,
		struct dsi_backlight_config *bl, struct dsi_display *display);
#ifdef CONFIG_ARCH_CHARA
static int dsi_panel_finger_boost_wait = 4000;
module_param(dsi_panel_finger_boost_wait, int, 0600);
static int dsi_panel_finger_boost_off_wait = 4000;
module_param(dsi_panel_finger_boost_off_wait, int, 0600);
#else /* CONFIG_ARCH_CHARA */
static int dsi_panel_finger_boost_wait = 4800;
module_param(dsi_panel_finger_boost_wait, int, 0600);
#endif /* CONFIG_ARCH_CHARA */
static int dsi_panel_finger_boost_notify_wait = 16000;
module_param(dsi_panel_finger_boost_notify_wait, int, 0600);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00031 */
static int dsi_panel_bl_down_wait = 17000;
module_param(dsi_panel_bl_down_wait, int, 0600);
#endif /*  CONFIG_SHARP_DISPLAY */

#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01014 */
static void dsi_panel_dimming_work(struct work_struct *work);
#endif /* CONFIG_SHARP_DISPLAY */
...
#if defined(CONFIG_SHARP_DISPLAY) && defined(CONFIG_ARCH_CHARA) /* CUST_ID_01009 */
...
static int dsi_panel_gpio_request(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(panel->panel_vddio_en_gpio)) {
		rc = gpio_request(panel->panel_vddio_en_gpio, "panel_vddio_en_gpio");
		if (rc) {
			DSI_ERR("request for vddio_en_gpio failed, rc=%d\n", rc);
			goto error_release_vddio_en;
		}
	}
#endif /*  CONFIG_SHARP_DISPLAY */
	...
	goto error;
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01001 */
error_release_vddio_en:
	if (gpio_is_valid(panel->panel_vddio_en_gpio))
		gpio_free(panel->panel_vddio_en_gpio);
#endif /*  CONFIG_SHARP_DISPLAY */
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00002 */ /* CUST_ID_00202 */
	...
	if (gpio_is_valid(panel->panel_vddio_en_gpio)) {
		...
		rc = gpio_direction_output(panel->panel_vddio_en_gpio, 0);
		if (rc)
			DSI_ERR("unable to set dir for vddio en gpio rc=%d\n", rc);
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01001 */
static int dsi_panel_set_display_brightness(u16 brightness)
{
	int ret = 0;
	struct dsi_display *display = NULL;
	unsigned char addr_value[][3] = {
		{0xFE, 0x00},
	...
	struct dsi_cmd_desc cmd[] = {
		...
		{{0, MIPI_DSI_DCS_LONG_WRITE        , MIPI_DSI_MSG_USE_LPM | MIPI_DSI_MSG_UNICAST, 0, 0, 3, addr_value[1], 0, NULL}, 1, 0},
	};

	switch (drm_cmn_get_panel_revision()) {
	case DRM_PANEL_REV_EVT:
		if (brightness > DRM_SVEN_EVT_BKL_MAX_LIMIT) {
			brightness = DRM_SVEN_EVT_BKL_MAX_LIMIT;
		}
		if ((brightness != 0) && (brightness < DRM_SVEN_EVT_BKL_MIN_LIMIT)) {
			brightness = DRM_SVEN_EVT_BKL_MIN_LIMIT;
		}
		break;
	default:
		break;
	...
	addr_value[1][1] = brightness >> 8;
	addr_value[1][2] = brightness & 0xff;

	ret = drm_cmn_dsi_cmds_transfer(display, cmd, ARRAY_SIZE(cmd));
	if (ret) {
		DSI_ERR("[%s] brightness change error:%d\n", __func__, (int)ret);
		return ret;
	}
...
{
	int rc = 0;
#ifndef CONFIG_SHARP_DISPLAY /* CUST_ID_01001 */
...
#ifndef CONFIG_SHARP_DISPLAY /* CUST_ID_01001 */ /* CUST_ID_01013 */
	...
	rc = dsi_panel_set_display_brightness(bl_lvl);
	if (rc < 0)
		DSI_ERR("failed to update dcs backlight:%d\n", bl_lvl);
#endif /* CONFIG_SHARP_DISPLAY */

#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01021 */ /* CUST_ID_00031 */
	bl = &panel->bl_config;
	if (bl && bl->bl_down){
		usleep_range(dsi_panel_bl_down_wait,
					 (dsi_panel_bl_down_wait + 100));
		DSI_DEBUG("%s:brightness_wait bl_down done\n",__func__);
	}
...
static int dsi_panel_update_dev_brightness(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (!bl->dev_bd) {
		bl->dev_bd = backlight_device_get_by_type(BACKLIGHT_RAW);
		if (!bl->dev_bd) {
			DSI_ERR("%s: backlight_device_set failed\n", __func__);
			return -EINVAL;
		}
	}
	sysfs_notify(&bl->dev_bd->dev.kobj, NULL, "dev_brightness");
	return 0;
}
...
static int dsi_panel_update_curr_boosted(struct dsi_panel *panel, bool boosted)
{
	...
	bl->curr_boosted_notify = boosted;
	...
	sysfs_notify(&bl->dev_bd->dev.kobj, NULL, "curr_boost");
	return 0;
}
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00041 */ /* CUST_ID_01013 */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00206 */
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"sharp,mdss-dsi-dfps-60-command",
	"sharp,mdss-dsi-dfps-30-command",
#endif /*  CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01005 */
	"sharp,mdss-dsi-aod-setting-command",
	"sharp,mdss-dsi-aod-start-command",
	"sharp,mdss-dsi-exit-aod-command",
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01018 */
...
#endif /*  CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01022 */
	"sharp,mdss-dsi-nits-apl-off",
	"sharp,mdss-dsi-nits-apl-on",
	"sharp,mdss-dsi-nits-normal",
	"sharp,mdss-dsi-nits-high",
	"sharp,mdss-dsi-nits-hdr",
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01025 */
	"sharp,mdss-dsi-on-command-gmm-vol",
...
#endif /*  CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00210 */
	"sharp,mdss-dsi-dfps-60-command-state",
	"sharp,mdss-dsi-dfps-30-command-state",
#endif /*  CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01005 */
	"sharp,mdss-dsi-aod-setting-command-state",
	"sharp,mdss-dsi-aod-start-command-state",
	"sharp,mdss-dsi-exit-aod-command-state",
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01018 */
...
#endif /*  CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01022 */
	"sharp,mdss-dsi-nits-apl-off-state",
	"sharp,mdss-dsi-nits-apl-on-state",
	"sharp,mdss-dsi-nits-normal-state",
	"sharp,mdss-dsi-nits-high-state",
	"sharp,mdss-dsi-nits-hdr-state",
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01025 */
	"sharp,mdss-dsi-on-command-gmm-vol-state",
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	if (!gpio_is_valid(panel->reset_config.tp_reset_gpio) &&
		...
		DSI_INFO("[%s] failed get tp-reset gpio, rc=%d\n", panel->name, rc);
	}
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01001 */
	panel->panel_vddio_en_gpio = utils->get_named_gpio(utils->data,
					"sharp,vddio-en-gpio", 0);
	if (!gpio_is_valid(panel->panel_vddio_en_gpio))
		DSI_DEBUG("%s: panel vddio enable gpio not specified\n", __func__);
#endif /*  CONFIG_SHARP_DISPLAY */
...
static int dsi_panel_parse_bl_pwm_config(struct dsi_panel *panel)
{
	...
	config->boost_commit_pending = false;
	config->boost_kickoff_pending = false;
#endif /*  CONFIG_SHARP_DISPLAY */
...
#else /* CONFIG_SHARP_BOOT */
	rc = dsi_panel_parse_mfr(panel, of_node);
#endif /* CONFIG_SHARP_BOOT */
	if (rc)
		DSI_DEBUG("failed to mfr params, rc=%d\n", rc);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00210 */
		panel->new_fps = 0;
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01014 */
	panel->dimming_status = DIMMING_STATUS_ON;
	panel->dimming_on_pending = false;
	panel->dimming_request = DIMMING_STATUS_ON;
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01028 */
#ifdef CONFIG_ARCH_CHARA
	panel->sleep_out_wait = false;
#endif /* CONFIG_ARCH_CHARA */
#endif /* CONFIG_SHARP_DISPLAY */
	goto exit;
...
{
#ifndef CONFIG_SHARP_DISPLAY /* CUST_ID_00206 */
	...
	int count, rc = 0;
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00206 */ /* CUST_ID_00039 */ /* CUST_ID_00030 */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00039 */ /* CUST_ID_00206 */ /* CUST_ID_00030 */
		loop++;
		mipiclkhalf = utils->read_bool(child_np, "sharp,mdss-dsi-panel-mipiclkhalf");
		if (diag_mode && mipiclkhalf) {
			num_mipiclkhalf_modes++;
		} else if (diag_mode && (loop > 1) && !panel->dfps_caps.dfps_support) {
			panel->num_timing_nodes--;
		} else if (!diag_mode && mipiclkhalf) {
			panel->num_timing_nodes--;
		} else
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00039 */ /* CUST_ID_00030 */
	int loop = 0;
	bool mipiclkhalf;
	bool diag_mode = drm_cmn_is_diag_mode();
#endif /* CONFIG_SHARP_DISPLAY */
...
#if defined(CONFIG_SHARP_DISPLAY) && defined(CONFIG_ARCH_SARAH) /* CUST_ID_00206 */
	if (mode->priv_info->mipiclkhalf) {
		config->common_config.t_clk_post = 0x0A;
		config->common_config.t_clk_pre = 0x20;
	}
#endif /* CONFIG_SHARP_DISPLAY *//* CONFIG_ARCH_SARAH */
#if defined(CONFIG_SHARP_DISPLAY) && defined(CONFIG_ARCH_RECOA) /* CUST_ID_01002 */
	if (mode->priv_info->mipiclkhalf) {
		config->common_config.t_clk_post = 0x0B;
		config->common_config.t_clk_pre = 0x29;
	}
#endif /* CONFIG_SHARP_DISPLAY *//* CONFIG_ARCH_RECOA */
	mutex_unlock(&panel->panel_lock);
	return rc;
}
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00002 *//* CUST_ID_00003 */
	if (panel->lp11_init) {
		if (gpio_is_valid(panel->vcc_gpio)) {
			gpio_set_value(panel->vcc_gpio, 1);
		}
		if (gpio_is_valid(panel->panel_vddio_en_gpio)) {
			DSI_DEBUG("[%s] vddio en gpio pulled high\n", panel->name);
			rc = gpio_direction_output(panel->panel_vddio_en_gpio, 1);
			gpio_set_value(panel->panel_vddio_en_gpio, 1);
			if (rc)
				DSI_ERR("unable to set dir for vddio en gpio rc=%d\n", rc);
#ifdef CONFIG_ARCH_CHARA
...
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	...
	drm_drive_mode_refresh_rate_update(panel, panel->cur_mode->timing.h_skew);

#if defined(CONFIG_ARCH_RECOA) || defined(CONFIG_ARCH_CHARA)
	mutex_unlock(&panel->panel_lock);
	if (!rc) {
		rc = dsi_panel_post_enable(panel);
	}
	return rc;
...
int dsi_panel_prepare(struct dsi_panel *panel)
{
	...
	if (panel->lp11_init) {
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00002 */
		rc = dsi_panel_reset_on(panel);
		if (rc) {
			DSI_ERR("[%s] failed dsi_panel_reset_on(),"
						"rc=%d\n", panel->name, rc);
			goto error;
		}
#else /* CONFIG_SHARP_DISPLAY */
...
#ifndef CONFIG_SHARP_DISPLAY /* CUST_ID_00203 */
	mutex_lock(&panel->panel_lock);
...
int dsi_panel_enable(struct dsi_panel *panel)
{
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01021 */ /* CUST_ID_01028 */
	if (!rc) {
		panel->mfr = MFR_INIT_VALUE_PANEL_ON;
		panel->bias_register = drm_bias_get_otp_voltage();
#ifdef CONFIG_ARCH_CHARA
		panel->sleep_out_wait = true;
#endif /* CONFIG_ARCH_CHARA */
	}
#endif /* CONFIG_SHARP_DISPLAY */

#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00030 */
	drm_drive_mode_refresh_rate_update(panel, 60);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01017 */
	if (!rc) {
		panel->mfr = MFR_INIT_VALUE_PANEL_ON;
	...
	if (panel->host) {
		...
		if (display) {
			rc = drm_gmm_active_gamma_table(display);
			if (rc) {
	...
	mutex_unlock(&panel->panel_lock);
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00034 */ /* CUST_ID_00037 */ /* CUST_ID_01020 */
#ifndef CONFIG_ARCH_RECOA
	if (!panel->host) {
		DSI_ERR("%s: Invalid host data\n", __func__);
	} else {
		display = container_of(panel->host, struct dsi_display, host);
		if (!display) {
			DSI_ERR("%s: dsi_display is NULL.\n", __func__);
		} else {
			drm_diag_set_adjusted(display);
		}
	}
#endif /* CONFIG_ARCH_RECOA */
#endif /* CONFIG_SHARP_DISPLAY */
...
{
	int rc = 0;
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01020 */ /* CUST_ID_01028 */
#ifdef CONFIG_ARCH_CHARA
	struct dsi_display *display = NULL;
#endif /* CONFIG_ARCH_CHARA */
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01020 */ /* CUST_ID_01025 */ /* CUST_ID_01028 */
#ifdef CONFIG_ARCH_CHARA
	if (panel->sleep_out_wait == true) {
		usleep_range(120000, 120010);
		panel->sleep_out_wait = false;
	}
	if (drm_cmn_get_panel_revision() < DRM_PANEL_REV_PVT) {
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_ON_GMM_VOL);
	if (rc) {
		DSI_ERR("[%s] failed to send DSI_CMD_SET_ON_GMM_VOL cmds, rc=%d\n",
		       panel->name, rc);
	}
	...
	if (panel->host) {
		display = container_of(panel->host, struct dsi_display, host);
		if (display) {
			rc = drm_gmm_write_gamma_table(display);
			if (rc) {
				DSI_ERR("[%s] failed to active gamma table, rc=%d\n",
				       panel->name, rc);
			}
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01022 */ /* CUST_ID_01020 */
#ifdef CONFIG_ARCH_CHARA
	if (panel->host) {
		display = container_of(panel->host, struct dsi_display, host);
		if (display) {
			rc = drm_gmm_dispon_gamma_table(display);
			if (rc) {
				DSI_ERR("[%s] failed to active gamma table, rc=%d\n",
				       panel->name, rc);
			}
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01022 */
#ifdef CONFIG_ARCH_CHARA
	drm_peak_nits_enable(false);
#endif /* CONFIG_ARCH_CHARA */
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01017 */
	bd = backlight_device_get_by_type(BACKLIGHT_RAW);
	if (bd) {
		sysfs_notify(&bd->dev.kobj, NULL, "dev_brightness");
	} else {
...
#if defined(CONFIG_SHARP_DISPLAY) && defined(CONFIG_ARCH_BANAGHER) /* CUST_ID_00002 */
	if (panel->lp11_init) {
		rc = dsi_panel_reset_off(panel);
		if (rc) {
			DSI_ERR("[%s] failed dsi_panel_reset_off(),"
						"rc=%d\n", panel->name, rc);
		}
	}
#endif /* CONFIG_SHARP_DISPLAY */ /* CONFIG_ARCH_BANAGHER */
error:
	mutex_unlock(&panel->panel_lock);
	return rc;
}
...
#if defined(CONFIG_SHARP_DISPLAY) && defined(CONFIG_ARCH_BANAGHER) /* CUST_ID_00002 */ /* CUST_ID_00003 */
	if (!panel->lp11_init) {
...
#else /* CONFIG_SHARP_DISPLAY */ /* CONFIG_ARCH_BANAGHER */
	rc = dsi_panel_power_off(panel);
	if (rc) {
		DSI_ERR("[%s] panel power_Off failed, rc=%d\n",
		       panel->name, rc);
					...
					enum dsi_cmd_set_type cmd_type)
{
	...
	if (rc) {
		DSI_ERR("[%s] failed to send set_type(%d) cmds, rc=%d\n",
		       panel->name, cmd_type, rc);
	}
	mutex_unlock(&panel->panel_lock);
	return rc;
}
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00053 */ /* CUST_ID_00059 */
int dsi_panel_backlight_mfr(struct dsi_panel *panel, int value)
{
	...
	if (panel == NULL) {
		DSI_ERR("%s: dsi_panel is NULL.\n", __func__);
		return -ENXIO;
	}

	display = msm_drm_get_dsi_display();
	if(!display){
		DSI_ERR("[%s]Invalid dsi_display\n", __func__);
		return -EINVAL;
	}

	connector = display->drm_conn;
	if (!connector) {
		DSI_ERR("[%s]Invalid drm_connector\n", __func__);
		return -EINVAL;
	}

	sde_conn = to_sde_connector(connector);
	if (!sde_conn) {
		DSI_ERR("[%s]Invalid sde_connector\n", __func__);
		return -EINVAL;
	}

	if (sde_conn->last_panel_power_mode == SDE_MODE_DPMS_LP1) {
		DSI_DEBUG("%s:power_mode = SDE_MODE_DPMS_LP1\n", __func__);
		return ret;
	}
	...
	for (i = 0; i < panel->num_mfr; i++) {
		if (value <= panel->mfr_config[i].upper && value >= panel->mfr_config[i].lower) {
			if (i == panel->mfr_idx) {
				DSI_DEBUG("%s:already mfr setting\n", __func__);
				goto end;
			} else {
	...
	if (mfr_cmds->count) {
		...
		DSI_ERR("%s:mfr_cmds is zero. idx=%d\n", __func__, i);
	}
end:
	DSI_DEBUG("%s:out\n", __func__);
	return ret;
	...
	struct device_node *of_node)
{
	...
	if (!mfr_root_node) {
		...
		for_each_child_of_node(mfr_root_node, mfr_node) {
			...
			DSI_DEBUG("%s:[%d] lower=%d upper=%d cmds_num=%d\n", __func__, i,
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00041 */ /* CUST_ID_01011 */ /* CUST_ID_01013 */ /* CUST_ID_01014 */ /* CUST_ID_01022 */ /* CUST_ID_01027 */ /* CUST_ID_01030 */
static struct dsi_panel *to_dsi_panel(struct backlight_device *bd)
{
	...
	if (!c_conn) {
		DSI_ERR("%s: sde_connector is null\n",__func__);
		return NULL;
	}
	...
	if (panel) {
		bl = &panel->bl_config;
		if (!bl) {
			DSI_ERR("%s: dsi_backlight_config is null\n",__func__);
		}
		bl->dev_bd = bd;
	}
	return panel;
					...
					u32 *boost_req, u32 *boosted)
{
	...
	*boosted = bl->curr_boosted_notify;
	return 0;
}
...
int dsi_panel_request_boost(struct backlight_device *bd, u32 onoff)
{
	...
	bl->curr_boosted_notify = (bool)onoff;
	switch (onoff) {
	case 0:
		// boost off
		if (dev) {
	...
	case 1:
		...
		break;
	case 2:
		bl->boost_mode = DSI_FINGER_BOOST_ON;
		if (dev) {
			sysfs_notify(&dev->kobj, "display", "duty");
		}
		break;
	default:
		DSI_WARN("boost_mode changed %d to boost_mode\n", onoff);
		bl->boost_mode = DSI_INCAMERA_BOOST_ON;
		break;
	}

	DSI_DEBUG("%s: boost mode(%d) req(%d)\n",__func__, bl->boost_mode, bl->curr_boost_req);

	usleep_range(dsi_panel_finger_boost_notify_wait,
				 (dsi_panel_finger_boost_notify_wait + 100));
	DSI_DEBUG("%s: boost wait done\n",__func__);

	if (bl->boost_mode != DSI_FINGER_BOOST_ON) {
		queue_delayed_work(panel->ordered_workqueue, &panel->curr_boost_work, 0);
		bl->boost_commit_pending = false;
	} else {
		DSI_DEBUG("%s: boost_commit_pending is true\n",__func__);
		bl->boost_commit_pending = true;
	}
		...
		void *display)
{
	...
	if ((connector == NULL) || (display == NULL)) {
		DSI_ERR("%s: Invalid param connector=%p display=%p\n", __func__,
				connector, display);
		return -EINVAL;
	}

	panel = ((struct dsi_display *)display)->panel;
	if (!panel) {
		DSI_ERR("%s: Invalid panel data\n", __func__);
		return -EINVAL;
	}
	...
	if (!c_conn) {
		DSI_ERR("%s: Invalid c_conn data\n", __func__);
		return -EINVAL;
	}

	if (!dsi_panel_is_type_oled(panel)) {
		DSI_DEBUG("%s: Not OLED device\n", __func__);
		return -ENOTSUPP;
	}
	...
	if (c_conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
		...
		mutex_lock(&panel->curr_boost_lock);
		if (!bl->boost_kickoff_pending) {
			DSI_DEBUG("curr_boost is already set.skip\n");
			mutex_unlock(&panel->curr_boost_lock);
		...
		/* curr_boost Request */
		rc = dsi_panel_apply_boost(panel, bl, (struct dsi_display *)display);
		mutex_unlock(&panel->curr_boost_lock);
		dsi_panel_curr_boost_panel_mutex_unlock(bl->type, panel);
	}
		...
		struct dsi_backlight_config *bl, struct dsi_display *display)
{
	...
	bl->boost_kickoff_pending = false;
	if (bl->curr_boost_req == true) {
		bl->curr_boost_req = false;
		DSI_DEBUG("%s request mode:%d\n",__func__, bl->boost_mode);
		if (bl->last_level) {
			/* set curr_boost on */
			rc = dsi_display_wait_for_te(display);
			if ((rc >= 0) && (dsi_panel_finger_boost_wait > 0)) {
				DSI_DEBUG("%s boost on delay st\n",__func__);
				udelay(dsi_panel_finger_boost_wait);
				DSI_DEBUG("%s boost on delay ed\n",__func__);
			}
			DSI_DEBUG("%s boost on last_level(%u)\n",__func__, bl->last_level);
...
#ifdef CONFIG_ARCH_CHARA
		rc = dsi_display_wait_for_te(display);
		if ((rc >= 0) && (dsi_panel_finger_boost_off_wait > 0)) {
			DSI_DEBUG("%s boost off delay st\n",__func__);
			udelay(dsi_panel_finger_boost_off_wait);
			DSI_DEBUG("%s boost off delay ed\n",__func__);
		}
#endif /* CONFIG_ARCH_CHARA */
		/* set curr_boost off */
		DSI_DEBUG("%s boost off last_level(%u)\n",__func__, bl->last_level);
		dsi_panel_bl_boost(panel, false);
		dsi_panel_update_curr_boosted(panel, false);

		if (panel->dimming_on_pending == true) {
			queue_delayed_work(panel->ordered_workqueue, &panel->dimming_work, 0);
			panel->dimming_on_pending = false;
		}
	}
...
static int dsi_panel_bl_boost(struct dsi_panel *panel, u32 onoff)
{
	...
	case DSI_BACKLIGHT_WLED:
		if (!bl->raw_bd) {
			DSI_ERR("%s: Invalid bl->raw_bd\n", __func__);
			return -EINVAL;
		}
		bl->raw_bd->props.ignore_dbc = onoff;
		rc = backlight_device_set_brightness(bl->raw_bd, bl_lvl);
		if (rc) {
			bl->raw_bd->props.ignore_dbc = !onoff;
		}
	...
	case DSI_BACKLIGHT_DCS:
		...
		rc = dsi_panel_update_backlight(panel, bl_lvl);
		if (onoff) {
			drm_peak_nits_set(NITS_USER_BOOST, true);
		} else {
			drm_peak_nits_set(NITS_USER_BOOST, false);
		}
		break;
	default:
		DSI_ERR("Backlight type(%d) not supported\n", bl->type);
...
int dsi_panel_boost_work_init(struct dsi_panel *panel, bool Initialize)
{
	...
	if (Initialize) {
		...
		INIT_DELAYED_WORK(&panel->dimming_work, dsi_panel_dimming_work);
	} else {
		if (panel->ordered_workqueue) {
			cancel_delayed_work_sync(&panel->curr_boost_work);
			cancel_delayed_work_sync(&panel->dimming_work);
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01014 */
static int dsi_panel_bl_dimming(struct dsi_panel *panel, u32 onoff)
{
	int ret = 0;
	struct dsi_display *display = NULL;
#ifdef CONFIG_ARCH_RECOA
	...
	struct dsi_cmd_desc cmd[] = {
		...
		{{0, MIPI_DSI_DCS_SHORT_WRITE_PARAM , MIPI_DSI_MSG_USE_LPM | MIPI_DSI_MSG_UNICAST, 0, 0, 2, addr_value[2], 0, NULL}, 0, 0},
		{{0, MIPI_DSI_DCS_SHORT_WRITE_PARAM , MIPI_DSI_MSG_USE_LPM | MIPI_DSI_MSG_UNICAST, 0, 0, 2, addr_value[3], 0, NULL}, 0, 0},
		{{0, MIPI_DSI_DCS_SHORT_WRITE_PARAM , MIPI_DSI_MSG_USE_LPM | MIPI_DSI_MSG_UNICAST, 0, 0, 2, addr_value[4], 0, NULL}, 0, 0},
		{{0, MIPI_DSI_DCS_SHORT_WRITE_PARAM , MIPI_DSI_MSG_USE_LPM | MIPI_DSI_MSG_UNICAST, 0, 0, 2, addr_value[5], 0, NULL}, 0, 0},
		{{0, MIPI_DSI_DCS_SHORT_WRITE_PARAM , MIPI_DSI_MSG_USE_LPM | MIPI_DSI_MSG_UNICAST, 0, 0, 2, addr_value[6], 0, NULL}, 1, 0},
	};
	...
	struct dsi_cmd_desc cmd[] = {
		{{0, MIPI_DSI_GENERIC_LONG_WRITE , MIPI_DSI_MSG_USE_LPM | MIPI_DSI_MSG_UNICAST, 0, 0, 2, addr_value[0], 0, NULL}, 0, 0},
		{{0, MIPI_DSI_GENERIC_LONG_WRITE , MIPI_DSI_MSG_USE_LPM | MIPI_DSI_MSG_UNICAST, 0, 0, 2, addr_value[1], 0, NULL}, 1, 0},
	};
	unsigned char step_addr_value[][2] = {
		{0xFE, 0x72},
		{0x1A, 0x01},
	};
	struct dsi_cmd_desc step_cmd[] = {
		{{0, MIPI_DSI_GENERIC_LONG_WRITE , MIPI_DSI_MSG_USE_LPM | MIPI_DSI_MSG_UNICAST, 0, 0, 2, step_addr_value[0], 0, NULL}, 0, 0},
		{{0, MIPI_DSI_GENERIC_LONG_WRITE , MIPI_DSI_MSG_USE_LPM | MIPI_DSI_MSG_UNICAST, 0, 0, 2, step_addr_value[1], 0, NULL}, 1, 0},
	};
#endif /* CONFIG_ARCH_RECOA */

	DSI_DEBUG("%s:in onoff=%d\n", __func__, onoff);
	...
	ret = drm_cmn_dsi_cmds_transfer(display, cmd, ARRAY_SIZE(cmd));
	if (ret) {
		DSI_ERR("[%s] dimming change error:%d\n", __func__, (int)ret);
		return ret;
	}
#ifndef CONFIG_ARCH_RECOA
	if (onoff == DIMMING_STATUS_ON) {
		ret = drm_cmn_dsi_cmds_transfer(display, step_cmd, ARRAY_SIZE(step_cmd));
		if (ret) {
			DSI_ERR("[%s] dimming step change error:%d\n", __func__, (int)ret);
			return ret;
		}
	}
#endif /* CONFIG_ARCH_RECOA */

	panel->dimming_status = onoff;
...
int dsi_panel_request_dimming(struct backlight_device *bd, u32 onoff)
{
	...
	panel = to_dsi_panel(bd);
	if (!panel || !panel->ordered_workqueue) {
		DSI_ERR("%s: Invalid dsi_panel data\n", __func__);
		return -EINVAL;
	}
	bl = &panel->bl_config;
	...
	if (panel->dimming_status == onoff) {
		DSI_DEBUG("%s:dimming_status not change\n",__func__);
		mutex_unlock(&panel->curr_boost_lock);
		mutex_unlock(&panel->panel_lock);
		return 0;
	}
	...
	switch (onoff) {
	case DIMMING_STATUS_OFF:
		/* Set BKL fast curve */
		DSI_DEBUG("dimming off reauest\n");
		panel->dimming_on_pending = false;
		dsi_panel_bl_dimming(panel, DIMMING_STATUS_OFF);
		DSI_DEBUG("dimming off wait st\n");
		usleep_range(DSI_PANEL_DIMMING_WAIT, DSI_PANEL_DIMMING_WAIT + 10);
		DSI_DEBUG("dimming off wait ed\n");
		break;
	case DIMMING_STATUS_ON:
		/* Set BKL slow curve */
		DSI_DEBUG("dimming on reauest\n");
		if (bl->curr_boosted == false) {
			panel->dimming_on_pending = false;
			DSI_DEBUG("dimming on wait st\n");
			usleep_range(DSI_PANEL_DIMMING_WAIT, DSI_PANEL_DIMMING_WAIT + 10);
			DSI_DEBUG("dimming on wait ed\n");
			dsi_panel_bl_dimming(panel, DIMMING_STATUS_ON);
		} else {
			panel->dimming_on_pending = true;
		}
		break;
	default:
		DSI_ERR("%s: Invalid onoff data\n", __func__);
		break;
	}
	mutex_unlock(&panel->curr_boost_lock);
	mutex_unlock(&panel->panel_lock);
...
static void dsi_panel_dimming_work(struct work_struct *work)
{
	...
	if (panel->dimming_status == panel->dimming_request) {
		DSI_DEBUG("%s:panel status == request\n",__func__);
		mutex_unlock(&panel->curr_boost_lock);
		mutex_unlock(&panel->panel_lock);
		return;
	}

	switch (panel->dimming_request) {
	case DIMMING_STATUS_OFF:
		dsi_panel_bl_dimming(panel, DIMMING_STATUS_OFF);
		panel->dimming_on_pending = false;
		DSI_DEBUG("dimming off wait st\n");
	...
	case DIMMING_STATUS_ON:
		if (bl->curr_boosted == false) {
			...
			DSI_WARN("%s:curr_boosted is true dimming on pending\n",__func__);
			panel->dimming_on_pending = true;
		}
		break;
	default:
...
int dsi_panel_update_dbc_brightness(struct backlight_device *bd, u32 dbc_brightness)
{
	...
	bl->raw_bd->props.dbc_brightness = dbc_brightness;
...
int dsi_panel_set_dfps(struct dsi_panel *panel, u32 fps)
{
	...
	if (panel->new_fps == fps) {
		DSI_DEBUG("[%s] ignore duplicated dfps(%u)\n", panel->name, fps);
		goto error;
	}
	...
	case 60:
		...
		break;
	case 30:
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DFPS_30);
		break;
	default:
		DSI_ERR("fps num(%u) not supported\n", fps);
		rc = -ENOTSUPP;
		goto error;
	}

	if (rc) {
		DSI_ERR("[%s] failed to send DSI_CMD_SET_DFPS_(%u) cmds, rc=%d\n",
		       panel->name, fps, rc);
		goto error;
	}
...
int dsi_panel_aod_setting(struct dsi_panel *panel)
{
	...
	drm_drive_mode_refresh_rate_update(panel, 0);

	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_AOD_SETTING);
	if (rc)
		DSI_ERR("[%s] failed to send DSI_CMD_SET_AOD_SETTING cmds, rc=%d\n",
		       panel->name, rc);
exit:
	mutex_unlock(&panel->panel_lock);
	return rc;
...
int dsi_panel_aod_start(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_AOD_START);
	if (rc)
		DSI_ERR("[%s] failed to send DSI_CMD_SET_AOD_START cmds, rc=%d\n",
		       panel->name, rc);
exit:
	mutex_unlock(&panel->panel_lock);
	return rc;
...
int dsi_panel_exit_aod(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_EXIT_AOD);
	if (rc)
		DSI_ERR("[%s] failed to send DSI_CMD_SET_EXIT_AOD cmds, rc=%d\n",
		       panel->name, rc);
exit:
	mutex_unlock(&panel->panel_lock);
	...
	struct device_node *of_node)
{
	...
	if (of_get_property(of_node,
		...
		DSI_DEBUG("%s:Property linear-params not available\n", __func__);
		return 0;
	}
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01017 */ /* CUST_ID_01021 */ /* CUST_ID_01027 */
...
static int dsi_panel_parse_therm(struct dsi_panel *panel)
{
	...
	rc = of_property_match_string(dev->of_node,
			"io-channel-names", "lcd_therm");
	if (rc < 0) {
		DSI_DEBUG("%s: of_property_match_string rc=%d\n", __func__, rc);
		return 0;
	}

	DSI_DEBUG("%s: device_name = [%s]\n", __func__, dev->kobj.name);
	panel->display_therm = iio_channel_get(dev, "lcd_therm");
