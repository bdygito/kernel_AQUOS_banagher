...
static int aod_det_interval = 15000;
module_param(aod_det_interval, int, 0664);
static int aod_move_interval = 60 * 1000;
module_param(aod_move_interval, int, 0664);

static void sde_connector_set_aod_status(struct sde_connector *c_conn, int mode);
static void sde_connector_check_lp_status_work(struct work_struct *work);
static enum alarmtimer_restart sde_connector_check_status_expired(struct alarm *alarm, ktime_t now);
static int sde_connector_wait_for_susend_exit(struct drm_device *dev);
#endif /* CONFIG_SHARP_DISPLAY*/
...
static const struct backlight_ops sde_backlight_device_ops = {
	...
	.get_dev_brightness = dsi_panel_get_dev_brightness,
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00052 */
	.update_dbc_brightness = dsi_panel_update_dbc_brightness,
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01014 */
#if defined(CONFIG_ARCH_RECOA) || defined(CONFIG_ARCH_CHARA)
	.req_bl_dimming = dsi_panel_request_dimming,
#else /* defined(CONFIG_ARCH_RECOA) || defined(CONFIG_ARCH_CHARA) */
	.req_bl_dimming = NULL,
#endif /* defined(CONFIG_ARCH_RECOA) || defined(CONFIG_ARCH_CHARA) */
#endif /* CONFIG_SHARP_DISPLAY */
};
...
static int _sde_connector_update_power_locked(struct sde_connector *c_conn)
{
	...
	if (mode != SDE_MODE_DPMS_ON) {
		...
		sde_connector_set_aod_status(c_conn, mode);
	} else {
		sde_connector_set_aod_status(c_conn, mode);
	...
	bool skip_pre_kickoff)
{
	...
	if (!display) {
		...
		if (display->recovery_cnt >= DSI_DISPLAY_RETRY_RECOVERY_CNT) {
			...
			SDE_ERROR("%s:recovery retry over. recovery_cnt=%d\n",
	...
	msm_mode_object_event_notify(&conn->base.base,
		conn->base.dev, &event, (u8 *)&conn->panel_dead, true);
#else /* CONFIG_SHARP_DISPLAY */
	msm_mode_object_event_notify(&conn->base.base,
...
int sde_connector_esd_status(struct drm_connector *conn)
{
	...
	if (!display || !display->panel) {
		SDE_ERROR("Invalid params(s) display %pK, panel %pK\n",
			display,
			((display) ? display->panel : NULL));
		return -EINVAL;
	}
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00203 */
	alarm_init(&c_conn->aod_check_status_alarm, ALARM_BOOTTIME, sde_connector_check_status_expired);
	INIT_WORK(&c_conn->aod_check_status_work, sde_connector_check_lp_status_work);
	c_conn->aod_expire_cnt = 0;
#endif /* CONFIG_SHARP_DISPLAY*/
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00203 */
static void sde_connector_set_aod_alarm(struct sde_connector *c_conn, bool enable)
{
	struct timespec ts;
	uint64_t temp = aod_det_interval;

	if (!c_conn) {
		SDE_ERROR("%s: Invalid input data\n", __func__);
		return;
	}

	alarm_try_to_cancel(&c_conn->aod_check_status_alarm);
	cancel_work_sync(&c_conn->aod_check_status_work);
	SDE_DEBUG("%s alarm cancel\n", __func__);

	if (enable == true) {
		do_div(temp,1000);
		ts.tv_sec = temp;
		ts.tv_nsec = (aod_det_interval - (ts.tv_sec * 1000)) * 1000000;
		c_conn->aod_expire_cnt = 0;

		SDE_DEBUG("%s alarm start\n", __func__);
		alarm_start_relative(&c_conn->aod_check_status_alarm,
...
static void sde_connector_set_aod_status(struct sde_connector *c_conn, int mode)
{
	...
	if (!dsi_display) {
		SDE_ERROR("%s: dsi_display is NULL\n", __func__);
		return;
	}
	...
	if (c_conn->connector_type != DRM_MODE_CONNECTOR_DSI) {
		SDE_DEBUG("%s: not connector DSI(%d)\n", __func__, c_conn->connector_type);
		return;
	}
	...
	if (mode == SDE_MODE_DPMS_LP1) {
		sde_connector_set_aod_alarm(c_conn, true);
	} else if (mode != SDE_MODE_DPMS_LP2) {
		sde_connector_set_aod_alarm(c_conn, false);
	}
}
...
static void sde_connector_check_lp_status_work(struct work_struct *work)
{
	struct sde_connector *c_conn = NULL;
	int ret = 0;
	int interval_threshold = 0;

	c_conn = container_of(work, struct sde_connector, aod_check_status_work);
	if (!c_conn) {
		SDE_ERROR("%s: Invalid input data\n", __func__);
		return;
	}

	ret = sde_connector_wait_for_susend_exit(c_conn->base.dev);
	if (ret) {
		return;
	}
	...
	if (!c_conn->ops.check_status ||
		...
		SDE_DEBUG("%s not lp mode\n", __func__);
		mutex_unlock(&c_conn->lock);
		return;
	}

	ret = c_conn->ops.check_status(&c_conn->base, c_conn->display, false);
	SDE_DEBUG("%s check status(%d)\n", __func__, ret);
	mutex_unlock(&c_conn->lock);
	...
	if ((aod_det_interval != 0) && (aod_move_interval != 0)) {
		interval_threshold = aod_move_interval / aod_det_interval;
	}

	c_conn->aod_expire_cnt++;
	if (c_conn->aod_expire_cnt >= interval_threshold) {
		c_conn->ops.aod_disp_move(c_conn->display);
		c_conn->aod_expire_cnt = 0;
	}
}
...
static enum alarmtimer_restart sde_connector_check_status_expired(struct alarm *alarm, ktime_t now)
{
	int timer_ms = aod_det_interval;
	struct sde_connector *c_conn = NULL;

	c_conn = container_of(alarm, struct sde_connector, aod_check_status_alarm);

	if (c_conn)
		schedule_work(&c_conn->aod_check_status_work);

	alarm_forward_now(alarm, ms_to_ktime((u64)timer_ms));

	SDE_DEBUG("%s alarm restart\n", __func__);
...
static int sde_connector_wait_for_susend_exit(struct drm_device *dev)
{
	...
	if (!dev) {
		SDE_ERROR("%s: drm_device is NULL\n", __func__);
		return -EINVAL;
	}

	for (i = 0; i < 100; i++) {
		if (!sde_kms_is_suspend_blocked(dev)) {
			SDE_DEBUG("%s: suspend exit\n", __func__);
			return 0;
		}
		SDE_DEBUG("%s: suspend: sleep 10ms\n", __func__);
		usleep_range(10000, 10010);
	}

	SDE_ERROR("%s: suspend check is time out\n", __func__);
	return -ETIME;
}
#endif /* CONFIG_SHARP_DISPLAY*/

#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00210 */
void sde_connector_update_dfps(struct drm_connector *connector)
{
	struct sde_connector *c_conn = NULL;
	int ret = 0;
	...
	if (c_conn->ops.update_dfps) {
		ret = c_conn->ops.update_dfps(c_conn->display);
		SDE_DEBUG("%s update_dfps ret(%d)\n", __func__, ret);
	}
}
#endif /* CONFIG_SHARP_DISPLAY*/
