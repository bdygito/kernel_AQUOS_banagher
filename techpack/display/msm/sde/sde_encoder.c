...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00063 */
	if (drm_enc && drm_enc->bridge) {
		#define to_dsi_bridge(x)     container_of((x), struct dsi_bridge, base)
		struct dsi_bridge *c_bridge = to_dsi_bridge(drm_enc->bridge);

		if (c_bridge && c_bridge->display && c_bridge->display->panel) {
			if (atomic_read(&c_bridge->display->panel->esd_recovery_pending)) {
				pr_warn("%s:ESD recovery already pending\n", __func__);
				return 0;
			}
		}
	}
	...
	struct msm_drm_private *priv, bool is_vid_mode)
{
	...
	if (is_vid_mode) {
		...
		if (cur_mode->vrefresh == 30) {
			if (sde_enc->early_wakeup_required) {
				SDE_DEBUG_ENC(sde_enc, "notify early wakeup event:%d\n",
					cur_mode->vrefresh);
				event.type = DRM_EVENT_EARLY_WAKEUP;
				event.length = sizeof(bool);
				msm_mode_object_event_notify(&drm_conn->base, drm_conn->dev,
					&event, (u8 *)&sde_enc->early_wakeup_required, false);
				sde_enc->early_wakeup_required = false;
			}
		}
		else {
			sde_enc->early_wakeup_required = true;
		}
		return 0;
	}
#endif /* CONFIG_SHARP_DISPLAY */
...
#else /* CONFIG_SHARP_DISPLAY_1HZ_CUST */
		if (!sde_crtc_frame_pending(sde_enc->crtc))
#endif /* CONFIG_SHARP_DISPLAY_1HZ_CUST */
...
#ifndef CONFIG_SHARP_DISPLAY /* CUST_ID_00208 */
	if (!sde_encoder_check_curr_mode(drm_enc, MSM_DISPLAY_CMD_MODE))
		return;
#endif /* CONFIG_SHARP_DISPLAY */
		...
		struct sde_encoder_phys *phy_enc)
{
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00030 */ /* CUST_ID_00044 */
	...
	if(sde_encoder_is_dsi_display(drm_enc)) {
		...
		if (display->panel->cur_mode->timing.h_skew != 0) {
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00208 */
	SDE_DEBUG_ENC(sde_enc, "%s:EarlyWakeUp\n", __func__);
#endif /* CONFIG_SHARP_DISPLAY */
		...
		struct sde_encoder_kickoff_params *params)
{
	...
	sde_connector_update_dfps(sde_enc->cur_master->connector);
#endif /* CONFIG_SHARP_DISPLAY */
