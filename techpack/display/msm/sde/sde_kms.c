...
static uint extension_wait_us = 500;
module_param(extension_wait_us, uint, 0660);
#endif /* CONFIG_SHARP_DISPLAY */
		...
		struct sde_kms *sde_kms)
{
	static const struct sde_connector_ops dsi_ops = {
		...
		.aod_disp_move = dsi_display_aod_disp_move,
#endif /* CONFIG_SHARP_DISPLAY*/
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00210 */
		.update_dfps = dsi_display_update_dfps,
#endif /* CONFIG_SHARP_DISPLAY*/
	...
	struct device *dev)
{
	...
	drm_for_each_connector_iter(conn, &conn_iter) {
		...
		if (ret && ret != -EWOULDBLOCK) {
			...
			usleep_range(extension_wait_us, extension_wait_us + 10);
