...
#if defined(CONFIG_SHARP_DISPLAY) && defined(CONFIG_ARCH_RECOA) /* CUST_ID_01024 */
static const struct dp_panel_info ar100_panel = {
	.h_active = 640,
	.v_active = 360,
	.h_back_porch = 48,
	.h_front_porch = 16,
...
#if defined(CONFIG_SHARP_DISPLAY) && defined(CONFIG_ARCH_RECOA) /* CUST_ID_01024 */
	if ((connector) &&
		(connector->panel_device == DRM_MODE_PANEL_AR100)) {
		memcpy(&mode->timing, &ar100_panel,
			sizeof(ar100_panel));
		DRM_DEBUG_KMS("AR100 timing h(%u) v(%u)\n", mode->timing.h_active,
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_01016 */
