...
static unsigned int sh_limit_freq_gold = 1958400;
static unsigned int sh_limit_freq_gold_plus = UINT_MAX;
static unsigned int sh_load_sum_mode = 2;
static unsigned int sh_release_limit_enter_load = 50;
static unsigned int sh_release_limit_enter_time = 150000;
static unsigned int sh_release_limit_exit_load = 50;
static unsigned int sh_release_limit_exit_time = 50000;
static unsigned int sh_limit_fps_freq_silver = UINT_MAX;
static unsigned int sh_limit_fps_freq_gold = 1958400;
static unsigned int sh_limit_fps_freq_gold_plus = UINT_MAX;
#else /* CONFIG_ARCH_BANAGHER */
static unsigned int sh_limit_freq_silver = 1516800;
static unsigned int sh_limit_freq_gold = 1248000;
static unsigned int sh_limit_freq_gold_plus = UINT_MAX;
static unsigned int sh_load_sum_mode = 2;
static unsigned int sh_release_limit_enter_load = 50;
static unsigned int sh_release_limit_enter_time = 83000;
static unsigned int sh_release_limit_exit_load = 50;
static unsigned int sh_release_limit_exit_time = 50000;
static unsigned int sh_limit_fps_freq_silver = UINT_MAX;
...
static bool notifier_register_done = false;
				   ...
				   unsigned int next_freq)
{
	...
	if (sg_policy->next_freq == next_freq &&
		...
		max(per_cpu(sh_load_limit_freq, sg_policy->policy->cpu), per_cpu(sh_fps_limit_freq, sg_policy->policy->cpu)) >= next_freq)
...
static int drm_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (event == DRM_PANEL_EVENT_DPMS) {
		if (evdata && evdata->data) {
			...
			switch (blank) {
			case DRM_PANEL_EVENT_DPMS_ON:
				sh_disp_on = true;
				break;
			case DRM_PANEL_EVENT_DPMS_OFF:
				sh_disp_on = false;
				break;
			default:
				break;
			}
			pr_debug("display status notify event:%lu blank:%d sh_disp_on:%d\n", event, blank, sh_disp_on);
			return 0;
		}
	}
	pr_debug("display status notify event:%lu blank:- sh_disp_on:%d\n", event, sh_disp_on);
	return 0;
}
#endif /* CONFIG_SHARP_PNP_CLOCK */
...
#ifdef CONFIG_SHARP_PNP_CLOCK
	sh_release_limit(sg_cpu, sg_cpu->util);
#endif /* CONFIG_SHARP_PNP_CLOCK */
	max = sg_cpu->max;
	sg_cpu->flags = flags;
...
static int sugov_start(struct cpufreq_policy *policy)
{
	...
	unsigned int cpu;
#ifdef CONFIG_SHARP_PNP_CLOCK
	struct drm_panel *active_panel = sh_get_active_panel();
#endif /* CONFIG_SHARP_PNP_CLOCK */
...
#ifdef CONFIG_SHARP_PNP_CLOCK
	if (!notifier_register_done && active_panel) {
		notifier_register_done = true;
